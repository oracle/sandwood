...
class Flip2CoinsMK9$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements Flip2CoinsMK9$CoreInterface {
	...
	public final void set$fixedFlag$sample20(boolean calculationVariable$value) {
		...
		// 
		// Substituted "fixedFlag$sample20" with its value "calculationVariable$value".
		fixedProbFlag$sample27 = (calculationVariable$value && fixedProbFlag$sample27);
	}
	...
	@Override
	public final void set$fixedFlag$sample27(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample27 including if probabilities
		// need to be updated.
		fixedFlag$sample27 = calculationVariable$value;
		
		// Should the probability of sample 27 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample27" with its value "calculationVariable$value".
		fixedProbFlag$sample27 = (calculationVariable$value && fixedProbFlag$sample27);
	}
	...
	private final void logProbabilityValue$sample20() {
		...
		if(!fixedProbFlag$sample20) {
			...
			for(int j=0; j<coins; j+=1)
				...
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBeta(bias[j], 1.0, 1.0));
			logProbability$beta = calculationVariable$sampleAccumulator;
			
			// Store the random variable instance probability
			logProbability$bias = calculationVariable$sampleAccumulator;
	...
	private final void logProbabilityValue$sample27() {
		...
		if(!fixedProbFlag$sample27) {
			...
			for(int j=0; j<coins; j+=1) {
				...
				for(int var23=0; var23<shape[j]; var23+=1)
					...
					// 
					// The sample value to calculate the probability of generating
					calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flipsMeasured[j][var23], bias[j]));
				...
				// Store the random variable instance probability
				logProbability$sample27[j] = calculationVariable$sampleAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=0; j<coins; j+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample27[j];
	...
	private final void sample20(int j) {
		...
		for(int var23=0; var23<shape[j]; var23+=1) {
			...
			if(flipsMeasured[j][var23])
				calculationVariable$Sum = (calculationVariable$Sum + 1);
		}
		
		// Write out the new value of the sample.
		bias[j] = Conjugates.sampleConjugateBetaBinomial(RNG$, 1.0, 1.0, calculationVariable$Sum, calculationVariable$Count);
	}
