...
class Flip2CoinsMK1$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Flip2CoinsMK1$CoreInterface {
	...
	private final void logProbabilityValue$sample16() {
		...
		if(!fixedProbFlag$sample16) {
			...
			for(int var13=0; var13<coins; var13+=1)
				...
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBeta(bias[var13], 1.0, 1.0));
	...
	// conjugate prior.
	private final void sample16(int var13, int threadID$calculationVariable$var13, Rng RNG$) {
		...
		for(int var28=0; var28<length$flipsMeasured[var13]; var28+=1) {
			...
			// 
			// Substituted "j" with its value "var13".
			if(flipsMeasured[var13][var28])
				calculationVariable$Sum = (calculationVariable$Sum + 1);
		}
		
		// Write out the value of the sample to a temporary variable prior to updating the
		// intermediate variables.
		bias[var13] = Conjugates.sampleConjugateBetaBinomial(RNG$, 1.0, 1.0, calculationVariable$Sum, calculationVariable$Count);
	}
