...
class HMMTestPart3$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart3$CoreInterface {
	...
	private double[] calculationVariable$var43$StateProbabilityGlobal;
	...
	private final void logProbabilityValue$sample45() {
		...
		if(!fixedProbFlag$sample45) {
			...
			for(int i$var37=1; i$var37<samples; i$var37+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityCategorical(st[i$var37], m[st[(i$var37 - 1)]]);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var42[(i$var37 - 1)] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample45[(i$var37 - 1)] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int i$var37=1; i$var37<samples; i$var37+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample45[(i$var37 - 1)];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var42[(i$var37 - 1)] = calculationVariable$RVaccumulator;
			}
	...
	private final void logProbabilityValue$sample54() {
		...
		if(!fixedProbFlag$sample54) {
			...
			for(int j=0; j<samples; j+=1) {
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var51[j] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample54[j] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=0; j<samples; j+=1) {
				// Variable declaration of calculationVariable$RVaccumulator moved.
				double calculationVariable$RVaccumulator = logProbability$sample54[j];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var51[j] = calculationVariable$RVaccumulator;
			}
	...
	private final void sample17(int var15, int threadID$calculationVariable$var15, Rng RNG$) {
		...
		// Substituted "calculationVariable$indexState$var266" with its value "0".
		calculationVariable$CountLocal[0] = 0.0;
		
		// Substituted "calculationVariable$indexState$var266" with its value "1".
		...
		for(int i$var37=1; i$var37<samples; i$var37+=1) {
			if((var15 == st[(i$var37 - 1)]))
				...
				// variable var42
				calculationVariable$CountLocal[st[i$var37]] = (calculationVariable$CountLocal[st[i$var37]] + 1.0);
		}
	...
	private final void sample35() {
		{
			...
			if((1 < samples))
				...
				// Substituted "i$var37" with its value "1".
				// 
				// Substituted "calculationVariable$temp$1$var41" with its value "var41".
				// 
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
			...
			if((0 < samples))
				...
				// Substituted "j" with its value "0".
				// 
				// Substituted "calculationVariable$temp$2$var50" with its value "var50".
				// 
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
	...
	private final void sample45(int i$var37) {
		{
			...
			// it is added to the index probabilities.
			// 
			// calculationVariable$temp$0$var41's comment
			// Constructing a random variable input for use later.
			double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(0, m[st[(i$var37 - 1)]]);
			...
			// inputs.
			// 
			// Substituted "j" with its value "i$var37".
			// 
			// Substituted "calculationVariable$temp$2$var50" with its value "var50".
			// 
			// Constructing a random variable input for use later.
			...
			// 
			// Substituted "calculationVariable$valuePos" with its value "0".
			calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flipsMeasured[i$var37], bias[0]) + calculationVariable$AccumulatedProbabilities);
			...
			// 
			// Initialize a counter to track the reached distributions.
			calculationVariable$var43$StateProbabilityGlobal[0] = calculationVariable$AccumulatedProbabilities;
		}
		...
		// calculationVariable$temp$0$var41's comment
		// Constructing a random variable input for use later.
		double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(1, m[st[(i$var37 - 1)]]);
		int index$i$1_2 = (i$var37 + 1);
		if((index$i$1_2 < samples))
			// Processing sample task 45 of consumer random variable null.
			// 
		...
		// 
		// Substituted "calculationVariable$valuePos" with its value "1".
		calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flipsMeasured[i$var37], bias[1]) + calculationVariable$AccumulatedProbabilities);
		...
		// 
		// Initialize a counter to track the reached distributions.
		calculationVariable$var43$StateProbabilityGlobal[1] = calculationVariable$AccumulatedProbabilities;
		...
		// 
		// Get a local reference to the scratch space.
		double calculationVariable$lseMax = calculationVariable$var43$StateProbabilityGlobal[0];
		
		// Find max value.
		// 
		// Get a local reference to the scratch space.
		for(int calculationVariable$lseIndex=1; calculationVariable$lseIndex<calculationVariable$var43$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1) {
			// Get a local reference to the scratch space.
			double calculationVariable$lseElementValue = calculationVariable$var43$StateProbabilityGlobal[calculationVariable$lseIndex];
			if((calculationVariable$lseMax < calculationVariable$lseElementValue))
				calculationVariable$lseMax = calculationVariable$lseElementValue;
		}
		...
		// Sum the values in the array.
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$lseIndex=0; calculationVariable$lseIndex<calculationVariable$var43$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1)
				// Get a local reference to the scratch space.
				calculationVariable$lseSum = (calculationVariable$lseSum + Math.exp((calculationVariable$var43$StateProbabilityGlobal[calculationVariable$lseIndex] - calculationVariable$lseMax)));
		...
		if((calculationVariable$logSum == Double.NEGATIVE_INFINITY)) {
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var43$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var43$StateProbabilityGlobal[calculationVariable$indexName] = (1.0 / calculationVariable$var43$StateProbabilityGlobal.length);
		} else {
			// Normalize log space values and move to normal space
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var43$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var43$StateProbabilityGlobal[calculationVariable$indexName] = Math.exp((calculationVariable$var43$StateProbabilityGlobal[calculationVariable$indexName] - calculationVariable$logSum));
		}
		...
		// 
		// Get a local reference to the scratch space.
		st[i$var37] = DistributionSampling.sampleCategorical(RNG$, calculationVariable$var43$StateProbabilityGlobal);
	}
	...
	public final void allocateScratch() {
		...
		// Allocation of calculationVariable$var43$StateProbabilityGlobal for single threaded
		// execution
		// 
		// Variable to record the maximum value of Task Get 43. Initially set to the value
		// of putTask 18.
		calculationVariable$var43$StateProbabilityGlobal = new double[2];
	}
