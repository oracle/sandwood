...
class NullModelMK3$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements NullModelMK3$CoreInterface {
	...
	public NullModelMK3$MultiThreadCPU(ExecutionTarget target) {
		super(target);
	}
	...
	// Setter for eta.
	@Override
	public final void set$eta(double calculationVariable$value) {
		eta = calculationVariable$value;
	}
	...
	@Override
	public final void set$observedPositiveCount(int calculationVariable$value) {
		observedPositiveCount = calculationVariable$value;
	}
	...
	@Override
	public final void set$observedSampleCount(int calculationVariable$value) {
		observedSampleCount = calculationVariable$value;
	}
	...
	@Override
	public final void set$positiveCount(int calculationVariable$value) {
		positiveCount = calculationVariable$value;
	}
	...
	private final void logProbabilityValue$sample10() {
		...
		if(!fixedProbFlag$sample10) {
			...
			// 
			// The sample value to calculate the probability of generating
			double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityUniform(bias, min, 1.0);
	...
	private final void logProbabilityValue$sample12() {
		...
		if(!fixedProbFlag$sample12) {
			...
			// 
			// The sample value to calculate the probability of generating
			double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBinomial(observedPositiveCount, bias, observedSampleCount);
			
			// Add the probability of this sample task to the sample task accumulator.
			// 
			// Accumulator for sample probabilities for a specific instance of the random variable.
			logProbability$binomial = calculationVariable$distributionAccumulator;
			
			// Store the sample task probability
			logProbability$positiveCount = calculationVariable$distributionAccumulator;
	...
	private final void sample10() {
		...
		double calculationVariable$OriginalValue = bias;
		
		// The proposed new value for the sample
		// 
		// The original value of the sample
		double calculationVariable$ProposedValue = (bias + ((bias == 0)?DistributionSampling.sampleGaussian(RNG$, 0.0, 3):(bias * DistributionSampling.sampleGaussian(RNG$, 0.0, 0.001))));
		
		// Variable declaration of calculationVariable$OriginalProbability moved.
		// Declaration comment was:
		...
		// 
		// Substituted "calculationVariable$temp$1$var8" with its value "1.0".
		// 
		// Set the current value to the current state of the tree.
		// 
		// The original value of the sample
		double calculationVariable$OriginalProbability = (DistributionSampling.logProbabilityBinomial(observedPositiveCount, bias, observedSampleCount) + DistributionSampling.logProbabilityUniform(bias, min, 1.0));
		
		// Update Sample and intermediate values
		// 
		// Write out the new value of the sample.
		bias = calculationVariable$ProposedValue;
		...
		// 
		// Substituted "calculationVariable$temp$1$var8" with its value "1.0".
		double calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityBinomial(observedPositiveCount, calculationVariable$ProposedValue, observedSampleCount) + DistributionSampling.logProbabilityUniform(calculationVariable$ProposedValue, min, 1.0));
		...
		// 
		// Variable declaration of calculationVariable$ProposedProbability moved.
		// Declaration comment was:
		...
		// 
		// Initialize a counter to track the reached distributions.
		if((((calculationVariable$AccumulatedProbabilities - calculationVariable$OriginalProbability) <= Math.log(DistributionSampling.sampleUniform(RNG$, 0.0, 1.0))) || Double.isNaN((calculationVariable$AccumulatedProbabilities - calculationVariable$OriginalProbability))))
			// If it is not revert the changes.
			// 
			// Set the sample value
			// 
			// Write out the new value of the sample.
			bias = calculationVariable$OriginalValue;
	}
