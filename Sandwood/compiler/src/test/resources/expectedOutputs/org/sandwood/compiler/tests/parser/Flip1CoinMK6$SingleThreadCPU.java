...
class Flip1CoinMK6$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements Flip1CoinMK6$CoreInterface {
	...
	public final void set$fixedFlag$sample17(boolean calculationVariable$value) {
		...
		// 
		// Substituted "fixedFlag$sample17" with its value "calculationVariable$value".
		fixedProbFlag$sample29 = (calculationVariable$value && fixedProbFlag$sample29);
	}
	...
	@Override
	public final void set$fixedFlag$sample29(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample29 including if probabilities
		// need to be updated.
		fixedFlag$sample29 = calculationVariable$value;
		
		// Should the probability of sample 29 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample29" with its value "calculationVariable$value".
		fixedProbFlag$sample29 = (calculationVariable$value && fixedProbFlag$sample29);
	}
	...
	private final void logProbabilityValue$sample23() {
		...
		if(!fixedProbFlag$sample23) {
			...
			for(int var18=0; var18<samples1; var18+=1)
				...
				// The sample value to calculate the probability of generating
				// 
				// Substituted "i$var29" with its value "var18".
			...
			logProbability$bernoulli = (logProbability$bernoulli + calculationVariable$sampleAccumulator);
	...
	private final void logProbabilityValue$sample29() {
		...
		if(!fixedProbFlag$sample29) {
			...
			for(int var24=0; var24<samples2; var24+=1)
				...
				// The sample value to calculate the probability of generating
				// 
				// Substituted "i$var34" with its value "var24".
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flipsMeasured2[var24], bias));
			logProbability$bernoulli = (logProbability$bernoulli + calculationVariable$sampleAccumulator);
	...
	private final void sample17() {
		...
		for(int var18=0; var18<samples1; var18+=1) {
			...
			// 
			// Looking for a path between Sample 23 and consumer boolean 30.
		...
		for(int var24=0; var24<samples2; var24+=1) {
			// Check that the value can reach the sample task
			// 
			// Looking for a path between Sample 29 and consumer boolean 35.
			// Include the value sampled by task 29 from random variable bernoulli.
			// 
			// Increment the number of samples.
			calculationVariable$Count = (calculationVariable$Count + 1);
