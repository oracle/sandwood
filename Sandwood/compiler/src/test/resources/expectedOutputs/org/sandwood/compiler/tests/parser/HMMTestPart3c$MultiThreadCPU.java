...
class HMMTestPart3c$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart3c$CoreInterface {
	...
	private double[] calculationVariable$var46$StateProbabilityGlobal;
	...
	public final void set$fixedFlag$sample26(boolean calculationVariable$value) {
		...
		// 
		// Substituted "fixedFlag$sample26" with its value "calculationVariable$value".
		fixedProbFlag$sample57 = (calculationVariable$value && fixedProbFlag$sample57);
	}
	...
	@Override
	public final void set$fixedFlag$sample57(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample57 including if probabilities
		// need to be updated.
		fixedFlag$sample57 = calculationVariable$value;
		
		// Should the probability of sample 57 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample57" with its value "calculationVariable$value".
		fixedProbFlag$sample57 = (calculationVariable$value && fixedProbFlag$sample57);
	}
	...
	private final void logProbabilityValue$sample48() {
		...
		if(!fixedProbFlag$sample48) {
			...
			for(int i$var37=1; i$var37<samples; i$var37+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityCategorical(st[((i$var37 + i$var37) / 2)], m[st[(i$var37 - 1)]]);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var45[(i$var37 - 1)] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample48[(i$var37 - 1)] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int i$var37=1; i$var37<samples; i$var37+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample48[(i$var37 - 1)];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var45[(i$var37 - 1)] = calculationVariable$RVaccumulator;
			}
	...
	private final void logProbabilityValue$sample57() {
		...
		if(!fixedProbFlag$sample57) {
			...
			for(int j=0; j<samples; j+=1) {
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var54[j] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample57[j] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=0; j<samples; j+=1) {
				// Variable declaration of calculationVariable$RVaccumulator moved.
				double calculationVariable$RVaccumulator = logProbability$sample57[j];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var54[j] = calculationVariable$RVaccumulator;
			}
	...
	private final void sample17(int var15, int threadID$calculationVariable$var15, Rng RNG$) {
		...
		// Substituted "calculationVariable$indexState$var269" with its value "0".
		calculationVariable$CountLocal[0] = 0.0;
		
		// Substituted "calculationVariable$indexState$var269" with its value "1".
		...
		for(int i$var37=1; i$var37<samples; i$var37+=1) {
			if((var15 == st[(i$var37 - 1)]))
				...
				// variable var45
				calculationVariable$CountLocal[st[((i$var37 + i$var37) / 2)]] = (calculationVariable$CountLocal[st[((i$var37 + i$var37) / 2)]] + 1.0);
		}
	...
	private final void sample35() {
		{
			...
			if((1 < samples))
				...
				// Substituted "i$var37" with its value "1".
				// 
				// Substituted "calculationVariable$temp$1$var44" with its value "var44".
				// 
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
			...
			if((0 < samples))
				...
				// Substituted "j" with its value "0".
				// 
				// Substituted "calculationVariable$temp$2$var53" with its value "var53".
				// 
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
	...
	private final void sample48(int i$var37) {
		{
			...
			// it is added to the index probabilities.
			// 
			// calculationVariable$temp$0$var44's comment
			// Constructing a random variable input for use later.
			...
			if((index$i$1_2 < samples))
				...
				// 
				// Substituted "calculationVariable$valuePos" with its value "0".
				calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[((index$i$1_2 + index$i$1_2) / 2)], m[0]) + calculationVariable$AccumulatedProbabilities);
			...
			// 
			// Initialize a counter to track the reached distributions.
			calculationVariable$var46$StateProbabilityGlobal[0] = calculationVariable$AccumulatedProbabilities;
		}
		...
		if((index$i$1_2 < samples))
			...
			// 
			// Substituted "calculationVariable$valuePos" with its value "1".
			calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[((index$i$1_2 + index$i$1_2) / 2)], m[1]) + calculationVariable$AccumulatedProbabilities);
		int j = ((i$var37 + i$var37) / 2);
		if((j < samples))
			// Processing sample task 57 of consumer random variable null.
			// 
		...
		// 
		// Initialize a counter to track the reached distributions.
		calculationVariable$var46$StateProbabilityGlobal[1] = calculationVariable$AccumulatedProbabilities;
		...
		// 
		// Get a local reference to the scratch space.
		double calculationVariable$lseMax = calculationVariable$var46$StateProbabilityGlobal[0];
		
		// Find max value.
		// 
		// Get a local reference to the scratch space.
		for(int calculationVariable$lseIndex=1; calculationVariable$lseIndex<calculationVariable$var46$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1) {
			// Get a local reference to the scratch space.
			double calculationVariable$lseElementValue = calculationVariable$var46$StateProbabilityGlobal[calculationVariable$lseIndex];
			if((calculationVariable$lseMax < calculationVariable$lseElementValue))
				calculationVariable$lseMax = calculationVariable$lseElementValue;
		}
		...
		// Sum the values in the array.
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$lseIndex=0; calculationVariable$lseIndex<calculationVariable$var46$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1)
				// Get a local reference to the scratch space.
				calculationVariable$lseSum = (calculationVariable$lseSum + Math.exp((calculationVariable$var46$StateProbabilityGlobal[calculationVariable$lseIndex] - calculationVariable$lseMax)));
		...
		if((calculationVariable$logSum == Double.NEGATIVE_INFINITY)) {
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var46$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var46$StateProbabilityGlobal[calculationVariable$indexName] = (1.0 / calculationVariable$var46$StateProbabilityGlobal.length);
		} else {
			// Normalize log space values and move to normal space
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var46$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var46$StateProbabilityGlobal[calculationVariable$indexName] = Math.exp((calculationVariable$var46$StateProbabilityGlobal[calculationVariable$indexName] - calculationVariable$logSum));
		}
		...
		// 
		// Get a local reference to the scratch space.
		st[((i$var37 + i$var37) / 2)] = DistributionSampling.sampleCategorical(RNG$, calculationVariable$var46$StateProbabilityGlobal);
	}
	...
	public final void allocateScratch() {
		...
		// Allocation of calculationVariable$var46$StateProbabilityGlobal for single threaded
		// execution
		// 
		// Variable to record the maximum value of Task Get 46. Initially set to the value
		// of putTask 18.
		calculationVariable$var46$StateProbabilityGlobal = new double[2];
	}
