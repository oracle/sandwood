...
class Flip1CoinMK1b$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Flip1CoinMK1b$CoreInterface {
	...
	public Flip1CoinMK1b$MultiThreadCPU(ExecutionTarget target) {
		super(target);
	}
	...
	private final void logProbabilityValue$sample10() {
		...
		if(!fixedProbFlag$sample10) {
			...
			// 
			// The sample value to calculate the probability of generating
			double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBeta(bias, a, b);
	...
	private final void logProbabilityValue$sample16() {
		...
		if(!fixedProbFlag$sample16) {
			...
			for(int var13=0; var13<samples; var13+=1)
				...
				// 
				// The sample value to calculate the probability of generating
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flipsMeasured[var13], bias));
			logProbability$bernoulli = calculationVariable$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$flips = (logProbability$flips + calculationVariable$sampleAccumulator);
	...
	private final void sample10() {
		...
		for(int var13=0; var13<samples; var13+=1) {
			...
			if(flipsMeasured[var13])
				calculationVariable$Sum = (calculationVariable$Sum + 1);
		}
		
		// Write out the new value of the sample.
		bias = Conjugates.sampleConjugateBetaBinomial(RNG$, a, b, calculationVariable$Sum, calculationVariable$Count);
	}
			...
			parallelFor(RNG$, 0, samples, 1,
				(int forStart$var13, int forEnd$var13, int threadID$var13, org.sandwood.random.internal.Rng RNG$1) -> { 
						...
						for(int var13=forStart$var13; var13<forEnd$var13; var13+=1)
							flips[var13] = DistributionSampling.sampleBernoulli(RNG$1, bias);
				}
			);
