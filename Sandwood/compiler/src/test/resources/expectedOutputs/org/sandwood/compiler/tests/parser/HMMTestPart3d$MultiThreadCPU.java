...
class HMMTestPart3d$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart3d$CoreInterface {
	...
	private double[] calculationVariable$var34$StateProbabilityGlobal;
	...
	public final void set$fixedFlag$sample26(boolean calculationVariable$value) {
		...
		// 
		// Substituted "fixedFlag$sample26" with its value "calculationVariable$value".
		fixedProbFlag$sample79 = (calculationVariable$value && fixedProbFlag$sample79);
	}
	...
	@Override
	public final void set$fixedFlag$sample79(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample79 including if probabilities
		// need to be updated.
		fixedFlag$sample79 = calculationVariable$value;
		
		// Should the probability of sample 79 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample79" with its value "calculationVariable$value".
		fixedProbFlag$sample79 = (calculationVariable$value && fixedProbFlag$sample79);
	}
	...
	// Setter for st2.
	@Override
	public final void set$st2(int[] calculationVariable$value) {
		// Set st2 with flag to mark that it has been set so another array doesn't need to
		// be constructed
		st2 = calculationVariable$value;
		setFlag$st2 = true;
	}
	...
	private final void logProbabilityValue$sample36() {
		...
		if(!fixedProbFlag$sample36) {
			...
			// Variable declaration of calculationVariable$guard$st2 moved.
			// Declaration comment was:
			// Guard to ensure that st2 is only updated once for this probability.
			// 
			// Set the guard so the update is only applied once.
			boolean calculationVariable$guard$st2 = true;
			...
			// 
			// Accumulator for sample probabilities for a specific instance of the random variable.
			logProbability$st2 = (logProbability$st2 + calculationVariable$distributionAccumulator);
			
			// Looking for a path between Sample 36 and consumer int[] 68.
			for(int i$var43=1; i$var43<samples; i$var43+=1) {
				if(((0 == (indirection[(i$var43 - 1)][i$var43] / i$var43)) && !calculationVariable$guard$st2)) {
					// Set the guard so the update is only applied once.
					calculationVariable$guard$st2 = true;
	...
	private final void logProbabilityValue$sample52() {
		...
		if(!fixedProbFlag$sample52) {
			...
			for(int i$var43=1; i$var43<samples; i$var43+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityCategorical(st[i$var43], m[(samples - st2[(i$var43 - 1)])]);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var49[(i$var43 - 1)] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample52[(i$var43 - 1)] = calculationVariable$distributionAccumulator;
				
				// Guard to ensure that st2 is only updated once for this probability.
				boolean calculationVariable$guard$st2 = false;
				...
				for(int index$i$2_1=1; index$i$2_1<samples; index$i$2_1+=1) {
					if(((i$var43 == (indirection[(index$i$2_1 - 1)][index$i$2_1] / index$i$2_1)) && !calculationVariable$guard$st2)) {
						// Set the guard so the update is only applied once.
						calculationVariable$guard$st2 = true;
		...
		// Using cached values.
			...
			double calculationVariable$accumulator = 0.0;
			for(int i$var43=1; i$var43<samples; i$var43+=1) {
				double calculationVariable$sampleValue = logProbability$sample52[(i$var43 - 1)];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$sampleValue);
				logProbability$var49[(i$var43 - 1)] = calculationVariable$sampleValue;
				...
				for(int index$i$3_1=1; index$i$3_1<samples; index$i$3_1+=1) {
					if(((i$var43 == (indirection[(index$i$3_1 - 1)][index$i$3_1] / index$i$3_1)) && !calculationVariable$guard$st2)) {
						// Set the guard so the update is only applied once.
						calculationVariable$guard$st2 = true;
						
						// Update the variable probability
						logProbability$st2 = (logProbability$st2 + calculationVariable$sampleValue);
					}
				}
			}
	...
	private final void logProbabilityValue$sample79() {
		...
		if(!fixedProbFlag$sample79) {
			...
			for(int j=0; j<samples; j+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBernoulli(flipsMeasured[j], bias[(samples - st2[j])]);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var76[j] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample79[j] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=0; j<samples; j+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample79[j];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var76[j] = calculationVariable$RVaccumulator;
			}
	...
	private final void sample17(int var15, int threadID$calculationVariable$var15, Rng RNG$) {
		// A local reference to the scratch space.
		double[] calculationVariable$CountLocal = calculationVariable$var16$CountGlobal[threadID$calculationVariable$var15];
		
		// Substituted "calculationVariable$indexState$var632" with its value "0".
		calculationVariable$CountLocal[0] = 0.0;
		
		// Substituted "calculationVariable$indexState$var632" with its value "1".
		calculationVariable$CountLocal[1] = 0.0;
		...
		if((var15 == 0))
			...
			// variable var33
			calculationVariable$CountLocal[st[0]] = (calculationVariable$CountLocal[st[0]] + 1.0);
		...
		for(int i$var43=1; i$var43<samples; i$var43+=1) {
			if((var15 == (samples - st2[(i$var43 - 1)])))
				...
				// variable var49
				calculationVariable$CountLocal[st[i$var43]] = (calculationVariable$CountLocal[st[i$var43]] + 1.0);
		}
		...
		Conjugates.sampleConjugateDirichletCategorical(RNG$, v, calculationVariable$CountLocal, m[var15]);
	}

	// Method to perform the inference steps to calculate new values for the samples generated
	// by sample task 26 drawn from Beta 20. Inference was performed using a Beta to Bernoulli/Binomial
	// conjugate prior.
	private final void sample26(int var24, int threadID$calculationVariable$var24, Rng RNG$) {
	...
	private final void sample36() {
		{
			...
			// 
			// calculationVariable$temp$0$var32's comment
			...
			if((1 < samples))
				...
				// 
				// calculationVariable$temp$1$var48's comment
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
				// Declaration comment was:
			...
			for(int i$var43=1; i$var43<samples; i$var43+=1) {
				if((0 == (indirection[(i$var43 - 1)][i$var43] / i$var43))) {
					...
					if(((1 <= index$i$4_4) && (index$i$4_4 < samples)))
						...
						// inputs.
						// 
						// calculationVariable$temp$2$var48's comment
						// Constructing a random variable input for use later.
						// 
						// Variable declaration of calculationVariable$CurrentValue moved.
						// Declaration comment was:
						...
						// 
						// Substituted "calculationVariable$valuePos" with its value "0".
						calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[index$i$4_4], m[0]) + calculationVariable$AccumulatedProbabilities);
				}
			}
			...
			if((0 < samples))
				...
				// 
				// calculationVariable$temp$3$var75's comment
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
				// Declaration comment was:
			...
			for(int i$var43=1; i$var43<samples; i$var43+=1) {
				if((0 == (indirection[(i$var43 - 1)][i$var43] / i$var43))) {
					...
					if(((0 <= j) && (j < samples)))
						...
						// inputs.
						// 
						// calculationVariable$temp$4$var75's comment
						// Constructing a random variable input for use later.
						// 
						// Variable declaration of calculationVariable$CurrentValue moved.
						// Declaration comment was:
						...
						// 
						// Substituted "calculationVariable$valuePos" with its value "0".
						calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flipsMeasured[j], bias[0]) + calculationVariable$AccumulatedProbabilities);
				}
			}
			...
			calculationVariable$var34$StateProbabilityGlobal[0] = calculationVariable$AccumulatedProbabilities;
		}
		...
		for(int i$var43=1; i$var43<samples; i$var43+=1) {
			if((0 == (indirection[(i$var43 - 1)][i$var43] / i$var43))) {
				...
				if(((1 <= index$i$4_4) && (index$i$4_4 < samples)))
					...
					// 
					// Substituted "calculationVariable$valuePos" with its value "1".
					calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[index$i$4_4], m[1]) + calculationVariable$AccumulatedProbabilities);
			}
		}
		...
		for(int i$var43=1; i$var43<samples; i$var43+=1) {
			if((0 == (indirection[(i$var43 - 1)][i$var43] / i$var43))) {
				...
				if(((0 <= j) && (j < samples)))
					...
					// 
					// Substituted "calculationVariable$valuePos" with its value "1".
					calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flipsMeasured[j], bias[1]) + calculationVariable$AccumulatedProbabilities);
			}
		}
		...
		// 
		// Initialize a counter to track the reached distributions.
		calculationVariable$var34$StateProbabilityGlobal[1] = calculationVariable$AccumulatedProbabilities;
		...
		// 
		// Get a local reference to the scratch space.
		double calculationVariable$lseMax = calculationVariable$var34$StateProbabilityGlobal[0];
		
		// Find max value.
		// 
		// Get a local reference to the scratch space.
		for(int calculationVariable$lseIndex=1; calculationVariable$lseIndex<calculationVariable$var34$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1) {
			// Get a local reference to the scratch space.
			double calculationVariable$lseElementValue = calculationVariable$var34$StateProbabilityGlobal[calculationVariable$lseIndex];
		...
		// Sum the values in the array.
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$lseIndex=0; calculationVariable$lseIndex<calculationVariable$var34$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1)
				// Get a local reference to the scratch space.
				calculationVariable$lseSum = (calculationVariable$lseSum + Math.exp((calculationVariable$var34$StateProbabilityGlobal[calculationVariable$lseIndex] - calculationVariable$lseMax)));
		...
		if((calculationVariable$logSum == Double.NEGATIVE_INFINITY)) {
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var34$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var34$StateProbabilityGlobal[calculationVariable$indexName] = (1.0 / calculationVariable$var34$StateProbabilityGlobal.length);
		} else {
			// Normalize log space values and move to normal space
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var34$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var34$StateProbabilityGlobal[calculationVariable$indexName] = Math.exp((calculationVariable$var34$StateProbabilityGlobal[calculationVariable$indexName] - calculationVariable$logSum));
		}
		...
		// 
		// Get a local reference to the scratch space.
		st[0] = DistributionSampling.sampleCategorical(RNG$, calculationVariable$var34$StateProbabilityGlobal);
	...
	private final void sample52(int i$var43) {
		{
			...
			// Constructing a random variable input for use later.
			double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(0, m[(samples - st2[(i$var43 - 1)])]);
			for(int index$i$2_2=1; index$i$2_2<samples; index$i$2_2+=1) {
				if((i$var43 == (indirection[(index$i$2_2 - 1)][index$i$2_2] / index$i$2_2))) {
					...
					if(((1 <= index$i$2_4) && (index$i$2_4 < samples)))
						...
						// 
						// Substituted "calculationVariable$valuePos" with its value "0".
						calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[index$i$2_4], m[0]) + calculationVariable$AccumulatedProbabilities);
				}
			}
			for(int index$i$5_2=1; index$i$5_2<samples; index$i$5_2+=1) {
				if((i$var43 == (indirection[(index$i$5_2 - 1)][index$i$5_2] / index$i$5_2))) {
					...
					if(((0 <= j) && (j < samples)))
						...
						// inputs.
						// 
						// calculationVariable$temp$2$var75's comment
						// Constructing a random variable input for use later.
						// 
						// Variable declaration of calculationVariable$CurrentValue moved.
						// Declaration comment was:
		...
		// calculationVariable$temp$0$var48's comment
		// Constructing a random variable input for use later.
		double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(1, m[(samples - st2[(i$var43 - 1)])]);
		for(int index$i$2_2=1; index$i$2_2<samples; index$i$2_2+=1) {
			if((i$var43 == (indirection[(index$i$2_2 - 1)][index$i$2_2] / index$i$2_2))) {
				int index$i$2_4 = ((indirection[(index$i$2_2 - 1)][index$i$2_2] / index$i$2_2) + 1);
				if(((1 <= index$i$2_4) && (index$i$2_4 < samples)))
					...
					// 
					// Substituted "calculationVariable$valuePos" with its value "1".
					calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[index$i$2_4], m[1]) + calculationVariable$AccumulatedProbabilities);
			}
		}
		for(int index$i$5_2=1; index$i$5_2<samples; index$i$5_2+=1) {
			if((i$var43 == (indirection[(index$i$5_2 - 1)][index$i$5_2] / index$i$5_2))) {
		...
		// 
		// Get a local reference to the scratch space.
		st[i$var43] = DistributionSampling.sampleCategorical(RNG$, calculationVariable$var50$StateProbabilityGlobal);
	...
	public final void allocateScratch() {
		...
		// Allocation of calculationVariable$var16$CountGlobal for multithreaded execution
		// 
		// Get the thread count.
		int calculationVariable$threadCount = threadCount();
		
		// Allocate an array to hold a copy per thread
		calculationVariable$var16$CountGlobal = new double[calculationVariable$threadCount][];
		
		// Populate the array with a copy per thread
		for(int calculationVariable$index=0; calculationVariable$index<calculationVariable$threadCount; calculationVariable$index+=1)
			// Unrolled loop
			// 
			// Calculate the longest array this random variable could produce and allocate an
			// array large enough to handle this.
			calculationVariable$var16$CountGlobal[calculationVariable$index] = new double[2];
		
		// Constructor for calculationVariable$var34$StateProbabilityGlobal
		// 
		// Allocation of calculationVariable$var34$StateProbabilityGlobal for single threaded
		// execution
		// 
		// Variable to record the maximum value of Task Get 34. Initially set to the value
		// of putTask 18.
		calculationVariable$var34$StateProbabilityGlobal = new double[2];
