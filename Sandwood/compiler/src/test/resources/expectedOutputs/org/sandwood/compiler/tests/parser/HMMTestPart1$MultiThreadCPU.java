...
class HMMTestPart1$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart1$CoreInterface {
	...
	private final void sample14(int var13, int threadID$calculationVariable$var13, Rng RNG$) {
		// A local reference to the scratch space.
		double[] calculationVariable$CountLocal = calculationVariable$var14$CountGlobal[threadID$calculationVariable$var13];
		
		// Substituted "calculationVariable$indexState$var122" with its value "0".
		calculationVariable$CountLocal[0] = 0.0;
		
		// Substituted "calculationVariable$indexState$var122" with its value "1".
		...
		if((var13 == 0))
			...
			// variable var27
			calculationVariable$CountLocal[st] = (calculationVariable$CountLocal[st] + 1.0);
		...
		Conjugates.sampleConjugateDirichletCategorical(RNG$, v, calculationVariable$CountLocal, m[var13]);
	}

	// Method to perform the inference steps to calculate new values for the samples generated
	// by sample task 23 drawn from Beta 18. Inference was performed using a Beta to Bernoulli/Binomial
	// conjugate prior.
	private final void sample23(int var22, int threadID$calculationVariable$var22, Rng RNG$) {
	...
	public final void allocateScratch() {
		...
		// Allocation of calculationVariable$var14$CountGlobal for multithreaded execution
		// 
		// Get the thread count.
		int calculationVariable$threadCount = threadCount();
		
		// Allocate an array to hold a copy per thread
		calculationVariable$var14$CountGlobal = new double[calculationVariable$threadCount][];
		...
		for(int calculationVariable$index=0; calculationVariable$index<calculationVariable$threadCount; calculationVariable$index+=1)
			...
			calculationVariable$var14$CountGlobal[calculationVariable$index] = new double[2];
