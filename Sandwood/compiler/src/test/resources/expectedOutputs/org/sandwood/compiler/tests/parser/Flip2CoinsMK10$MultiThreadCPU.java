...
class Flip2CoinsMK10$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Flip2CoinsMK10$CoreInterface {
	...
	@Override
	public final void set$fixedFlag$sample19(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample19 including if probabilities
		// need to be updated.
		fixedFlag$sample19 = calculationVariable$value;
		
		// Should the probability of sample 19 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample19" with its value "calculationVariable$value".
		fixedProbFlag$sample19 = (calculationVariable$value && fixedProbFlag$sample19);
	...
	@Override
	public final void set$fixedFlag$sample24(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample24 including if probabilities
		// need to be updated.
		fixedFlag$sample24 = calculationVariable$value;
		
		// Should the probability of sample 24 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample24" with its value "calculationVariable$value".
		fixedProbFlag$sample24 = (calculationVariable$value && fixedProbFlag$sample24);
	...
	private final void logProbabilityValue$sample19() {
		...
		if(!fixedProbFlag$sample19) {
			...
			// 
			// The sample value to calculate the probability of generating
			double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBeta(bias[0], 1.0, 1.0);
			
			// Add the probability of this sample task to the sample task accumulator.
			// 
			// Accumulator for sample probabilities for a specific instance of the random variable.
			logProbability$beta = (logProbability$beta + calculationVariable$distributionAccumulator);
			...
			// 
			// Accumulator for sample probabilities for a specific instance of the random variable.
			logProbability$bias = (logProbability$bias + calculationVariable$distributionAccumulator);
	...
	private final void logProbabilityValue$sample24() {
		...
		if(!fixedProbFlag$sample24) {
			...
			for(int i=1; i<coins; i+=1)
				...
				// 
				// The sample value to calculate the probability of generating
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBeta(bias[i], 1.0, 1.0));
			logProbability$beta = (logProbability$beta + calculationVariable$sampleAccumulator);
			
			// Store the random variable instance probability
			logProbability$var21 = calculationVariable$sampleAccumulator;
		...
		if(!fixedProbFlag$sample36) {
			...
			for(int j=0; j<coins; j+=1) {
				...
				for(int var32=0; var32<shape[j]; var32+=1)
					...
					// 
					// The sample value to calculate the probability of generating
					calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flipsMeasured[j][var32], bias[j]));
	...
	// by sample task 19 drawn from beta. Inference was performed using a Beta to Bernoulli/Binomial
	// conjugate prior.
	private final void sample19() {
		...
		// 
		// Looking for a path between Sample 19 and consumer Bernoulli 28.
		if((0 < coins)) {
		...
		bias[0] = Conjugates.sampleConjugateBetaBinomial(RNG$, 1.0, 1.0, calculationVariable$Sum, calculationVariable$Count);
	}

	// Method to perform the inference steps to calculate new values for the samples generated
	// by sample task 24 drawn from beta. Inference was performed using a Beta to Bernoulli/Binomial
	// conjugate prior.
	private final void sample24(int i, int threadID$calculationVariable$i, Rng RNG$) {
		// Local variable to record the number of true samples.
		int calculationVariable$Sum = 0;
