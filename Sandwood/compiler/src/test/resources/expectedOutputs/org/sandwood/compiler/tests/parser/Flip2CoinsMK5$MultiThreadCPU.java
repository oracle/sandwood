...
class Flip2CoinsMK5$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Flip2CoinsMK5$CoreInterface {
	...
	private final void logProbabilityValue$sample20() {
		...
		if(!fixedProbFlag$sample20) {
			...
			for(int var16=0; var16<coins; var16+=1)
				...
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBeta(bias[var16], 1.0, 1.0));
			logProbability$var12 = calculationVariable$sampleAccumulator;
			
			// Store the random variable instance probability
			logProbability$var17 = calculationVariable$sampleAccumulator;
	...
	private final void logProbabilityValue$sample33() {
		...
		if(!fixedProbFlag$sample33) {
			...
			for(int j=0; j<coins; j+=1) {
				...
				for(int var29=0; var29<shape[j]; var29+=1)
					...
					// 
					// The sample value to calculate the probability of generating
					calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flipsMeasured[j][var29], bias[j]));
				...
				// Store the random variable instance probability
				logProbability$sample33[j] = calculationVariable$sampleAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=0; j<coins; j+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample33[j];
	...
	// conjugate prior.
	private final void sample20(int var16, int threadID$calculationVariable$var16, Rng RNG$) {
		...
		for(int var29=0; var29<shape[var16]; var29+=1) {
			...
			// 
			// Substituted "j" with its value "var16".
			if(flipsMeasured[var16][var29])
				calculationVariable$Sum = (calculationVariable$Sum + 1);
		}
		
		// Write out the value of the sample to a temporary variable prior to updating the
		// intermediate variables.
		bias[var16] = Conjugates.sampleConjugateBetaBinomial(RNG$, 1.0, 1.0, calculationVariable$Sum, calculationVariable$Count);
	}
