...
class HMMTestPart3b$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements HMMTestPart3b$CoreInterface {
	...
	private double[] calculationVariable$var47$StateProbabilityGlobal;
	...
	public final void set$fixedFlag$sample26(boolean calculationVariable$value) {
		...
		// 
		// Substituted "fixedFlag$sample26" with its value "calculationVariable$value".
		fixedProbFlag$sample64 = (calculationVariable$value && fixedProbFlag$sample64);
	}
	...
	public final void set$fixedFlag$sample49(boolean calculationVariable$value) {
		...
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample49" with its value "calculationVariable$value".
		fixedProbFlag$sample49 = (calculationVariable$value && fixedProbFlag$sample49);
	...
	@Override
	public final void set$fixedFlag$sample64(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample64 including if probabilities
		// need to be updated.
		fixedFlag$sample64 = calculationVariable$value;
		
		// Should the probability of sample 64 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample64" with its value "calculationVariable$value".
		fixedProbFlag$sample64 = (calculationVariable$value && fixedProbFlag$sample64);
	}
	...
	private final void logProbabilityValue$sample49() {
		...
		if(!fixedProbFlag$sample49) {
			...
			for(int i$var39=4; i$var39<(samples + 3); i$var39+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityCategorical(st[(i$var39 - 3)], m[st[(i$var39 - 4)]]);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var46[(i$var39 - 4)] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample49[(i$var39 - 4)] = calculationVariable$distributionAccumulator;
			}
			...
			// Now the probability is calculated store if it can be cached or if it needs to be
			// recalculated next time.
			fixedProbFlag$sample49 = ((fixedFlag$sample49 && fixedFlag$sample17) && fixedFlag$sample35);
		}
		// Using cached values.
		else {
			// Updating random variable and model probabilities using cached probabilities for
			...
			for(int i$var39=4; i$var39<(samples + 3); i$var39+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample49[(i$var39 - 4)];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var46[(i$var39 - 4)] = calculationVariable$RVaccumulator;
			}
	...
	private final void logProbabilityValue$sample64() {
		...
		if(!fixedProbFlag$sample64) {
			...
			for(int j=5; j<(samples + 5); j+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBernoulli(flipsMeasured[(j - 5)], bias[st[(j - 5)]]);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var61[(j - 5)] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample64[(j - 5)] = calculationVariable$distributionAccumulator;
			}
			...
			// Now the probability is calculated store if it can be cached or if it needs to be
			// recalculated next time.
			fixedProbFlag$sample64 = (((fixedFlag$sample64 && fixedFlag$sample26) && fixedFlag$sample35) && fixedFlag$sample49);
		}
		// Using cached values.
		else {
			// Updating random variable and model probabilities using cached probabilities for
			...
			for(int j=5; j<(samples + 5); j+=1) {
				// Variable declaration of calculationVariable$RVaccumulator moved.
				double calculationVariable$RVaccumulator = logProbability$sample64[(j - 5)];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var61[(j - 5)] = calculationVariable$RVaccumulator;
			}
	...
	private final void sample17(int var15) {
		...
		// 
		// Looking for a path between Sample 17 and consumer Categorical 46.
		for(int i$var39=4; i$var39<(samples + 3); i$var39+=1) {
	...
	private final void sample26(int var24) {
		...
		// 
		// Looking for a path between Sample 26 and consumer Bernoulli 61.
		for(int j=5; j<(samples + 5); j+=1) {
			if((var24 == st[(j - 5)])) {
				// Processing sample task 64 of consumer random variable null.
				// 
				// Include the value sampled by task 64 from random variable var61.
	...
	private final void sample35() {
		{
			...
			if((1 < samples))
				...
				// Substituted "i$var39" with its value "4".
				// 
				// Substituted "calculationVariable$temp$1$var45" with its value "var45".
				// 
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
			...
			if((0 < samples))
				...
				// Substituted "j" with its value "5".
				// 
				// Substituted "calculationVariable$temp$2$var60" with its value "var60".
				// 
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of calculationVariable$CurrentValue moved.
	...
	// Method to perform the inference steps to calculate new values for the samples generated
	// by sample task 49 drawn from Categorical 46. Inference was performed using variable
	// marginalization.
	private final void sample49(int i$var39) {
		{
			// Variable declaration of calculationVariable$CurrentValue moved.
			// Declaration comment was:
			// The value currently being tested
			...
			// it is added to the index probabilities.
			// 
			// calculationVariable$temp$0$var45's comment
			// Constructing a random variable input for use later.
			double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(0, m[st[(i$var39 - 4)]]);
			...
			// 
			// Initialize a counter to track the reached distributions.
			calculationVariable$var47$StateProbabilityGlobal[0] = calculationVariable$AccumulatedProbabilities;
		}
		...
		// calculationVariable$temp$0$var45's comment
		// Constructing a random variable input for use later.
		double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(1, m[st[(i$var39 - 4)]]);
		int index$i$1_2 = (i$var39 + 1);
		if((index$i$1_2 < (samples + 3)))
			// Processing sample task 49 of consumer random variable null.
			// 
		...
		// 
		// Initialize a counter to track the reached distributions.
		calculationVariable$var47$StateProbabilityGlobal[1] = calculationVariable$AccumulatedProbabilities;
		...
		// 
		// Get a local reference to the scratch space.
		double calculationVariable$lseMax = calculationVariable$var47$StateProbabilityGlobal[0];
		
		// Find max value.
		// 
		// Get a local reference to the scratch space.
		for(int calculationVariable$lseIndex=1; calculationVariable$lseIndex<calculationVariable$var47$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1) {
			// Get a local reference to the scratch space.
			double calculationVariable$lseElementValue = calculationVariable$var47$StateProbabilityGlobal[calculationVariable$lseIndex];
			if((calculationVariable$lseMax < calculationVariable$lseElementValue))
				calculationVariable$lseMax = calculationVariable$lseElementValue;
		}
		...
		// Sum the values in the array.
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$lseIndex=0; calculationVariable$lseIndex<calculationVariable$var47$StateProbabilityGlobal.length; calculationVariable$lseIndex+=1)
				// Get a local reference to the scratch space.
				calculationVariable$lseSum = (calculationVariable$lseSum + Math.exp((calculationVariable$var47$StateProbabilityGlobal[calculationVariable$lseIndex] - calculationVariable$lseMax)));
		...
		if((calculationVariable$logSum == Double.NEGATIVE_INFINITY)) {
			...
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var47$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var47$StateProbabilityGlobal[calculationVariable$indexName] = (1.0 / calculationVariable$var47$StateProbabilityGlobal.length);
		} else {
			// Normalize log space values and move to normal space
			// 
			// Get a local reference to the scratch space.
			for(int calculationVariable$indexName=0; calculationVariable$indexName<calculationVariable$var47$StateProbabilityGlobal.length; calculationVariable$indexName+=1)
				// Get a local reference to the scratch space.
				calculationVariable$var47$StateProbabilityGlobal[calculationVariable$indexName] = Math.exp((calculationVariable$var47$StateProbabilityGlobal[calculationVariable$indexName] - calculationVariable$logSum));
		}
		...
		// 
		// Get a local reference to the scratch space.
		st[(i$var39 - 3)] = DistributionSampling.sampleCategorical(RNG$, calculationVariable$var47$StateProbabilityGlobal);
	}
	...
	public final void allocateScratch() {
		...
		// Allocation of calculationVariable$var47$StateProbabilityGlobal for single threaded
		// execution
		// 
		// Variable to record the maximum value of Task Get 47. Initially set to the value
		// of putTask 18.
		calculationVariable$var47$StateProbabilityGlobal = new double[2];
	}
	...
	public final void allocator() {
		...
		logProbability$var46 = new double[(length$flipsMeasured - 1)];
		
		// Constructor for logProbability$sample49
		logProbability$sample49 = new double[(length$flipsMeasured - 1)];
		
		// Constructor for logProbability$var61
		logProbability$var61 = new double[length$flipsMeasured];
		
		// Constructor for logProbability$sample64
		logProbability$sample64 = new double[length$flipsMeasured];
	...
	private final void initializeLogProbabilityFields() {
		...
		for(int i$var39=4; i$var39<(samples + 3); i$var39+=1)
			logProbability$var46[(i$var39 - 4)] = 0.0;
		if(!fixedProbFlag$sample49) {
			for(int i$var39=4; i$var39<(samples + 3); i$var39+=1)
				logProbability$sample49[(i$var39 - 4)] = 0.0;
		}
		for(int j=5; j<(samples + 5); j+=1)
			logProbability$var61[(j - 5)] = 0.0;
		logProbability$flips = 0.0;
		if(!fixedProbFlag$sample64) {
			for(int j=5; j<(samples + 5); j+=1)
				logProbability$sample64[(j - 5)] = 0.0;
		}
	}
