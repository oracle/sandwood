...
class Flip2CoinsMK4$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Flip2CoinsMK4$CoreInterface {
	...
	public final void set$fixedFlag$sample16(boolean calculationVariable$value) {
		...
		// 
		// Substituted "fixedFlag$sample16" with its value "calculationVariable$value".
		fixedProbFlag$sample32 = (calculationVariable$value && fixedProbFlag$sample32);
	}
	...
	@Override
	public final void set$fixedFlag$sample32(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample32 including if probabilities
		// need to be updated.
		fixedFlag$sample32 = calculationVariable$value;
		
		// Should the probability of sample 32 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample32" with its value "calculationVariable$value".
		fixedProbFlag$sample32 = (calculationVariable$value && fixedProbFlag$sample32);
	}
	...
	private final void logProbabilityValue$sample16() {
		...
		if(!fixedProbFlag$sample16) {
			...
			for(int i=0; i<coins; i+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBeta(bias[i], a, b);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var13[i] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample16[i] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int i=0; i<coins; i+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample16[i];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var13[i] = calculationVariable$RVaccumulator;
			}
	...
	private final void logProbabilityValue$sample32() {
		...
		if(!fixedProbFlag$sample32) {
			...
			for(int j=0; j<coins; j+=1) {
				...
				for(int var28=0; var28<length$flipsMeasured[j]; var28+=1)
					...
					// 
					// The sample value to calculate the probability of generating
					calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flipsMeasured[j][var28], bias[j]));
				...
				logProbability$sample32[j] = calculationVariable$sampleAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=0; j<coins; j+=1) {
				// Variable declaration of calculationVariable$RVaccumulator moved.
				double calculationVariable$RVaccumulator = logProbability$sample32[j];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
	...
	// conjugate prior.
	private final void sample16(int i, int threadID$calculationVariable$i, Rng RNG$) {
		...
		for(int var28=0; var28<length$flipsMeasured[i]; var28+=1) {
			...
			// 
			// Substituted "j" with its value "i".
			if(flipsMeasured[i][var28])
				calculationVariable$Sum = (calculationVariable$Sum + 1);
		}
		
		// Write out the value of the sample to a temporary variable prior to updating the
		// intermediate variables.
		bias[i] = Conjugates.sampleConjugateBetaBinomial(RNG$, a, b, calculationVariable$Sum, calculationVariable$Count);
	}
