...
class HMMTestPart2b$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart2b$CoreInterface {
	...
	private final void logProbabilityValue$sample45() {
		...
		if(!fixedProbFlag$sample45) {
			...
			for(int i$var37=0; i$var37<(samples - 1); i$var37+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityCategorical(st[(i$var37 + 1)], m[st[i$var37]]);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var42[i$var37] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample45[i$var37] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int i$var37=0; i$var37<(samples - 1); i$var37+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample45[i$var37];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var42[i$var37] = calculationVariable$RVaccumulator;
			}
	...
	private final void logProbabilityValue$sample53() {
		...
		if(!fixedProbFlag$sample53) {
			...
			for(int j=1; j<samples; j+=1) {
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var50[(j - 1)] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample53[(j - 1)] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=1; j<samples; j+=1) {
				// Variable declaration of calculationVariable$RVaccumulator moved.
				double calculationVariable$RVaccumulator = logProbability$sample53[(j - 1)];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$var50[(j - 1)] = calculationVariable$RVaccumulator;
			}
	...
	private final void sample17(int var15, int threadID$calculationVariable$var15, Rng RNG$) {
		...
		// Substituted "calculationVariable$indexState$var231" with its value "0".
		calculationVariable$CountLocal[0] = 0.0;
		
		// Substituted "calculationVariable$indexState$var231" with its value "1".
		...
		for(int i$var37=0; i$var37<(samples - 1); i$var37+=1) {
			if((var15 == st[i$var37]))
				...
				// variable var42
				calculationVariable$CountLocal[st[(i$var37 + 1)]] = (calculationVariable$CountLocal[st[(i$var37 + 1)]] + 1.0);
		}
	...
	private final void sample45(int i$var37) {
		{
			...
			double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(0, m[st[i$var37]]);
			...
			if((index$i$1_2 < (samples - 1)))
				...
				// 
				// Substituted "calculationVariable$valuePos" with its value "0".
				calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[(index$i$1_2 + 1)], m[0]) + calculationVariable$AccumulatedProbabilities);
			...
			// inputs.
			// 
			// Substituted "calculationVariable$temp$2$var49" with its value "var49".
			// 
			// Constructing a random variable input for use later.
			// 
			// Variable declaration of calculationVariable$CurrentValue moved.
			...
			// 
			// Substituted "calculationVariable$valuePos" with its value "0".
			calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flipsMeasured[(i$var37 + 1)], bias[0]) + calculationVariable$AccumulatedProbabilities);
		...
		// calculationVariable$temp$0$var41's comment
		// Constructing a random variable input for use later.
		double calculationVariable$AccumulatedProbabilities = DistributionSampling.logProbabilityCategorical(1, m[st[i$var37]]);
		int index$i$1_2 = (i$var37 + 1);
		if((index$i$1_2 < (samples - 1)))
			// Processing sample task 45 of consumer random variable null.
			// 
			...
			// 
			// Substituted "calculationVariable$valuePos" with its value "1".
			calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[(index$i$1_2 + 1)], m[1]) + calculationVariable$AccumulatedProbabilities);
		...
		// 
		// Substituted "calculationVariable$valuePos" with its value "1".
		calculationVariable$AccumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flipsMeasured[(i$var37 + 1)], bias[1]) + calculationVariable$AccumulatedProbabilities);
		...
		// 
		// Get a local reference to the scratch space.
		st[(i$var37 + 1)] = DistributionSampling.sampleCategorical(RNG$, calculationVariable$var43$StateProbabilityGlobal);
	}
