...
class Flip1CoinMK5$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements Flip1CoinMK5$CoreInterface {
	...
	public final void set$fixedFlag$sample17(boolean calculationVariable$value) {
		...
		// 
		// Substituted "fixedFlag$sample17" with its value "calculationVariable$value".
		fixedProbFlag$sample30 = (calculationVariable$value && fixedProbFlag$sample30);
	}
	...
	@Override
	public final void set$fixedFlag$sample30(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample30 including if probabilities
		// need to be updated.
		fixedFlag$sample30 = calculationVariable$value;
		
		// Should the probability of sample 30 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample30" with its value "calculationVariable$value".
		fixedProbFlag$sample30 = (calculationVariable$value && fixedProbFlag$sample30);
	}
	...
	// Setter for flips1.
	@Override
	public final void set$flips1(boolean[] calculationVariable$value) {
		// Set flips1 with flag to mark that it has been set so another array doesn't need
		// to be constructed
		flips1 = calculationVariable$value;
		setFlag$flips1 = true;
	}
	...
	// Setter for flips2.
	@Override
	public final void set$flips2(boolean[] calculationVariable$value) {
		// Set flips2 with flag to mark that it has been set so another array doesn't need
		// to be constructed
		flips2 = calculationVariable$value;
		setFlag$flips2 = true;
	}
	...
	@Override
	public final void set$flipsMeasured1(boolean[] calculationVariable$value) {
		// Set flipsMeasured1 with flag to mark that it has been set so another array doesn't
		// need to be constructed
		flipsMeasured1 = calculationVariable$value;
	}
	...
	@Override
	public final void set$flipsMeasured2(boolean[] calculationVariable$value) {
		// Set flipsMeasured2 with flag to mark that it has been set so another array doesn't
		// need to be constructed
		flipsMeasured2 = calculationVariable$value;
	}
	...
	@Override
	public final void set$length$flipsMeasured1(int calculationVariable$value) {
		length$flipsMeasured1 = calculationVariable$value;
	}
	...
	@Override
	public final void set$length$flipsMeasured2(int calculationVariable$value) {
		length$flipsMeasured2 = calculationVariable$value;
	}
	...
	private final void logProbabilityValue$sample23() {
		...
		if(!fixedProbFlag$sample23) {
			...
			for(int var18=0; var18<samples1; var18+=1)
				...
				// The sample value to calculate the probability of generating
				// 
				// Substituted "i$var33" with its value "var18".
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flipsMeasured1[var18], bias));
			logProbability$bernoulli1 = calculationVariable$sampleAccumulator;
			
			// Store the random variable instance probability
			logProbability$var19 = calculationVariable$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$flips1 = (logProbability$flips1 + calculationVariable$sampleAccumulator);
	...
	private final void logProbabilityValue$sample30() {
		...
		if(!fixedProbFlag$sample30) {
			...
			for(int var25=0; var25<samples2; var25+=1) {
				...
				// Looking for a path between Sample 30 and consumer boolean 45.
				for(int i$var42=(samples2 * 2); i$var42>=0; i$var42-=2) {
					if((var25 == (i$var42 / 2))) {
						// Store the value of the function call, so the function call is only made once.
						// 
						// The sample value to calculate the probability of generating
						double calculationVariable$weightedProbability = DistributionSampling.logProbabilityBernoulli(flipsMeasured2[(i$var42 / 2)], bias);
				...
				// Add the probability of this sample task to the sample task accumulator.
				calculationVariable$sampleAccumulator = (calculationVariable$sampleAccumulator + calculationVariable$distributionAccumulator);
			}
			logProbability$bernoulli2 = calculationVariable$sampleAccumulator;
			
			// Store the random variable instance probability
			logProbability$var26 = calculationVariable$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$flips2 = (logProbability$flips2 + calculationVariable$sampleAccumulator);
	...
	private final void sample17() {
		...
		for(int var18=0; var18<samples1; var18+=1) {
			...
			// 
			// Looking for a path between Sample 23 and consumer boolean 34.
