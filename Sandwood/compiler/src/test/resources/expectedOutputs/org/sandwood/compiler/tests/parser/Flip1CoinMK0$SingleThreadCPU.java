...
class Flip1CoinMK0$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements Flip1CoinMK0$CoreInterface {
	...
	@Override
	public final void set$fixedFlag$sample5(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample5 including if probabilities
		// need to be updated.
		fixedFlag$sample5 = calculationVariable$value;
		
		// Should the probability of sample 5 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample5" with its value "calculationVariable$value".
		fixedProbFlag$sample5 = (calculationVariable$value && fixedProbFlag$sample5);
		
		// Should the probability of sample 7 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample5" with its value "calculationVariable$value".
		fixedProbFlag$sample7 = (calculationVariable$value && fixedProbFlag$sample7);
	}
	...
	@Override
	public final void set$fixedFlag$sample7(boolean calculationVariable$value) {
		// Set flags for all the side effects of fixedFlag$sample7 including if probabilities
		// need to be updated.
		fixedFlag$sample7 = calculationVariable$value;
		
		// Should the probability of sample 7 be set to fixed. This will only every change
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample7" with its value "calculationVariable$value".
		fixedProbFlag$sample7 = (calculationVariable$value && fixedProbFlag$sample7);
	}
	...
	private final void logProbabilityValue$sample5() {
		...
		if(!fixedProbFlag$sample5) {
			...
			logProbability$var4 = calculationVariable$distributionAccumulator;
	...
	private final void logProbabilityValue$sample7() {
		...
		if(!fixedProbFlag$sample7) {
			...
			// 
			// The sample value to calculate the probability of generating
			double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBernoulli(flipMeasured, bias);
			
			// Add the probability of this sample task to the sample task accumulator.
			// 
			// Accumulator for sample probabilities for a specific instance of the random variable.
			logProbability$bernoulli = calculationVariable$distributionAccumulator;
	...
	private final void sample5() {
		...
		bias = Conjugates.sampleConjugateBetaBinomial(RNG$, 1.0, 1.0, calculationVariable$Sum, 1);
	}
