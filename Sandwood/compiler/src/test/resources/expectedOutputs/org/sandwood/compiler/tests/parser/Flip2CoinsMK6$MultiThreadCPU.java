...
class Flip2CoinsMK6$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Flip2CoinsMK6$CoreInterface {
	...
	private final void logProbabilityValue$sample20() {
		...
		if(!fixedProbFlag$sample20) {
			...
			for(int j=0; j<coins; j+=1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double calculationVariable$distributionAccumulator = DistributionSampling.logProbabilityBeta(bias[j], 1.0, 1.0);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$beta[j] = calculationVariable$distributionAccumulator;
				
				// Store the sample task probability
				logProbability$sample20[j] = calculationVariable$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j=0; j<coins; j+=1) {
				...
				double calculationVariable$RVaccumulator = logProbability$sample20[j];
				calculationVariable$accumulator = (calculationVariable$accumulator + calculationVariable$RVaccumulator);
				logProbability$beta[j] = calculationVariable$RVaccumulator;
			}
	...
	// conjugate prior.
	private final void sample20(int j, int threadID$calculationVariable$j, Rng RNG$) {
