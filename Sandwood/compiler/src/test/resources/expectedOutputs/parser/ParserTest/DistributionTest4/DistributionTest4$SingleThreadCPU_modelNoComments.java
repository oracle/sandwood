		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
				...
				cv$currentValue = cv$valuePos;
				{
					...
					int $var461 = weightings.length;
					double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < $var461))?Math.log(weightings[cv$currentValue]):Double.NEGATIVE_INFINITY));
					{
						{
	...
	private final void sample27(int i) {
		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
				...
				{
					cv$reachedDistributionSourceRV = (cv$reachedDistributionSourceRV + 1.0);
					int $var1116 = weightings.length;
					double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < $var1116))?Math.log(weightings[cv$currentValue]):Double.NEGATIVE_INFINITY));
					{
						{
	...
	private final void sample5() {
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
				...
				{
					cv$reachedDistributionSourceRV = (cv$reachedDistributionSourceRV + 1.0);
					int $var131 = weightings.length;
					double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < $var131))?Math.log(weightings[cv$currentValue]):Double.NEGATIVE_INFINITY));
					{
						{
							{
								for(int j = 0; j < size; j += 1) {
