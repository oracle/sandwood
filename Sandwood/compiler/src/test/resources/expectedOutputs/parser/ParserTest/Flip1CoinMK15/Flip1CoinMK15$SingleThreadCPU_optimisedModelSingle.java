	...
	private final void inferSample8() {
		...
		double cv$proposedValue = ((Math.sqrt(cv$var) * DistributionSampling.sampleGaussian(state.RNG$)) + state.b);
		{
			...
			// Constraints moved from conditionals in inner loops/scopes/etc.
				...
				// 
				// Substituted "cv$reduction171Index" with its value "1".
				double reduceVar$var33$2 = ((state.b / 2) + state.c[1]);
				...
				for(int var46 = 0; var46 < state.samples; var46 += 1) {
					...
					// Set an accumulator to sum the probabilities for each possible configuration of
					// inputs.
					cv$accumulatedProbabilities = ((((0.0 <= reduceVar$var33$2) && (reduceVar$var33$2 <= 1.0))?Math.log((state.flips[var46]?reduceVar$var33$2:(1.0 - reduceVar$var33$2))):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
				}
			}
		...
		if(state.constrainedFlag$sample8) {
			...
			// Constraints moved from conditionals in inner loops/scopes/etc.
				...
				// 
				// Substituted "cv$reduction171Index" with its value "1".
				double reduceVar$var33$2 = ((cv$proposedValue / 2) + state.c[1]);
