...
import org.sandwood.compiler.tests.parser.Flip2CoinsMK3$SingleThreadCPU.Scratch;
...
final class Flip2CoinsMK3$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	public Flip2CoinsMK3$SingleThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
	...
	private final void drawValueSample17(int var16) {
		state.bias[var16] = DistributionSampling.sampleBeta(state.RNG$, state.a, state.b);
	}
	...
	public final void gibbsRound() {
		...
		// Infer the samples in reverse chronological order.
			...
			for(int var16 = (state.coins - ((((state.coins - 1) - 0) % 1) + 1)); var16 >= ((0 - 1) + 1); var16 -= 1) {
				if(!state.fixedFlag$sample17)
					inferSample17(var16);
			}
		}
