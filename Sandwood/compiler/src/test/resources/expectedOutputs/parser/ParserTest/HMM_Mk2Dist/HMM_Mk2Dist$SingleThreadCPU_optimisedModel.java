...
final class HMM_Mk2Dist$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	final class Scratch implements CoreModelScratch {
		...
		public final void allocateScratch() {
			...
			// of putTask 43.
			cv$distributionAccumulator$var122 = new double[state.noStates];
	...
	private final void inferSample126(int i$var104, int j$var115) {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			if((index$j$57_3 < state.length$eventsMeasured[i$var104])) {
				...
				for(int cv$i = 0; cv$i < state.noStates; cv$i += 1)
					...
					scratch.cv$distributionAccumulator$var122[cv$i] = 0.0;
				...
				if((cv$valuePos < state.noStates)) {
					...
					// 
					// Value of the variable at this index
					DistributionSampling.addProbabilityDistributionCategorical(scratch.cv$distributionAccumulator$var122, scopeVariable$reachedSourceProbability, state.m[cv$valuePos], state.noStates);
				}
				...
				for(int cv$i = 0; cv$i < state.noStates; cv$i += 1) {
					...
					// A local array to hold the accumulated distributions of the sample tasks for each
					// configuration of distributions.
					double cv$normalisedDistValue = (scratch.cv$distributionAccumulator$var122[cv$i] / cv$reachedDistributionProbability);
			...
			scratch.cv$var123$stateProbabilityGlobal[cv$valuePos] = ((cv$stateProbabilityValue + cv$accumulatedDistributionProbabilities) - Math.log(cv$reachedDistributionSourceRV));
		}
		if(state.constrainedFlag$sample126[i$var104][(j$var115 - 1)]) {
			...
			if((cv$logSum == Double.NEGATIVE_INFINITY)) {
				...
				for(int cv$indexName = 0; cv$indexName < cv$numStates; cv$indexName += 1)
					// Get a local reference to the scratch space.
					cv$localProbability[cv$indexName] = Math.exp((scratch.cv$var123$stateProbabilityGlobal[cv$indexName] - cv$logSum));
			}
				...
				if((1 < state.length$eventsMeasured[i$var104])) {
					if(state.fixedFlag$sample95) {
						...
						if((var41 == state.st[i$var104][0])) {
							...
							// 
																					// A local reference to the scratch space.
							scratch.cv$var42$countGlobal[state.st[i$var104][1]] = (scratch.cv$var42$countGlobal[state.st[i$var104][1]] + 1.0);
						}
					} else {
						// Mark that the sample has observed constrained data.
						state.constrainedFlag$sample42[var41] = true;
						...
						scratch.cv$var42$countGlobal[state.st[i$var104][1]] = (scratch.cv$var42$countGlobal[state.st[i$var104][1]] + state.distribution$sample95[i$var104][var41]);
					}
				}
			}
			for(int i$var104 = 0; i$var104 < state.samples; i$var104 += 1) {
		...
		if(((var41 == state.initialState) && !state.fixedFlag$sample95)) {
			...
			for(int i$var87 = 0; i$var87 < state.samples; i$var87 += 1) {
				...
				for(int cv$loopIndex = 0; cv$loopIndex < state.noStates; cv$loopIndex += 1)
					...
					// 
					// Declare and zero an accumulator for tracking the reached source probability space.
					scratch.cv$var42$countGlobal[cv$loopIndex] = (scratch.cv$var42$countGlobal[cv$loopIndex] + state.distribution$sample95[i$var87][cv$loopIndex]);
			}
		}
					...
					if(state.fixedFlag$sample95) {
						...
						if((var41 == state.st[i$var104][0])) {
							...
							for(int cv$loopIndex = 0; cv$loopIndex < state.noStates; cv$loopIndex += 1)
								...
								// 
								// Declare and zero an accumulator for tracking the reached source probability space.
								scratch.cv$var42$countGlobal[cv$loopIndex] = (scratch.cv$var42$countGlobal[cv$loopIndex] + state.distribution$sample126[i$var104][0][cv$loopIndex]);
						}
					} else {
						// The probability of reaching the consumer with this set of consumer arguments
						// 
						...
						for(int cv$loopIndex = 0; cv$loopIndex < state.noStates; cv$loopIndex += 1)
																					// A local reference to the scratch space.
							scratch.cv$var42$countGlobal[cv$loopIndex] = (scratch.cv$var42$countGlobal[cv$loopIndex] + (state.distribution$sample126[i$var104][0][cv$loopIndex] * cv$distributionProbability));
					}
				}
			}
			for(int i$var104 = 0; i$var104 < state.samples; i$var104 += 1) {
				for(int j$var115 = 1; j$var115 < state.length$eventsMeasured[i$var104]; j$var115 += 1) {
					...
					if((1 <= index$j$59)) {
						...
						for(int cv$loopIndex = 0; cv$loopIndex < state.noStates; cv$loopIndex += 1)
																					// A local reference to the scratch space.
							scratch.cv$var42$countGlobal[cv$loopIndex] = (scratch.cv$var42$countGlobal[cv$loopIndex] + (state.distribution$sample126[i$var104][(j$var115 - 1)][cv$loopIndex] * cv$distributionProbability));
					}
				}
			}
		}
		...
		for(int i$var136 = 0; i$var136 < state.samples; i$var136 += 1) {
			for(int j$var149 = 1; j$var149 < state.length$eventsMeasured[i$var136]; j$var149 += 1) {
				if(state.fixedFlag$sample126) {
					...
					scratch.cv$var56$countGlobal[(state.events[i$var136][j$var149] - 1)] = (scratch.cv$var56$countGlobal[(state.events[i$var136][j$var149] - 1)] + state.distribution$sample126[i$var136][(j$var149 - 1)][var55]);
				}
			}
		}
	...
	private final void inferSample95(int i$var87) {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			if((1 < state.length$eventsMeasured[i$var87])) {
				...
				// Constraints moved from conditionals in inner loops/scopes/etc.
					...
					if((cv$valuePos < state.noStates)) {
						...
						// 
																		// Value of the variable at this index
						DistributionSampling.addProbabilityDistributionCategorical(scratch.cv$distributionAccumulator$var122, 1.0, state.m[cv$valuePos], state.noStates);
					}
			...
			// 
			// Initialize a counter to track the reached distributions.
			scratch.cv$var92$stateProbabilityGlobal[cv$valuePos] = (cv$accumulatedProbabilities + cv$accumulatedDistributionProbabilities);
		}
		if(state.constrainedFlag$sample95[i$var87]) {
			...
			if((cv$logSum == Double.NEGATIVE_INFINITY)) {
				...
				for(int cv$indexName = 0; cv$indexName < cv$numStates; cv$indexName += 1)
					// Get a local reference to the scratch space.
					cv$localProbability[cv$indexName] = Math.exp((scratch.cv$var92$stateProbabilityGlobal[cv$indexName] - cv$logSum));
			}
