...
final class HMM_Mk2Dist$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
		...
		if(!state.fixedProbFlag$sample126) {
			...
			if(state.fixedFlag$sample126) {
				...
				if(cv$sampleReached)
					...
					state.logProbability$var123 = cv$sampleAccumulator;
			...
			if(state.fixedFlag$sample126)
				...
				state.logProbability$st = (state.logProbability$st + state.logProbability$var123);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var123);
			
			// If this value is fixed, add it to the probability of this model producing the fixed
			// values
			if(state.fixedFlag$sample126)
				// Variable declaration of cv$accumulator moved.
				state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var123);
		}
	}
	...
	private final void logProbabilityDistribution$sample159() {
		...
		if(!state.fixedProbFlag$sample159) {
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var155 = cv$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			state.logProbability$events = (state.logProbability$events + cv$sampleAccumulator);
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$events = (state.logProbability$events + state.logProbability$var155);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var155);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var155);
		}
	}
		...
		if(!state.fixedProbFlag$sample95) {
			...
			if(state.fixedFlag$sample95) {
				...
				for(int i$var87 = 0; i$var87 < state.samples; i$var87 += 1) {
					...
					// 
					// Store the value of the function call, so the function call is only made once.
					cv$sampleAccumulator = (cv$sampleAccumulator + ((((((0.0 <= cv$sampleValue) && (cv$sampleValue < state.noStates)) && (0 < state.noStates)) && (0.0 <= var90[cv$sampleValue])) && (var90[cv$sampleValue] <= 1.0))?Math.log(var90[cv$sampleValue]):Double.NEGATIVE_INFINITY));
				}
				...
				if(cv$sampleReached)
					...
					// 
					// Accumulator for probabilities of instances of the random variable
					state.logProbability$var92 = cv$sampleAccumulator;
			...
			if(state.fixedFlag$sample95)
				...
				// 
				// Variable declaration of cv$accumulator moved.
				state.logProbability$st = (state.logProbability$st + state.logProbability$var92);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var92);
			
			// If this value is fixed, add it to the probability of this model producing the fixed
			// values
			if(state.fixedFlag$sample95)
				// Variable declaration of cv$accumulator moved.
				state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var92);
		}
	}
		...
		if(!state.fixedProbFlag$sample126) {
			...
			for(int i$var104 = 0; i$var104 < state.samples; i$var104 += 1) {
				for(int j$var115 = 1; j$var115 < state.length$eventsMeasured[i$var104]; j$var115 += 1) {
					...
					// 
					// Store the value of the function call, so the function call is only made once.
					cv$sampleAccumulator = (cv$sampleAccumulator + ((((((0.0 <= cv$sampleValue) && (cv$sampleValue < state.noStates)) && (0 < state.noStates)) && (0.0 <= var121[cv$sampleValue])) && (var121[cv$sampleValue] <= 1.0))?Math.log(var121[cv$sampleValue]):Double.NEGATIVE_INFINITY));
				}
			}
		...
		if(!state.fixedProbFlag$sample159) {
			...
			for(int i$var136 = 0; i$var136 < state.samples; i$var136 += 1) {
				for(int j$var149 = 1; j$var149 < state.length$eventsMeasured[i$var136]; j$var149 += 1) {
					...
					// 
					// Store the value of the function call, so the function call is only made once.
					cv$sampleAccumulator = (cv$sampleAccumulator + ((((((0.0 <= cv$sampleValue) && (cv$sampleValue < state.noEvents)) && (0 < state.noEvents)) && (0.0 <= var153[cv$sampleValue])) && (var153[cv$sampleValue] <= 1.0))?Math.log(var153[cv$sampleValue]):Double.NEGATIVE_INFINITY));
				}
			}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample95)
			state.logProbability$var92 = Double.NaN;
		if(!state.fixedProbFlag$sample126)
			state.logProbability$var123 = Double.NaN;
		state.logProbability$events = 0.0;
		if(!state.fixedProbFlag$sample159)
			state.logProbability$var155 = Double.NaN;
	}
