...
final class GaussianMixtureTest$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements GaussianMixtureTest$CoreInterface {
	...
	private final void logProbabilityValue$sample68() {
		...
		for(int i$var66 = 0; i$var66 < length$xMeasured; i$var66 += 1) {
			...
			// 
			// Store the value of the function call, so the function call is only made once.
			cv$sampleAccumulator = (cv$sampleAccumulator + (((((0.0 <= cv$sampleValue) && (cv$sampleValue < 5)) && (0.0 <= phi[cv$sampleValue])) && (phi[cv$sampleValue] <= 1.0))?Math.log(phi[cv$sampleValue]):Double.NEGATIVE_INFINITY));
		}
	...
	private final void logProbabilityValue$sample72() {
		...
		for(int i$var66 = 0; i$var66 < length$xMeasured; i$var66 += 1) {
			...
			// 
			// The sample value to calculate the probability of generating
			cv$sampleAccumulator = (cv$sampleAccumulator + ((0.0 < var70)?(DistributionSampling.logProbabilityGaussian(((x[i$var66] - mu[z[i$var66]]) / Math.sqrt(var70))) - (Math.log(var70) * 0.5)):Double.NEGATIVE_INFINITY));
		}
