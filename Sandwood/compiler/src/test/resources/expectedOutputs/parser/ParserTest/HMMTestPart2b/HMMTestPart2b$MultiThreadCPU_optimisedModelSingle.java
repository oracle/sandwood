	...
	private final void logProbabilityValue$sample86() {
		...
		if(!state.fixedProbFlag$sample86) {
			...
			for(int j = 1; j < state.samples; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var83) && (var83 <= 1.0))?Math.log((state.flips[j]?var83:(1.0 - var83))):Double.NEGATIVE_INFINITY));
			}
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var85);
