...
import org.sandwood.compiler.tests.parser.PoissonDecayMK1$SingleThreadCPU.Scratch;
...
final class PoissonDecayMK1$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	public PoissonDecayMK1$SingleThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
	...
	private final void logProbabilityValue$sample19() {
		...
		if(!state.fixedProbFlag$sample19) {
			...
			for(int var18 = 0; var18 < state.samples; var18 += 1)
				...
				cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityPoisson(state.decay[var18], state.rate));
			...
			// this sample
			state.logProbability$poisson = state.logProbability$var19;
	...
	public final void forwardGeneration() {
		if(!state.fixedFlag$sample6)
			state.rate = DistributionSampling.sampleGamma(state.RNG$, state.a, state.b);
		for(int var18 = 0; var18 < state.samples; var18 += 1)
			state.decay[var18] = DistributionSampling.samplePoisson(state.RNG$, state.rate);
	}
