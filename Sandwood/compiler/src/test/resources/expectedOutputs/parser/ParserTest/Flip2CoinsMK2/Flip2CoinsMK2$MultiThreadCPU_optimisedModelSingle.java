...
final class Flip2CoinsMK2$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	// by sample task 20 drawn from Beta 8. Inference was performed using a Beta to Bernoulli/Binomial
	// conjugate prior.
	private final void inferSample20(int var19, int threadID$cv$var19, Rng RNG$) {
		...
		for(int i = 0; i < state.samples; i += 1) {
			// Processing sample task 45 of consumer random variable bernoulli.
			...
			// 
			// Substituted "j" with its value "var19".
		...
		if(!state.fixedProbFlag$sample45) {
			...
			for(int i = 0; i < state.samples; i += 1) {
				for(int j = 0; j < state.coins; j += 1) {
					...
					// 
					// The sample value to calculate the probability of generating
					cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var43) && (var43 <= 1.0))?Math.log((state.flips[i][j]?var43:(1.0 - var43))):Double.NEGATIVE_INFINITY));
				}
			}
			...
			if(cv$sampleReached) {
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var45 = cv$sampleAccumulator;
			}
			...
			// A guard to check if the sample value is ever reached.
			boolean cv$sampleReached = false;
			if(((0 < state.samples) && (0 < state.coins)))
				// Record that the sample was reached.
				cv$sampleReached = true;
			if(cv$sampleReached)
				state.logProbability$bernoulli = state.logProbability$var45;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var45);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var45);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var45);
		}
	}
