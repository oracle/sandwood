...
final class HMMTestPart4$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements HMMTestPart4$CoreInterface {
			...
			for(int i1 = 1; i1 < samples; i1 += 1) {
				for(int j1 = 0; j1 < samples; j1 += 1) {
					for(int k1 = 0; k1 < samples; k1 += 1) {
							...
							{
								{
									...
									double cv$weightedProbability = (Math.log(1.0) + (((0.0 <= cv$sampleValue) && (cv$sampleValue < states))?Math.log(var119[cv$sampleValue]):Double.NEGATIVE_INFINITY));
						...
						logProbability$var120[((i1 - 1) / 1)][((j1 - 0) / 1)][((k1 - 0) / 1)] = cv$sampleAccumulator;
			...
			for(int i1 = 1; i1 < samples; i1 += 1) {
				for(int j1 = 0; j1 < samples; j1 += 1) {
					for(int k1 = 0; k1 < samples; k1 += 1) {
						...
						logProbability$var120[((i1 - 1) / 1)][((j1 - 0) / 1)][((k1 - 0) / 1)] = cv$rvAccumulator;
					}
				}
			}
			...
			for(int l = 0; l < samples; l += 1) {
				for(int p = 0; p < samples; p += 1) {
					for(int n = 0; n < samples; n += 1) {
							...
							{
								{
									...
									// Store the value of the function call, so the function call is only made once.
									double cv$weightedProbability = (Math.log(1.0) + Math.log((cv$sampleValue?var184:(1.0 - var184))));
						...
						// of all instances of the random variable.
						cv$accumulator = (cv$accumulator + cv$sampleAccumulator);
						logProbability$var185[((l - 0) / 1)][((p - 0) / 1)][((n - 0) / 1)] = cv$sampleAccumulator;
			...
			for(int l = 0; l < samples; l += 1) {
				for(int p = 0; p < samples; p += 1) {
					for(int n = 0; n < samples; n += 1) {
						...
						cv$sampleReached = true;
						cv$accumulator = (cv$accumulator + cv$rvAccumulator);
						logProbability$var185[((l - 0) / 1)][((p - 0) / 1)][((n - 0) / 1)] = cv$rvAccumulator;
					}
				}
			}
				...
				{
					{
						...
						// Store the value of the function call, so the function call is only made once.
						double cv$weightedProbability = (Math.log(1.0) + (((0.0 <= cv$sampleValue) && (cv$sampleValue < states))?Math.log(var79[cv$sampleValue]):Double.NEGATIVE_INFINITY));
		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numNumStates; cv$valuePos += 1) {
				...
				}
				{
					...
					int cv$temp$1$$var321;
					{
						// Constructing a random variable input for use later.
						int $var321 = states;
						cv$temp$1$$var321 = $var321;
					}
					
					// An accumulator to allow the value for each distribution to be constructed before
					// it is added to the index probabilities.
					double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < cv$temp$1$$var321))?Math.log(cv$temp$0$var119[cv$currentValue]):Double.NEGATIVE_INFINITY));
																...
																{
																	{
																		...
																		if(((Math.log(1.0) + Math.log((flips[l][n][p]?cv$temp$2$var184:(1.0 - cv$temp$2$var184)))) < cv$accumulatedConsumerProbabilities))
																			cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(1.0) + Math.log((flips[l][n][p]?cv$temp$2$var184:(1.0 - cv$temp$2$var184)))) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
																		else {
																			// If the second value is -infinity.
																			if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
																				cv$accumulatedConsumerProbabilities = (Math.log(1.0) + Math.log((flips[l][n][p]?cv$temp$2$var184:(1.0 - cv$temp$2$var184))));
																			else
																				cv$accumulatedConsumerProbabilities = (Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(1.0) + Math.log((flips[l][n][p]?cv$temp$2$var184:(1.0 - cv$temp$2$var184)))))) + 1)) + (Math.log(1.0) + Math.log((flips[l][n][p]?cv$temp$2$var184:(1.0 - cv$temp$2$var184)))));
																		}
		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numNumStates; cv$valuePos += 1) {
				...
				}
				{
					...
					int cv$temp$1$$var297;
					{
						// Constructing a random variable input for use later.
						int $var297 = states;
						cv$temp$1$$var297 = $var297;
					}
					
					// An accumulator to allow the value for each distribution to be constructed before
					// it is added to the index probabilities.
					double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < cv$temp$1$$var297))?Math.log(cv$temp$0$var79[cv$currentValue]):Double.NEGATIVE_INFINITY));
	...
	public final void allocator() {
		...
		{
			logProbability$var185 = new double[((((length$flipsMeasured.length - 1) - 0) / 1) + 1)][][];
			for(int l = 0; l < length$flipsMeasured.length; l += 1) {
				double[][] subarray$0 = new double[((((length$flipsMeasured.length - 1) - 0) / 1) + 1)][];
				logProbability$var185[((l - 0) / 1)] = subarray$0;
	...
	private final void initializeLogProbabilityFields() {
		...
		for(int i1 = 1; i1 < samples; i1 += 1) {
			for(int j1 = 0; j1 < samples; j1 += 1) {
				for(int k1 = 0; k1 < samples; k1 += 1)
					logProbability$var120[((i1 - 1) / 1)][((j1 - 0) / 1)][((k1 - 0) / 1)] = Double.NaN;
			}
		}
		if(!fixedProbFlag$sample124) {
			for(int i1 = 1; i1 < samples; i1 += 1) {
		...
		for(int l = 0; l < samples; l += 1) {
			for(int p = 0; p < samples; p += 1) {
				for(int n = 0; n < samples; n += 1)
					logProbability$var185[((l - 0) / 1)][((p - 0) / 1)][((n - 0) / 1)] = Double.NaN;
			}
		}
