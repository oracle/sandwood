...
final class HMMTestPart5$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void logProbabilityValue$sample89() {
		...
		if(!state.fixedProbFlag$sample89) {
			...
			for(int j = 0; j < state.samples; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double cv$distributionAccumulator = (((0.0 <= var86) && (var86 <= 1.0))?Math.log((state.flips[j]?var86:(1.0 - var86))):Double.NEGATIVE_INFINITY);
				...
				// Store the sample task probability
				state.logProbability$sample89[j] = cv$distributionAccumulator;
			}
			...
			double cv$accumulator = 0.0;
			for(int j = 0; j < state.samples; j += 1)
				cv$accumulator = (cv$accumulator + state.logProbability$sample89[j]);
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample89) {
			for(int j = 0; j < state.samples; j += 1)
				state.logProbability$sample89[j] = Double.NaN;
		}
	}
