	...
	private final void sample71(int i$var64) {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numNumStates; cv$valuePos += 1) {
			...
			st[(i$var64 + 1)] = cv$currentValue;
			{
				...
				int cv$temp$1$$var276;
				{
					// Constructing a random variable input for use later.
					int $var276 = states;
					cv$temp$1$$var276 = $var276;
				}
				
				// An accumulator to allow the value for each distribution to be constructed before
				// it is added to the index probabilities.
				double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < cv$temp$1$$var276))?Math.log(cv$temp$0$var68[cv$currentValue]):Double.NEGATIVE_INFINITY));
											...
											{
												{
													...
													int cv$temp$3$$var285;
													{
														// Constructing a random variable input for use later.
														int $var285 = states;
														cv$temp$3$$var285 = $var285;
													}
													
													// Record the probability of sample task 71 generating output with current configuration.
													if(((Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var285))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)) < cv$accumulatedConsumerProbabilities))
														cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var285))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
													else {
														// If the second value is -infinity.
														if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
															cv$accumulatedConsumerProbabilities = (Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var285))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY));
														else
															cv$accumulatedConsumerProbabilities = (Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var285))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)))) + 1)) + (Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var285))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)));
													}
