																	...
																	{
																		{
																			...
																			if((0 < state.k)) {
																				...
																				for(int cv$reduction429Index = 0; cv$reduction429Index < j$var55; cv$reduction429Index += 1) {
																					...
																					double j$var68 = state.phi[((i$var45 - 0) / 1)][cv$reduction429Index];
																				...
																				for(int cv$reduction429Index = (j$var55 + 1); cv$reduction429Index < state.k; cv$reduction429Index += 1) {
																					// Set the left hand term of the reduction function to the return variable value.
																					double i$var67 = reduceVar$var70$6;
												...
												{
													{
														...
														double cv$var72$mu = (reduceVar$var70$8 + state.bias);
	...
	private final void logProbabilityValue$sample74() {
		...
		if(!state.fixedProbFlag$sample74) {
						...
						{
							{
								...
								double var71 = (reduceVar$var70$9 + state.bias);
			...
			if(cv$sampleReached)
				...
				state.logProbability$var73 = cv$accumulator;
			...
			for(int i$var45 = 0; i$var45 < state.n; i$var45 += 1)
				// Record that the sample was reached.
				cv$sampleReached = true;
			double cv$sampleValue = state.logProbability$var73;
			cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
			cv$accumulator = (cv$accumulator + cv$rvAccumulator);
		...
		parallelFor(state.RNG$, 0, state.k, 1,
			(int forStart$var23, int forEnd$var23, int threadID$var23, org.sandwood.random.internal.Rng RNG$1) -> { 
					...
					for(int var23 = forStart$var23; var23 < forEnd$var23; var23 += 1) {
						if(!state.fixedFlag$sample24)
							state.weights[var23] = ((Math.sqrt(10.0) * DistributionSampling.sampleGaussian(RNG$1)) + 0.0);
					}
			}
		);
		if(!state.fixedFlag$sample31)
