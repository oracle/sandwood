...
import org.sandwood.compiler.tests.parser.HMMTestPart2$SingleThreadCPU.Scratch;
...
final class HMMTestPart2$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
...
double[] cv$var28$countGlobal;
	...
	public HMMTestPart2$SingleThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
	...
	private final void logProbabilityValue$sample84() {
		if(!state.fixedProbFlag$sample84) {
			...
			for(int j = 1; j < state.samples; j += 1) {
				...
				cv$sampleAccumulator = (cv$sampleAccumulator + cv$sampleProbability);
				cv$accumulator = (cv$accumulator + cv$sampleAccumulator);
				state.logProbability$sample84[((j - 1) / 1)] = cv$sampleProbability;
			}
			...
			for(int j = 1; j < state.samples; j += 1) {
				double cv$rvAccumulator = 0.0;
				double cv$sampleValue = state.logProbability$sample84[((j - 1) / 1)];
				cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
				cv$sampleReached = true;
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
			}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample84) {
			for(int j = 1; j < state.samples; j += 1)
				state.logProbability$sample84[((j - 1) / 1)] = Double.NaN;
		}
	}
