...
final class AnonymousSample$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	private final void logProbabilityValue$sample35() {
		...
		if(!state.fixedProbFlag$sample35) {
			...
			for(int i = 0; i < state.n; i += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + ((0.0 < state.priorSigma2)?(DistributionSampling.logProbabilityGaussian(((state.amounts1[i] - state.mean1) / Math.sqrt(state.priorSigma2))) - (Math.log(state.priorSigma2) * 0.5)):Double.NEGATIVE_INFINITY));
			}
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			state.logProbability$amounts1 = (state.logProbability$amounts1 + cv$sampleAccumulator);
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$amounts1 = (state.logProbability$amounts1 + state.logProbability$var35);
	...
	private final void logProbabilityValue$sample39() {
		...
		if(!state.fixedProbFlag$sample39) {
			...
			for(int i = 0; i < state.n; i += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + ((0.0 < state.priorSigma2)?(DistributionSampling.logProbabilityGaussian(((state.var39[i] - state.mean2) / Math.sqrt(state.priorSigma2))) - (Math.log(state.priorSigma2) * 0.5)):Double.NEGATIVE_INFINITY));
			}
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var39 = cv$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			state.logProbability$amounts2 = (state.logProbability$amounts2 + cv$sampleAccumulator);
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$amounts2 = (state.logProbability$amounts2 + state.logProbability$var39);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var39);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var39);
		}
	}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample39)
			state.logProbability$var39 = Double.NaN;
	}
