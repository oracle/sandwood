	...
	private final void logProbabilityValue$sample80() {
		...
		if(!fixedProbFlag$sample80) {
			...
			for(int i$var69 = 4; i$var69 < (samples + 3); i$var69 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= cv$sampleValue) && (cv$sampleValue < var75.length))?Math.log(var75[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
	...
	private final void sample31(int var29) {
		...
		for(int i$var69 = 4; i$var69 < (samples + 3); i$var69 += 1) {
			if((var29 == st[(i$var69 - 4)]))
				...
				// 
				// A local reference to the scratch space.
				cv$var30$countGlobal[(st[(i$var69 - 3)] / 2)] = (cv$var30$countGlobal[(st[(i$var69 - 3)] / 2)] + 1.0);
		}
