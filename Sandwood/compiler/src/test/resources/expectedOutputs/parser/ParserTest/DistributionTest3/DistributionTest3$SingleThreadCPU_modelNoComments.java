...
final class DistributionTest3$SingleThreadCPU extends CoreModelSingleThreadCPU implements DistributionTest3$CoreInterface {
		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
				...
				cv$currentValue = cv$valuePos;
				{
					...
					double cv$accumulatedProbabilities = (Math.log(1.0) + ((((((0.0 <= cv$currentValue) && (cv$currentValue < $var47)) && (0 < $var47)) && (0.0 <= weightings[cv$currentValue])) && (weightings[cv$currentValue] <= 1.0))?Math.log(weightings[cv$currentValue]):Double.NEGATIVE_INFINITY));
					{
						{
							{
		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
				...
				cv$currentValue = cv$valuePos;
				{
					...
					double cv$accumulatedProbabilities = (Math.log(1.0) + ((((((0.0 <= cv$currentValue) && (cv$currentValue < $var60)) && (0 < $var60)) && (0.0 <= weightings[cv$currentValue])) && (weightings[cv$currentValue] <= 1.0))?Math.log(weightings[cv$currentValue]):Double.NEGATIVE_INFINITY));
					{
						{
							{
