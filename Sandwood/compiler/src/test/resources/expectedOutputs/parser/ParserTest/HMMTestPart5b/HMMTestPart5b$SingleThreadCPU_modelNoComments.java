...
double[] cv$var28$countGlobal;
	...
	private final void logProbabilityValue$sample76() {
		if(!state.fixedProbFlag$sample76) {
			...
			for(int i$var67 = 4; i$var67 < (state.samples + 3); i$var67 += 1) {
				...
				cv$sampleAccumulator = (cv$sampleAccumulator + cv$sampleProbability);
				cv$accumulator = (cv$accumulator + cv$sampleAccumulator);
				state.logProbability$sample76[((i$var67 - 4) / 1)] = cv$sampleProbability;
			}
			state.logProbability$st = (state.logProbability$st + cv$accumulator);
			state.logProbability$$model = (state.logProbability$$model + cv$accumulator);
			...
			for(int i$var67 = 4; i$var67 < (state.samples + 3); i$var67 += 1) {
				double cv$rvAccumulator = 0.0;
				double cv$sampleValue = state.logProbability$sample76[((i$var67 - 4) / 1)];
				cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
				cv$sampleReached = true;
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
			}
	...
	private final void logProbabilityValue$sample99() {
		if(!state.fixedProbFlag$sample99) {
			...
			for(int j = 5; j < (state.samples + 5); j += 1) {
				...
				cv$sampleAccumulator = (cv$sampleAccumulator + cv$sampleProbability);
				cv$accumulator = (cv$accumulator + cv$sampleAccumulator);
				state.logProbability$sample99[((j - 5) / 1)] = cv$sampleProbability;
			}
			...
			for(int j = 5; j < (state.samples + 5); j += 1) {
				double cv$rvAccumulator = 0.0;
				double cv$sampleValue = state.logProbability$sample99[((j - 5) / 1)];
				cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
				cv$sampleReached = true;
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
			}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample76) {
			for(int i$var67 = 4; i$var67 < (state.samples + 3); i$var67 += 1)
				state.logProbability$sample76[((i$var67 - 4) / 1)] = Double.NaN;
		}
		state.logProbability$flips = 0.0;
		if(!state.fixedProbFlag$sample99) {
			for(int j = 5; j < (state.samples + 5); j += 1)
				state.logProbability$sample99[((j - 5) / 1)] = Double.NaN;
		}
	}
