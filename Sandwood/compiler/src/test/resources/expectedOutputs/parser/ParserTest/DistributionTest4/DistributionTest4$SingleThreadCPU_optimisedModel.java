...
final class DistributionTest4$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void inferSample11() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var468) && (0 < $var468)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	private final void inferSample27(int i) {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var1125) && (0 < $var1125)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
			{
				int j = (i + 1);
				if((j < state.size))
					// Set the flags to false
	...
	private final void inferSample5() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var138) && (0 < $var138)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	public final void forwardGeneration() {
		...
		for(int j = 0; j < state.size; j += 1)
			state.v[j] = DistributionSampling.sampleBernoulli(state.RNG$, ((double)((state.v1 + state.v2[j]) + state.v2[(j + 1)]) / state.v2[(j + 1)]));
	}
