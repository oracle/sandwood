...
class HMMTestPart3$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements HMMTestPart3$CoreInterface {
	...
	private final void sample75(int i$var66) {
		// Unrolled loop
		{
			...
			// 
			// Constructing a random variable input for use later.
			double[] cv$temp$0$var70 = m[st[(i$var66 - 1)]];
			...
			// inputs.
			// 
			// Substituted "j" with its value "i$var66".
			// 
			// cv$temp$2$var86's comment
			// Constructing a random variable input for use later.
			// 
			...
			// 
			// Substituted "cv$valuePos" with its value "0".
			cv$accumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flips[i$var66], bias[0]) + cv$accumulatedProbabilities);
		...
		// 
		// Substituted "cv$valuePos" with its value "1".
		cv$accumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flips[i$var66], bias[1]) + cv$accumulatedProbabilities);
		...
		// 
		// Get a local reference to the scratch space.
		st[i$var66] = DistributionSampling.sampleCategorical(RNG$, cv$var72$stateProbabilityGlobal);
	}
