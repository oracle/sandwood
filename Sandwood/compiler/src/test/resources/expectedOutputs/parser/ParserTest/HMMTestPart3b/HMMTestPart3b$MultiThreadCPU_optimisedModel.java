	...
	private final void logProbabilityValue$sample49() {
		...
		if(!fixedProbFlag$sample49) {
			...
			for(int i$var39 = 4; i$var39 < (samples + 3); i$var39 += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double cv$distributionAccumulator = DistributionSampling.logProbabilityCategorical(st[(i$var39 - 3)], m[st[(i$var39 - 4)]]);
	...
	private final void sample49(int i$var39) {
		{
			...
			// it is added to the index probabilities.
			// 
			// cv$temp$0$var45's comment
			// Constructing a random variable input for use later.
			double cv$accumulatedProbabilities = DistributionSampling.logProbabilityCategorical(0, m[st[(i$var39 - 4)]]);
			...
			if((index$i$1_2 < (samples + 3)))
				...
				// 
				// Substituted "cv$valuePos" with its value "0".
				cv$accumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[(index$i$1_2 - 3)], m[0]) + cv$accumulatedProbabilities);
		...
		// cv$temp$0$var45's comment
		// Constructing a random variable input for use later.
		double cv$accumulatedProbabilities = DistributionSampling.logProbabilityCategorical(1, m[st[(i$var39 - 4)]]);
		int index$i$1_2 = (i$var39 + 1);
		if((index$i$1_2 < (samples + 3)))
			// Processing sample task 49 of consumer random variable null.
			// 
			...
			// 
			// Substituted "cv$valuePos" with its value "1".
			cv$accumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[(index$i$1_2 - 3)], m[1]) + cv$accumulatedProbabilities);
