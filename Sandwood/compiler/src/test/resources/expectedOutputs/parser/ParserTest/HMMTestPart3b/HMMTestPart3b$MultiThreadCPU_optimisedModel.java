...
final class HMMTestPart3b$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	final class Scratch implements CoreModelScratch {
		...
		public final void allocateScratch() {
			...
			// of putTask 29.
			cv$var74$stateProbabilityGlobal = new double[2];
		}
	}
	...
	private final void drawValueSample75(int i$var66) {
		state.st[(i$var66 - 3)] = DistributionSampling.sampleCategorical(state.RNG$, state.m[state.st[(i$var66 - 4)]], 2);
	}
	...
	private final void inferSample28(int var27, int threadID$cv$var27, Rng RNG$) {
		...
		for(int i$var66 = 4; i$var66 < (state.samples + 3); i$var66 += 1) {
			if(((var27 == state.st[(i$var66 - 4)]) && (state.fixedFlag$sample75 || state.constrainedFlag$sample75[(i$var66 - 4)]))) {
	...
	private final void inferSample53() {
		...
		// Unrolled loop
		{
			...
			if(((1 < state.samples) && (state.fixedFlag$sample75 || state.constrainedFlag$sample75[0]))) {
				// Processing sample task 75 of consumer random variable null.
				// Mark that the sample has observed constrained data.
				...
				// 
												// Substituted "i$var66" with its value "4".
				cv$accumulatedProbabilities = ((((((0.0 <= state.st[1]) && (state.st[1] < 2)) && (0.0 <= var72[state.st[1]])) && (var72[state.st[1]] <= 1.0))?Math.log(var72[state.st[1]]):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
			...
			if((0 < state.samples)) {
				...
				// 
				// Substituted "j" with its value "5".
				cv$accumulatedProbabilities = ((((0.0 <= var94) && (var94 <= 1.0))?Math.log((state.flips[0]?var94:(1.0 - var94))):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
	...
	private final void inferSample75(int i$var66) {
		state.constrainedFlag$sample75[(i$var66 - 4)] = false;
		...
		// Unrolled loop
		{
			...
			// 
									// Substituted "cv$valuePos" with its value "0".
			double cv$accumulatedProbabilities = (((0.0 <= var72[0]) && (var72[0] <= 1.0))?Math.log(var72[0]):Double.NEGATIVE_INFINITY);
			int index$i$2_2 = (i$var66 + 1);
			if(((index$i$2_2 < (state.samples + 3)) && (state.fixedFlag$sample75 || state.constrainedFlag$sample75[(index$i$2_2 - 4)]))) {
				// Processing sample task 75 of consumer random variable null.
				// Constructing a random variable input for use later.
				// 
				// Processing random variable 73.
				...
				// Set an accumulator to sum the probabilities for each possible configuration of
				// inputs.
				cv$accumulatedProbabilities = ((((((0.0 <= state.st[(index$i$2_2 - 3)]) && (state.st[(index$i$2_2 - 3)] < 2)) && (0.0 <= sc$var72$1[state.st[(index$i$2_2 - 3)]])) && (sc$var72$1[state.st[(index$i$2_2 - 3)]] <= 1.0))?Math.log(sc$var72$1[state.st[(index$i$2_2 - 3)]]):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
			
			// Mark that the sample has observed constrained data.
			state.constrainedFlag$sample75[(i$var66 - 4)] = true;
			...
			cv$accumulatedProbabilities = ((((0.0 <= var94) && (var94 <= 1.0))?Math.log((state.flips[(i$var66 - 3)]?var94:(1.0 - var94))):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			...
			// 
			// Initialize a counter to track the reached distributions.
			scratch.cv$var74$stateProbabilityGlobal[0] = cv$accumulatedProbabilities;
		}
		...
		// 
						// Substituted "cv$valuePos" with its value "1".
		double cv$accumulatedProbabilities = (((0.0 <= var72[1]) && (var72[1] <= 1.0))?Math.log(var72[1]):Double.NEGATIVE_INFINITY);
		int index$i$2_2 = (i$var66 + 1);
		if(((index$i$2_2 < (state.samples + 3)) && (state.fixedFlag$sample75 || state.constrainedFlag$sample75[(index$i$2_2 - 4)]))) {
			// Processing sample task 75 of consumer random variable null.
			// Constructing a random variable input for use later.
		...
		// 
		// Initialize a counter to track the reached distributions.
		scratch.cv$var74$stateProbabilityGlobal[1] = cv$accumulatedProbabilities;
		if(state.constrainedFlag$sample75[(i$var66 - 4)]) {
			// This value is not used before it is set again, so removing the value declaration.
			// 
			// The sum of all the probabilities in log space
			double cv$logSum;
			...
			// 
			// Get a local reference to the scratch space.
			double cv$lseElementValue = scratch.cv$var74$stateProbabilityGlobal[1];
			if((cv$lseMax < cv$lseElementValue))
				cv$lseMax = cv$lseElementValue;
			...
			// Sum the values in the array.
				...
				// 
				// Initialise the sum of the array elements
				cv$logSum = (Math.log((Math.exp((scratch.cv$var74$stateProbabilityGlobal[0] - cv$lseMax)) + Math.exp((scratch.cv$var74$stateProbabilityGlobal[1] - cv$lseMax)))) + cv$lseMax);
			
			// If all the sum is zero, just share the probability evenly.
			if((cv$logSum == Double.NEGATIVE_INFINITY)) {
				// Unrolled loop
												// Get a local reference to the scratch space.
				scratch.cv$var74$stateProbabilityGlobal[0] = 0.5;
				
												// Get a local reference to the scratch space.
				scratch.cv$var74$stateProbabilityGlobal[1] = 0.5;
			} else {
				// Unrolled loop
												// Get a local reference to the scratch space.
				scratch.cv$var74$stateProbabilityGlobal[0] = Math.exp((scratch.cv$var74$stateProbabilityGlobal[0] - cv$logSum));
				
												// Get a local reference to the scratch space.
				scratch.cv$var74$stateProbabilityGlobal[1] = Math.exp((scratch.cv$var74$stateProbabilityGlobal[1] - cv$logSum));
			}
			
			// Set array values that are not computed for the input to negative infinity.
			// 
									// Get a local reference to the scratch space.
			for(int cv$indexName = 2; cv$indexName < scratch.cv$var74$stateProbabilityGlobal.length; cv$indexName += 1)
				// Get a local reference to the scratch space.
				scratch.cv$var74$stateProbabilityGlobal[cv$indexName] = Double.NEGATIVE_INFINITY;
			...
			// 
												// cv$numStates's comment
			// variable marginalization
			state.st[(i$var66 - 3)] = DistributionSampling.sampleCategorical(state.RNG$, scratch.cv$var74$stateProbabilityGlobal, 2);
		}
	}
	...
	private final void logProbabilityValue$sample75() {
		...
		if(!state.fixedProbFlag$sample75) {
			...
			for(int i$var66 = 4; i$var66 < (state.samples + 3); i$var66 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				double cv$distributionAccumulator = (((((0.0 <= cv$sampleValue) && (cv$sampleValue < 2)) && (0.0 <= var72[cv$sampleValue])) && (var72[cv$sampleValue] <= 1.0))?Math.log(var72[cv$sampleValue]):Double.NEGATIVE_INFINITY);
				...
				// Store the sample task probability
				state.logProbability$sample75[(i$var66 - 4)] = cv$distributionAccumulator;
			}
			...
			double cv$accumulator = 0.0;
			for(int i$var66 = 4; i$var66 < (state.samples + 3); i$var66 += 1)
				cv$accumulator = (cv$accumulator + state.logProbability$sample75[(i$var66 - 4)]);
	...
	private final void logProbabilityValue$sample97() {
		...
		if(!state.fixedProbFlag$sample97) {
			...
			for(int j = 5; j < (state.samples + 5); j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double cv$distributionAccumulator = (((0.0 <= var94) && (var94 <= 1.0))?Math.log((state.flips[(j - 5)]?var94:(1.0 - var94))):Double.NEGATIVE_INFINITY);
				...
				// Store the sample task probability
				state.logProbability$sample97[(j - 5)] = cv$distributionAccumulator;
			}
			...
			double cv$accumulator = 0.0;
			for(int j = 5; j < (state.samples + 5); j += 1)
				cv$accumulator = (cv$accumulator + state.logProbability$sample97[(j - 5)]);
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample75) {
			for(int i$var66 = 4; i$var66 < (state.samples + 3); i$var66 += 1)
				state.logProbability$sample75[(i$var66 - 4)] = Double.NaN;
		}
		state.logProbability$flips = 0.0;
		if(!state.fixedProbFlag$sample97) {
			for(int j = 5; j < (state.samples + 5); j += 1)
				state.logProbability$sample97[(j - 5)] = Double.NaN;
		}
	}
