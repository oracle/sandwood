...
final class HMMTestPart2b$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void drawValueSample71(int i$var64) {
		state.st[(i$var64 + 1)] = DistributionSampling.sampleCategorical(state.RNG$, state.m[state.st[i$var64]], 2);
	}
	...
	private final void inferSample28(int var27) {
		...
		for(int i$var64 = 0; i$var64 < (state.samples - 1); i$var64 += 1) {
			if(((var27 == state.st[i$var64]) && (state.fixedFlag$sample71 || state.constrainedFlag$sample71[i$var64]))) {
				...
				// 
												// A local reference to the scratch space.
				scratch.cv$var28$countGlobal[state.st[(i$var64 + 1)]] = (scratch.cv$var28$countGlobal[state.st[(i$var64 + 1)]] + 1.0);
			}
		}
	...
	private final void inferSample71(int i$var64) {
		state.constrainedFlag$sample71[i$var64] = false;
		...
		// Unrolled loop
		{
			...
			if(((index$i$2_2 < (state.samples - 1)) && (state.fixedFlag$sample71 || state.constrainedFlag$sample71[index$i$2_2]))) {
				// Processing sample task 71 of consumer random variable null.
				// Constructing a random variable input for use later.
				// 
				// Processing random variable 69.
				...
				// inputs.
				cv$accumulatedProbabilities = ((((((0.0 <= state.st[(index$i$2_2 + 1)]) && (state.st[(index$i$2_2 + 1)] < 2)) && (0.0 <= sc$var68$1[state.st[(index$i$2_2 + 1)]])) && (sc$var68$1[state.st[(index$i$2_2 + 1)]] <= 1.0))?Math.log(sc$var68$1[state.st[(index$i$2_2 + 1)]]):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
			
			// Mark that the sample has observed constrained data.
			state.constrainedFlag$sample71[i$var64] = true;
			...
			cv$accumulatedProbabilities = ((((0.0 <= var83) && (var83 <= 1.0))?Math.log((state.flips[(i$var64 + 1)]?var83:(1.0 - var83))):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
		...
		if(state.constrainedFlag$sample71[i$var64]) {
			...
			state.st[(i$var64 + 1)] = DistributionSampling.sampleCategorical(state.RNG$, scratch.cv$var70$stateProbabilityGlobal, 2);
		}
	}
	...
	private final void logProbabilityValue$sample71() {
		...
		if(!state.fixedProbFlag$sample71) {
			...
			for(int i$var64 = 0; i$var64 < (state.samples - 1); i$var64 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				double cv$distributionAccumulator = (((((0.0 <= cv$sampleValue) && (cv$sampleValue < 2)) && (0.0 <= var68[cv$sampleValue])) && (var68[cv$sampleValue] <= 1.0))?Math.log(var68[cv$sampleValue]):Double.NEGATIVE_INFINITY);
				...
				state.logProbability$sample71[i$var64] = cv$distributionAccumulator;
			}
			...
			double cv$accumulator = 0.0;
			for(int i$var64 = 0; i$var64 < (state.samples - 1); i$var64 += 1)
				cv$accumulator = (cv$accumulator + state.logProbability$sample71[i$var64]);
	...
	private final void logProbabilityValue$sample86() {
		...
		if(!state.fixedProbFlag$sample86) {
			...
			for(int j = 1; j < state.samples; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double cv$distributionAccumulator = (((0.0 <= var83) && (var83 <= 1.0))?Math.log((state.flips[j]?var83:(1.0 - var83))):Double.NEGATIVE_INFINITY);
				...
				// Store the sample task probability
				state.logProbability$sample86[(j - 1)] = cv$distributionAccumulator;
			}
			...
			double cv$accumulator = 0.0;
			for(int j = 1; j < state.samples; j += 1)
				cv$accumulator = (cv$accumulator + state.logProbability$sample86[(j - 1)]);
	...
	public final void gibbsRound() {
		...
		// Infer the samples in reverse chronological order.
			...
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample71) {
				for(int i$var64 = (state.samples - 2); i$var64 >= 0; i$var64 -= 1)
					inferSample71(i$var64);
			}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample71) {
			for(int i$var64 = 0; i$var64 < (state.samples - 1); i$var64 += 1)
				state.logProbability$sample71[i$var64] = Double.NaN;
		}
		state.logProbability$flips = 0.0;
		if(!state.fixedProbFlag$sample86) {
			for(int j = 1; j < state.samples; j += 1)
				state.logProbability$sample86[(j - 1)] = Double.NaN;
		}
	}
