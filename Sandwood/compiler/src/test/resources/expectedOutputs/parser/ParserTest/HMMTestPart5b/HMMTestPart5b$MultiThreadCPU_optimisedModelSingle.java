...
class HMMTestPart5b$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart5b$CoreInterface {
	...
	public final void set$bias(double[] cv$value) {
		...
		// Unset the fixed probability flag for sample 67 as it depends on bias.
	...
	public final void set$flips(boolean[] cv$value) {
		...
		// Unset the fixed probability flag for sample 67 as it depends on flips.
		fixedProbFlag$sample67 = false;
	}
	...
	public final void set$m(double[][] cv$value) {
		...
		// Unset the fixed probability flag for sample 51 as it depends on m.
	...
	public final void set$st(int[] cv$value) {
		...
		// Unset the fixed probability flag for sample 51 as it depends on st.
		fixedProbFlag$sample51 = false;
	...
	private final void logProbabilityValue$sample51() {
		...
		if(!fixedProbFlag$sample51) {
			...
			for(int i$var40 = 4; i$var40 < (samples + 3); i$var40 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= cv$sampleValue) && (cv$sampleValue < var46.length))?Math.log(var46[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var47 = logProbability$var48;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$st = (logProbability$st + logProbability$var48);
	...
	private final void logProbabilityValue$sample67() {
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var64);
	...
	private final void sample27(int var24, int threadID$cv$var24, Rng RNG$) {
		...
		// 
		// Looking for a path between Sample 27 and consumer Bernoulli 63.
		for(int j = 5; j < (samples + 5); j += 1) {
			if((var24 == st[(j - 5)])) {
				// Processing sample task 67 of consumer random variable null.
				// 
				// Include the value sampled by task 67 from random variable var63.
	...
	private final void sample36() {
		// Unrolled loop
		{
			...
			// Substituted "i$var40" with its value "4".
			if((1 < samples)) {
				// Processing sample task 51 of consumer random variable null.
				// Variable declaration of cv$temp$1$var46 moved.
				// 
				// Constructing a random variable input for use later.
				// 
				// Processing random variable 47.
				// 
				// Looking for a path between Sample 36 and consumer Categorical 47.
				// 
				// Value of the variable at this index
				// 
				// Substituted "cv$valuePos" with its value "0".
			...
			if((0 < samples))
				...
				// Substituted "j" with its value "5".
				// 
				// cv$temp$2$var62's comment
				// Constructing a random variable input for use later.
				// 
				// Processing random variable 63.
				// 
				// Looking for a path between Sample 36 and consumer Bernoulli 63.
				// 
				// Value of the variable at this index
				// 
				// Substituted "cv$valuePos" with its value "0".
	...
	// Method to perform the inference steps to calculate new values for the samples generated
	// by sample task 51 drawn from Categorical 47. Inference was performed using variable
	// marginalization.
	private final void sample51(int i$var40) {
		// Unrolled loop
		{
			...
			// Variable declaration of cv$temp$0$var46 moved.
			// 
			// Constructing a random variable input for use later.
			...
			if((index$i$1_2 < (samples + 3))) {
				...
				// Processing random variable 47.
				// 
				// Looking for a path between Sample 51 and consumer Categorical 47.
				// 
				// Value of the variable at this index
				// 
				// Substituted "cv$valuePos" with its value "0".
			...
			// Processing random variable 63.
			// 
			// Looking for a path between Sample 51 and consumer Bernoulli 63.
			// 
			// Value of the variable at this index
			// 
			// Substituted "cv$valuePos" with its value "0".
		...
		// 
		// Get a local reference to the scratch space.
		double cv$lseElementValue = cv$var48$stateProbabilityGlobal[1];
		if((cv$lseMax < cv$lseElementValue))
			cv$lseMax = cv$lseElementValue;
		...
		// Sum the values in the array.
			...
			// 
			// Initialise the sum of the array elements
			cv$logSum = (Math.log((Math.exp((cv$var48$stateProbabilityGlobal[0] - cv$lseMax)) + Math.exp((cv$var48$stateProbabilityGlobal[1] - cv$lseMax)))) + cv$lseMax);
		
		// If all the sum is zero, just share the probability evenly.
		if((cv$logSum == Double.NEGATIVE_INFINITY)) {
			// Unrolled loop
			// Get a local reference to the scratch space.
			cv$var48$stateProbabilityGlobal[0] = 0.5;
			
			// Get a local reference to the scratch space.
			cv$var48$stateProbabilityGlobal[1] = 0.5;
		} else {
			// Unrolled loop
			// Get a local reference to the scratch space.
			cv$var48$stateProbabilityGlobal[0] = Math.exp((cv$var48$stateProbabilityGlobal[0] - cv$logSum));
			
			// Get a local reference to the scratch space.
			cv$var48$stateProbabilityGlobal[1] = Math.exp((cv$var48$stateProbabilityGlobal[1] - cv$logSum));
		}
		
		// Set array values that are not computed for the input to negative infinity.
		// 
		// Get a local reference to the scratch space.
		for(int cv$indexName = 2; cv$indexName < cv$var48$stateProbabilityGlobal.length; cv$indexName += 1)
			// Get a local reference to the scratch space.
			cv$var48$stateProbabilityGlobal[cv$indexName] = Double.NEGATIVE_INFINITY;
