...
class HMMTestPart5$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart5$CoreInterface {
	...
	private final void logProbabilityValue$sample93() {
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var89 = logProbability$var90;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var90);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$$model = (logProbability$$model + logProbability$var90);
			
			// Variable declaration of cv$accumulator moved.
			logProbability$$evidence = (logProbability$$evidence + logProbability$var90);
		}
	}
	...
	private final void sample31(int var29, int threadID$cv$var29, Rng RNG$) {
		...
		for(int i$var67 = 1; i$var67 < samples; i$var67 += 1) {
			if((var29 == st[(i$var67 - 1)]))
				...
				// variable var72
				cv$countLocal[(st[i$var67] / 2)] = (cv$countLocal[(st[i$var67] / 2)] + 1.0);
		}
