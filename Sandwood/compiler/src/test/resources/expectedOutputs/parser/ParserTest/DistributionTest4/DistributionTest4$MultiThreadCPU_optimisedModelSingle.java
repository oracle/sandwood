...
final class DistributionTest4$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	private final void inferSample11() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			int $var2914 = state.weightings.length;
			
			// An accumulator to allow the value for each distribution to be constructed before
			// it is added to the index probabilities.
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var2914) && (0 < $var2914)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	private final void inferSample27(int i) {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			// Constructing a random variable input for use later.
			int $var3571 = state.weightings.length;
			
			// An accumulator to allow the value for each distribution to be constructed before
			// it is added to the index probabilities.
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var3571) && (0 < $var3571)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
			{
				int j = (i + 1);
				if((j < state.size))
					// Set the flags to false
	...
	private final void inferSample5() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			// Constructing a random variable input for use later.
			int $var2584 = state.weightings.length;
			
			// An accumulator to allow the value for each distribution to be constructed before
			// it is added to the index probabilities.
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var2584) && (0 < $var2584)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	private final void logProbabilityDistribution$sample53() {
		...
		if(!state.fixedProbFlag$sample53) {
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var53 = cv$sampleAccumulator;
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$v = (state.logProbability$v + state.logProbability$var53);
	...
	private final void logProbabilityValue$sample53() {
		...
		if(!state.fixedProbFlag$sample53) {
			...
			for(int j = 0; j < state.size; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var51) && (var51 <= 1.0))?Math.log((state.v[j]?var51:(1.0 - var51))):Double.NEGATIVE_INFINITY));
			}
