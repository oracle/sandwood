...
final class ReductionTest1$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	private final void inferSample101(int i$var80, int var95, int threadID$cv$i$var80, Rng RNG$) {
		...
		double cv$proposedValue = ((Math.sqrt(cv$var) * DistributionSampling.sampleGaussian(RNG$)) + cv$originalValue);
		{
			...
			for(int t = 1; t < state.T; t += 1) {
				...
				for(int cv$reduction728Index = 0; cv$reduction728Index < var95; cv$reduction728Index += 1)
					// Execute the reduction function, saving the result into the return value.
					// 
					// Copy the result of the reduction into the variable returned by the reduction.
					// 
					...
					reduceVar$var151$11 = (reduceVar$var151$11 + state.time_impact[t][i$var80][cv$reduction728Index]);
				
				// Substituted "j" with its value "var95".
				for(int cv$reduction728Index = (var95 + 1); cv$reduction728Index < state.time_dim; cv$reduction728Index += 1)
					// Execute the reduction function, saving the result into the return value.
					// 
					// Copy the result of the reduction into the variable returned by the reduction.
					// 
	...
	private final void logProbabilityValue$sample101() {
		...
		if(!state.fixedProbFlag$sample101) {
			...
			state.logProbability$time_coeff = (state.logProbability$time_coeff + cv$sampleAccumulator);
			...
			// Update the variable probability
			state.logProbability$time_coeff = (state.logProbability$time_coeff + cv$rvAccumulator);
		...
		if(!state.fixedProbFlag$sample165) {
			...
			for(int t = 1; t < state.T; t += 1) {
				for(int i$var119 = 0; i$var119 < state.n_ac; i$var119 += 1) {
					...
					// 
					// The sample value to calculate the probability of generating
					cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityPoisson(state.arr[t][i$var119], state.sum_t[t][i$var119]));
				}
			}
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var158 = cv$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			state.logProbability$arr = (state.logProbability$arr + cv$sampleAccumulator);
			...
			state.logProbability$arr = (state.logProbability$arr + state.logProbability$var158);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var158);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var158);
		}
	}
