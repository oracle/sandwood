package org.sandwood.compiler.tests.parser;

interface ReductionTest1$CoreInterface extends org.sandwood.runtime.internal.model.CoreModel {

	// Getter for ObsArr.
	public int[][] get$ObsArr();

	// Setter for ObsArr.
	public void set$ObsArr(int[][] cv$value, boolean allocated$);

	// Getter for T.
	public int get$T();

	// Setter for T.
	public void set$T(int cv$value, boolean allocated$);

	// Getter for TimeFeat.
	public double[][] get$TimeFeat();

	// Setter for TimeFeat.
	public void set$TimeFeat(double[][] cv$value, boolean allocated$);

	// Getter for arr.
	public int[][] get$arr();

	// Getter for fixedFlag$sample101.
	public boolean get$fixedFlag$sample101();

	// Setter for fixedFlag$sample101.
	public void set$fixedFlag$sample101(boolean cv$value, boolean allocated$);

	// Getter for logProbability$arr.
	public double get$logProbability$arr();

	// Getter for logProbability$sum_t.
	public double get$logProbability$sum_t();

	// Getter for logProbability$time_coeff.
	public double get$logProbability$time_coeff();

	// Getter for logProbability$time_impact.
	public double get$logProbability$time_impact();

	// Getter for n_ac.
	public int get$n_ac();

	// Setter for n_ac.
	public void set$n_ac(int cv$value, boolean allocated$);

	// Getter for sum_t.
	public double[][] get$sum_t();

	// Getter for time_coeff.
	public double[][] get$time_coeff();

	// Setter for time_coeff.
	public void set$time_coeff(double[][] cv$value, boolean allocated$);

	// Getter for time_dim.
	public int get$time_dim();

	// Getter for time_impact.
	public double[][][] get$time_impact();
}