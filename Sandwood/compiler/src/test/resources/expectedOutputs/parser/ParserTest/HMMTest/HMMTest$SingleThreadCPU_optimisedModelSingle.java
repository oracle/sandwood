	...
	private final void inferSample28(int var27) {
		...
		for(int i$var62 = 1; i$var62 < state.samples; i$var62 += 1) {
			if(((var27 == state.st[(i$var62 - 1)]) && (state.fixedFlag$sample69 || state.constrainedFlag$sample69[(i$var62 - 1)]))) {
				...
				// 
												// A local reference to the scratch space.
				scratch.cv$var28$countGlobal[state.st[i$var62]] = (scratch.cv$var28$countGlobal[state.st[i$var62]] + 1.0);
			}
		}
	...
	private final void logProbabilityValue$sample69() {
		...
		if(!state.fixedProbFlag$sample69) {
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var68 = cv$sampleAccumulator;
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$st = (state.logProbability$st + state.logProbability$var68);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var68);
			
			// If this value is fixed, add it to the probability of this model producing the fixed
			// values
			if(state.fixedFlag$sample69)
				// Variable declaration of cv$accumulator moved.
				state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var68);
		}
	}
	...
	private final void logProbabilityValue$sample84() {
		...
		if(!state.fixedProbFlag$sample84) {
			...
			for(int j = 0; j < state.samples; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var81) && (var81 <= 1.0))?Math.log((state.flips[j]?var81:(1.0 - var81))):Double.NEGATIVE_INFINITY));
			}
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var83);
