...
final class HMMMetrics$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void drawValueSample113(int var111) {
		state.pageFaultsMean[var111] = ((DistributionSampling.sampleGaussian(state.RNG$) * 579.2667779184303) + 814.0);
	}
	...
	private final void drawValueSample77(int var75) {
		state.cpuMean[var75] = ((DistributionSampling.sampleGaussian(state.RNG$) * 2.932575659723036) + 16.0);
	}
	...
	private final void drawValueSample95(int var93) {
		state.memMean[var93] = (DistributionSampling.sampleGaussian(state.RNG$) + 94.0);
	}
		...
		if(state.fixedFlag$sample57) {
			...
			if((1 < state.samples)) {
				if(state.fixedFlag$sample39) {
					...
					if((var29 == state.st[0])) {
						...
						// 
																		// A local reference to the scratch space.
						scratch.cv$var30$countGlobal[state.st[1]] = (scratch.cv$var30$countGlobal[state.st[1]] + 1.0);
					}
				} else {
					// Mark that the sample has observed constrained data.
					state.constrainedFlag$sample30[var29] = true;
					...
					scratch.cv$var30$countGlobal[state.st[1]] = (scratch.cv$var30$countGlobal[state.st[1]] + state.distribution$sample39[var29]);
				}
			}
			for(int i$var50 = 2; i$var50 < state.samples; i$var50 += 1) {
				if((var29 == state.st[(i$var50 - 1)])) {
					...
					// 
															// A local reference to the scratch space.
					scratch.cv$var30$countGlobal[state.st[i$var50]] = (scratch.cv$var30$countGlobal[state.st[i$var50]] + 1.0);
				}
			}
		}
		...
		// Constraints moved from conditionals in inner loops/scopes/etc.
				...
				if(state.fixedFlag$sample39) {
					...
					if((var29 == state.st[0])) {
						...
						for(int cv$loopIndex = 0; cv$loopIndex < state.noStates; cv$loopIndex += 1)
							...
							scratch.cv$var30$countGlobal[cv$loopIndex] = (scratch.cv$var30$countGlobal[cv$loopIndex] + state.distribution$sample57[0][cv$loopIndex]);
					}
				} else {
					...
					for(int cv$loopIndex = 0; cv$loopIndex < state.noStates; cv$loopIndex += 1)
																		// A local reference to the scratch space.
						scratch.cv$var30$countGlobal[cv$loopIndex] = (scratch.cv$var30$countGlobal[cv$loopIndex] + (state.distribution$sample57[0][cv$loopIndex] * cv$distributionProbability));
				}
			}
			for(int i$var50 = 1; i$var50 < state.samples; i$var50 += 1) {
				...
				if((1 <= index$i$40)) {
					...
					for(int cv$loopIndex = 0; cv$loopIndex < state.noStates; cv$loopIndex += 1)
																		// A local reference to the scratch space.
						scratch.cv$var30$countGlobal[cv$loopIndex] = (scratch.cv$var30$countGlobal[cv$loopIndex] + (state.distribution$sample57[(i$var50 - 1)][cv$loopIndex] * cv$distributionProbability));
				}
			}
		}
	...
	private final void logProbabilityDistribution$sample180() {
		...
		if(!state.fixedProbFlag$sample180) {
			...
			if(cv$sampleReached)
				...
				state.logProbability$var179 = cv$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			state.logProbability$cpu = (state.logProbability$cpu + cv$sampleAccumulator);
			...
			state.logProbability$cpu = (state.logProbability$cpu + state.logProbability$var179);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var179);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var179);
		}
	}
	...
	private final void logProbabilityDistribution$sample185() {
		...
		if(!state.fixedProbFlag$sample185) {
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			state.logProbability$mem = (state.logProbability$mem + cv$sampleAccumulator);
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$mem = (state.logProbability$mem + state.logProbability$var184);
	...
	private final void logProbabilityDistribution$sample190() {
		...
		if(!state.fixedProbFlag$sample190) {
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var189 = cv$sampleAccumulator;
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			state.logProbability$pageFaults = (state.logProbability$pageFaults + cv$sampleAccumulator);
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$pageFaults = (state.logProbability$pageFaults + state.logProbability$var189);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var189);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var189);
		}
	}
	...
	private final void logProbabilityDistribution$sample57() {
		...
		if(!state.fixedProbFlag$sample57) {
			...
			if(state.fixedFlag$sample57)
				...
				// 
				// Variable declaration of cv$accumulator moved.
				state.logProbability$st = (state.logProbability$st + state.logProbability$var56);
	...
	private final void logProbabilityValue$sample180() {
		...
		if(!state.fixedProbFlag$sample180) {
			...
			for(int i$var174 = 0; i$var174 < state.samples; i$var174 += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + ((0.0 < var177)?(DistributionSampling.logProbabilityGaussian(((state.cpu[i$var174] - state.cpuMean[state.st[i$var174]]) / Math.sqrt(var177))) - (Math.log(var177) * 0.5)):Double.NEGATIVE_INFINITY));
			}
	...
	private final void logProbabilityValue$sample185() {
		...
		if(!state.fixedProbFlag$sample185) {
			...
			for(int i$var174 = 0; i$var174 < state.samples; i$var174 += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + ((0.0 < var182)?(DistributionSampling.logProbabilityGaussian(((state.mem[i$var174] - state.memMean[state.st[i$var174]]) / Math.sqrt(var182))) - (Math.log(var182) * 0.5)):Double.NEGATIVE_INFINITY));
			}
	...
	private final void logProbabilityValue$sample190() {
		...
		if(!state.fixedProbFlag$sample190) {
			...
			for(int i$var174 = 0; i$var174 < state.samples; i$var174 += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + ((0.0 < var187)?(DistributionSampling.logProbabilityGaussian(((state.pageFaults[i$var174] - state.pageFaultsMean[state.st[i$var174]]) / Math.sqrt(var187))) - (Math.log(var187) * 0.5)):Double.NEGATIVE_INFINITY));
			}
	...
	private final void logProbabilityValue$sample57() {
		...
		if(!state.fixedProbFlag$sample57) {
			...
			for(int i$var50 = 1; i$var50 < state.samples; i$var50 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + ((((((0.0 <= cv$sampleValue) && (cv$sampleValue < state.noStates)) && (0 < state.noStates)) && (0.0 <= var54[cv$sampleValue])) && (var54[cv$sampleValue] <= 1.0))?Math.log(var54[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
	...
	public final void gibbsRound() {
		...
		// Infer the samples in reverse chronological order.
			...
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample164) {
				for(int var162 = (state.noStates - 1); var162 >= 0; var162 -= 1)
					inferSample164(var162);
			}
			
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample147) {
				for(int var145 = (state.noStates - 1); var145 >= 0; var145 -= 1)
					inferSample147(var145);
			}
			
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample130) {
				for(int var128 = (state.noStates - 1); var128 >= 0; var128 -= 1)
					inferSample130(var128);
			}
			
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample113) {
				for(int var111 = (state.noStates - 1); var111 >= 0; var111 -= 1)
					inferSample113(var111);
			}
			
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample95) {
				for(int var93 = (state.noStates - 1); var93 >= 0; var93 -= 1)
					inferSample95(var93);
			}
			
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample77) {
				for(int var75 = (state.noStates - 1); var75 >= 0; var75 -= 1)
					inferSample77(var75);
			}
			...
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample30) {
				for(int var29 = (state.noStates - 1); var29 >= 0; var29 -= 1)
					inferSample30(var29);
			}
		}
