...
final class Flip2CoinsMK3$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void inferSample17(int var16) {
		...
		if(state.constrainedFlag$sample17[var16])
			state.bias[var16] = Conjugates.sampleConjugateBetaBinomial(state.RNG$, state.a, state.b, cv$sum, cv$count);
	}
	...
	private final void logProbabilityValue$sample17() {
		if(!state.fixedProbFlag$sample17) {
			...
			for(int var16 = 0; var16 < state.coins; var16 += 1)
				cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityBeta(state.bias[var16], state.a, state.b));
