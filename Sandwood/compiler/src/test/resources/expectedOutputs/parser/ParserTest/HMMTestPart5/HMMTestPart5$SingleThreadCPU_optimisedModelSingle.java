...
import org.sandwood.compiler.tests.parser.HMMTestPart5$SingleThreadCPU.Scratch;
...
final class HMMTestPart5$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	public HMMTestPart5$SingleThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
	...
	private final void drawValueSample72(int i$var65) {
		state.st[i$var65] = (DistributionSampling.sampleCategorical(state.RNG$, state.m[state.st[(i$var65 - 1)]], 2) * 2);
	}
	...
	private final void inferSample28(int var27) {
		...
		for(int i$var65 = 1; i$var65 < state.samples; i$var65 += 1) {
			if(((var27 == state.st[(i$var65 - 1)]) && (state.fixedFlag$sample72 || state.constrainedFlag$sample72[(i$var65 - 1)]))) {
				...
				// 
												// A local reference to the scratch space.
				scratch.cv$var28$countGlobal[(state.st[i$var65] / 2)] = (scratch.cv$var28$countGlobal[(state.st[i$var65] / 2)] + 1.0);
			}
		}
	...
	private final void inferSample53() {
		...
		// Unrolled loop
		{
			...
			if(((1 < state.samples) && (state.fixedFlag$sample72 || state.constrainedFlag$sample72[0]))) {
				...
				// 
												// Substituted "i$var65" with its value "1".
				cv$accumulatedProbabilities = ((((((0.0 <= (state.st[1] / 2)) && ((state.st[1] / 2) < 2)) && (0.0 <= var69[(state.st[1] / 2)])) && (var69[(state.st[1] / 2)] <= 1.0))?Math.log(var69[(state.st[1] / 2)]):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
			...
			if((0 < state.samples)) {
				...
				// 
				// Substituted "j" with its value "0".
				cv$accumulatedProbabilities = ((((0.0 <= var86) && (var86 <= 1.0))?Math.log((state.flips[0]?var86:(1.0 - var86))):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
	...
	private final void inferSample72(int i$var65) {
		...
		// Unrolled loop
		{
			...
			if(((index$i$2_2 < state.samples) && (state.fixedFlag$sample72 || state.constrainedFlag$sample72[(index$i$2_2 - 1)]))) {
				...
				// Set an accumulator to sum the probabilities for each possible configuration of
				// inputs.
				cv$accumulatedProbabilities = ((((((0.0 <= (state.st[index$i$2_2] / 2)) && ((state.st[index$i$2_2] / 2) < 2)) && (0.0 <= sc$var69$1[(state.st[index$i$2_2] / 2)])) && (sc$var69$1[(state.st[index$i$2_2] / 2)] <= 1.0))?Math.log(sc$var69$1[(state.st[index$i$2_2] / 2)]):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
			...
			cv$accumulatedProbabilities = ((((0.0 <= var86) && (var86 <= 1.0))?Math.log((state.flips[i$var65]?var86:(1.0 - var86))):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
		...
		if(state.constrainedFlag$sample72[(i$var65 - 1)]) {
			...
			// 
												// cv$numStates's comment
			// variable marginalization
			state.st[i$var65] = (DistributionSampling.sampleCategorical(state.RNG$, scratch.cv$var71$stateProbabilityGlobal, 2) * 2);
		}
	}
	...
	private final void logProbabilityValue$sample72() {
		...
		if(!state.fixedProbFlag$sample72) {
			...
			for(int i$var65 = 1; i$var65 < state.samples; i$var65 += 1) {
				// The sample value to calculate the probability of generating
				int cv$sampleValue = (state.st[i$var65] / 2);
				double[] var69 = state.m[state.st[(i$var65 - 1)]];
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((((0.0 <= cv$sampleValue) && (cv$sampleValue < 2)) && (0.0 <= var69[cv$sampleValue])) && (var69[cv$sampleValue] <= 1.0))?Math.log(var69[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var71 = cv$sampleAccumulator;
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$st = (state.logProbability$st + state.logProbability$var71);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var71);
			
			// If this value is fixed, add it to the probability of this model producing the fixed
			// values
			if(state.fixedFlag$sample72)
				// Variable declaration of cv$accumulator moved.
				state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var71);
		}
	}
	...
	private final void logProbabilityValue$sample89() {
		...
		if(!state.fixedProbFlag$sample89) {
			...
			for(int j = 0; j < state.samples; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var86) && (var86 <= 1.0))?Math.log((state.flips[j]?var86:(1.0 - var86))):Double.NEGATIVE_INFINITY));
			}
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var88 = cv$sampleAccumulator;
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var88);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var88);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var88);
		}
	}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample72)
			state.logProbability$var71 = Double.NaN;
		state.logProbability$flips = 0.0;
		if(!state.fixedProbFlag$sample89)
			state.logProbability$var88 = Double.NaN;
	}
