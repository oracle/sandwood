...
/**
  * Class representing the Sandwood model HMMTestPart4b This is the class that
  * all user interactions with the model should occur through.
  */
public class HMMTestPart4b extends Model {
    ...
    private final ComputedObjectArrayInternal<boolean[][]> $flips = new ComputedObjectArrayInternal<boolean[][]>(this, "flips", true, org.sandwood.runtime.internal.model.util.BaseType.BOOLEAN, 3) {
        ...
        public void setFixed(boolean fixed) {
            synchronized(model) {
                system$c.set$fixedFlag$sample195(fixed);
            }
        }

        @Override
        public Immutability isFixed() {
            if(system$c.get$fixedFlag$sample195())
                return Immutability.FIXED;
            else
                return Immutability.FREE;
        }
    ...
    private final ComputedObjectArrayInternal<int[][]> $st = new ComputedObjectArrayInternal<int[][]>(this, "st", true, org.sandwood.runtime.internal.model.util.BaseType.INT, 3) {
        ...
        @Override
        public Immutability isFixed() {
            boolean fixedFlag$sample128 = system$c.get$fixedFlag$sample128();
            boolean fixedFlag$sample88 = system$c.get$fixedFlag$sample88();
            if(fixedFlag$sample128 && fixedFlag$sample88)
                return Immutability.FIXED;
            else if(fixedFlag$sample128 || fixedFlag$sample88)
    ...
    private void transferData(HMMTestPart4b$CoreInterface oldCore, HMMTestPart4b$CoreInterface newCore) {
        ...
        if(flips.isSet())
            newCore.set$fixedFlag$sample195(oldCore.get$fixedFlag$sample195());
        if(m.isSet())
