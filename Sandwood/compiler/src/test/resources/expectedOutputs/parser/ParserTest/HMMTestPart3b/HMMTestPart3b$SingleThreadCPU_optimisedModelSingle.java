	...
	private final void logProbabilityValue$sample97() {
		...
		if(!fixedProbFlag$sample97) {
			...
			for(int j = 5; j < (samples + 5); j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var94) && (var94 <= 1.0))?Math.log((flips[(j - 5)]?var94:(1.0 - var94))):Double.NEGATIVE_INFINITY));
			}
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var96);
