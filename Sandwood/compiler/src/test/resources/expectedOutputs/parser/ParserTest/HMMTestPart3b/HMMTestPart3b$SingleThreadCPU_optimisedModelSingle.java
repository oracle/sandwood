	...
	private final void logProbabilityValue$sample75() {
		...
		if(!state.fixedProbFlag$sample75) {
			...
			for(int i$var66 = 4; i$var66 < (state.samples + 3); i$var66 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((((0.0 <= cv$sampleValue) && (cv$sampleValue < 2)) && (0.0 <= var72[cv$sampleValue])) && (var72[cv$sampleValue] <= 1.0))?Math.log(var72[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
	...
	private final void logProbabilityValue$sample97() {
		...
		if(!state.fixedProbFlag$sample97) {
			...
			for(int j = 5; j < (state.samples + 5); j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var94) && (var94 <= 1.0))?Math.log((state.flips[(j - 5)]?var94:(1.0 - var94))):Double.NEGATIVE_INFINITY));
			}
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var96);
