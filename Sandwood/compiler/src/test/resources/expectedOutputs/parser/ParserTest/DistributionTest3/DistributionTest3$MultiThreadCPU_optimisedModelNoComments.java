...
double[] cv$var4$stateProbabilityGlobal;
	...
	private final void inferSample4() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var163) && (0 < $var163)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
			state.constrainedFlag$sample4 = true;
			double cv$accumulatedConsumerProbabilities = Double.NEGATIVE_INFINITY;
			double cv$consumerDistributionProbabilityAccumulator = 1.0;
			if(state.fixedFlag$sample6) {
				double var10 = ((double)(cv$valuePos + cv$valuePos) / state.v2);
				cv$accumulatedConsumerProbabilities = (((0.0 <= var10) && (var10 <= 1.0))?Math.log((state.v?var10:(1.0 - var10))):Double.NEGATIVE_INFINITY);
	...
	private final void inferSample6() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var176) && (0 < $var176)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
			state.constrainedFlag$sample6 = true;
			double cv$accumulatedConsumerProbabilities = Double.NEGATIVE_INFINITY;
			double cv$consumerDistributionProbabilityAccumulator = 1.0;
			if(state.fixedFlag$sample4) {
				double var10 = ((double)(state.v1 + state.v1) / cv$valuePos);
				cv$accumulatedConsumerProbabilities = (((0.0 <= var10) && (var10 <= 1.0))?Math.log((state.v?var10:(1.0 - var10))):Double.NEGATIVE_INFINITY);
				cv$consumerDistributionProbabilityAccumulator = 0.0;
			} else {
	...
	private final void logProbabilityDistribution$sample12() {
		if(!state.fixedProbFlag$sample12) {
			...
			if(state.fixedFlag$sample4) {
				if(state.fixedFlag$sample6) {
					double var10 = ((double)(state.v1 + state.v1) / state.v2);
					cv$distributionAccumulator = (((0.0 <= var10) && (var10 <= 1.0))?Math.log((state.v?var10:(1.0 - var10))):Double.NEGATIVE_INFINITY);
					cv$probabilityReached = 1.0;
				} else {
					for(int index$sample6$14 = 0; index$sample6$14 < state.weightings.length; index$sample6$14 += 1) {
						double cv$probabilitySample6Value15 = state.distribution$sample6[index$sample6$14];
						double var10 = ((double)(state.v1 + state.v1) / index$sample6$14);
						double cv$weightedProbability = (Math.log(cv$probabilitySample6Value15) + (((0.0 <= var10) && (var10 <= 1.0))?Math.log((state.v?var10:(1.0 - var10))):Double.NEGATIVE_INFINITY));
				...
				for(int index$sample4$3 = 0; index$sample4$3 < state.weightings.length; index$sample4$3 += 1) {
					...
					if(state.fixedFlag$sample6) {
						double var10 = ((double)(index$sample4$3 + index$sample4$3) / state.v2);
						double cv$weightedProbability = (Math.log(cv$probabilitySample4Value4) + (((0.0 <= var10) && (var10 <= 1.0))?Math.log((state.v?var10:(1.0 - var10))):Double.NEGATIVE_INFINITY));
						if((cv$weightedProbability < cv$distributionAccumulator))
							cv$distributionAccumulator = (Math.log((Math.exp((cv$weightedProbability - cv$distributionAccumulator)) + 1)) + cv$distributionAccumulator);
						else {
							if((cv$distributionAccumulator == Double.NEGATIVE_INFINITY))
						...
						for(int index$sample6$19 = 0; index$sample6$19 < state.weightings.length; index$sample6$19 += 1) {
							...
							double cv$weightedProbability = (Math.log(cv$probabilitySample6Value20) + (((0.0 <= var10) && (var10 <= 1.0))?Math.log((state.v?var10:(1.0 - var10))):Double.NEGATIVE_INFINITY));
							if((cv$weightedProbability < cv$distributionAccumulator))
								cv$distributionAccumulator = (Math.log((Math.exp((cv$weightedProbability - cv$distributionAccumulator)) + 1)) + cv$distributionAccumulator);
							else {
								if((cv$distributionAccumulator == Double.NEGATIVE_INFINITY))
	...
	private final void logProbabilityValue$sample12() {
		if(!state.fixedProbFlag$sample12) {
			double var10 = ((double)(state.v1 + state.v1) / state.v2);
			double cv$distributionAccumulator = (((0.0 <= var10) && (var10 <= 1.0))?Math.log((state.v?var10:(1.0 - var10))):Double.NEGATIVE_INFINITY);
			state.logProbability$v = cv$distributionAccumulator;
			state.logProbability$$model = (state.logProbability$$model + cv$distributionAccumulator);
			state.logProbability$$evidence = (state.logProbability$$evidence + cv$distributionAccumulator);
			state.fixedProbFlag$sample12 = (state.fixedFlag$sample4 && state.fixedFlag$sample6);
	...
	public final void forwardGeneration() {
		...
		if(!state.fixedFlag$sample6)
			state.v2 = DistributionSampling.sampleCategorical(state.RNG$, state.weightings, state.weightings.length);
		state.v = DistributionSampling.sampleBernoulli(state.RNG$, ((double)(state.v1 + state.v1) / state.v2));
	}
