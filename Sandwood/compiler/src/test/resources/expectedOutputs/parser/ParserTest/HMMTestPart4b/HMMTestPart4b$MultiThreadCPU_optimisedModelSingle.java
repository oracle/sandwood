...
final class HMMTestPart4b$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart4b$CoreInterface {
	...
	private final void logProbabilityValue$sample122() {
		...
		if(!fixedProbFlag$sample122) {
			...
			logProbability$st = (logProbability$st + logProbability$var119);
	...
	private final void logProbabilityValue$sample189() {
		...
		if(!fixedProbFlag$sample189) {
			...
			for(int l = 0; l < samples; l += 1) {
				for(int p = 0; p < samples; p += 1) {
					for(int n = 0; n < samples; n += 1) {
						...
						// 
						// The sample value to calculate the probability of generating
						cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var182) && (var182 <= 1.0))?Math.log((flips[l][n][p]?var182:(1.0 - var182))):Double.NEGATIVE_INFINITY));
					}
				}
			}
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var184);
