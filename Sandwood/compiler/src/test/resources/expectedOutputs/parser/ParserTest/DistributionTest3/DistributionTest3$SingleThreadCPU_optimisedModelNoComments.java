...
double[] cv$var4$stateProbabilityGlobal;
	...
	private final void inferSample4() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var47) && (0 < $var47)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	private final void inferSample6() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var60) && (0 < $var60)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
