...
class Flip2CoinsMK4$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Flip2CoinsMK4$CoreInterface {
	...
	// by sample task 23 drawn from Beta 20. Inference was performed using a Beta to Bernoulli/Binomial
	// conjugate prior.
	private final void sample23(int i, int threadID$cv$i, Rng RNG$) {
		...
		int cv$count = 0;
		{
			...
			{
				// Looking for a path between Sample 23 and consumer Bernoulli 38.
				{
					for(int j = 0; j < coins; j += 1) {
						if((i == j)) {
