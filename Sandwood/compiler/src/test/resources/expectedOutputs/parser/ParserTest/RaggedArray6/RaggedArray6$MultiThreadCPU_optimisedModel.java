...
final class RaggedArray6$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	private final void inferSample50() {
		...
		int cv$indexToChange = (int)((double)lengthCV$a$48_13 * DistributionSampling.sampleUniform(state.RNG$));
		...
		if((cv$movementRatio < 0))
			...
			for(int cv$loopIndex = 0; cv$loopIndex < cv$indexToChange; cv$loopIndex += 1) {
				...
				// 
				// A reference local to the function for the sample variable.
				double cv$temp = (state.d[cv$loopIndex] * (lengthCV$a$48_13 - 1));
		...
		double cv$rebalanceValue = (cv$proposedDifference / (lengthCV$a$48_13 - 1));
		{
			...
			// 
									// Constructing a random variable input for use later.
			double cv$accumulatedProbabilities = DistributionSampling.logProbabilityDirichlet(state.d, state.a[state.y], lengthCV$a$48_14);
	...
	private final void logProbabilityValue$sample50() {
		...
		if(!state.fixedProbFlag$sample50) {
			...
			// 
			// The sample value to calculate the probability of generating
			double cv$distributionAccumulator = DistributionSampling.logProbabilityDirichlet(state.d, state.a[state.y], lengthCV$a$48_16);
