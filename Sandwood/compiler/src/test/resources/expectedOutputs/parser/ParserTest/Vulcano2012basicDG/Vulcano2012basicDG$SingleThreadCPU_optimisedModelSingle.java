...
class Vulcano2012basicDG$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements Vulcano2012basicDG$CoreInterface {
	...
	private final void logProbabilityValue$sample112() {
		...
		// Using cached values.
			...
			// this sample
			logProbability$var106 = logProbability$lambda;
	...
	private final void logProbabilityValue$sample114() {
		...
		if(!fixedProbFlag$sample114) {
			...
			if((0 < numTimeSteps)) {
				for(int t = 0; t < numTimeSteps; t += 1) {
					...
					// 
					// The sample value to calculate the probability of generating
					cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityPoisson((arrivals[t] - reduceVar$numSales$4), lambda[t]));
				}
			}
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$var109 = cv$sampleAccumulator;
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var108 = logProbability$var109;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$arrivals = (logProbability$arrivals + logProbability$var109);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$$model = (logProbability$$model + logProbability$var109);
			
			// If this value is fixed, add it to the probability of this model producing the fixed
			// values
			if(fixedFlag$sample114)
				// Variable declaration of cv$accumulator moved.
				logProbability$$evidence = (logProbability$$evidence + logProbability$var109);
		}
	}
	...
	private final void initializeLogProbabilityFields() {
		...
		if((!fixedProbFlag$sample32 && (0 < numTimeSteps))) {
			for(int var29 = 0; var29 < avail[0].length; var29 += 1)
