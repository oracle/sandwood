...
<<<<<<< Upstream, based on origin/Creating_separate_classes_to_hold_the_model_data_prior_to_creating_classes_on_a_per_sample_basis
=======
...
>>>>>>> 5a865f8 Changing the way the variance for MH is selected. Previously this would have been selected by multiplying a constant by the square of the original value. Squaring the value was done to avoid issues with negative values. However this also created issues with values getting to small or too large. Now we have conditional assignment this can be replaced with a function that just computes the absolute value before multiplying it by a constant.
	@Override
