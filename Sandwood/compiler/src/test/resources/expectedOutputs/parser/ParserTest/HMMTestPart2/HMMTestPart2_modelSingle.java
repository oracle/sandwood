...
/**
  * Class representing the Sandwood model HMMTestPart2 This is the class that
  * all user interactions with the model should occur through.
  */
public class HMMTestPart2 extends Model {

    private HMMTestPart2$CoreInterface system$c = new HMMTestPart2$SingleThreadCPU(ExecutionTarget.singleThread);
    ...
    private final ComputedIntegerArrayInternal $st = new ComputedIntegerArrayInternal(this, "st", true) {
        ...
        @Override
        public Immutability isFixed() {
            if(system$c.get$fixedFlag$sample73())
                return Immutability.FIXED;
            else
                return Immutability.FREE;
        }
    ...
    @Override
    protected HMMTestPart2$CoreInterface setExecutionTargetInternal(ExecutionTarget target) {
        HMMTestPart2$CoreInterface newCore;
        switch(target.executionType) {
            case SingleThreadCPU:
                newCore = new HMMTestPart2$SingleThreadCPU(target);
                break;
            case MultiThreadCPU:
                newCore = new HMMTestPart2$MultiThreadCPU(target);
                break;
            default:
    ...
    private void transferData(HMMTestPart2$CoreInterface oldCore, HMMTestPart2$CoreInterface newCore) {
