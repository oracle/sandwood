...
import org.sandwood.compiler.tests.parser.Conditional2$SingleThreadCPU.Scratch;
import org.sandwood.compiler.tests.parser.Conditional2.State;
import org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU;
import org.sandwood.runtime.internal.model.state.CoreModelScratch;
import org.sandwood.runtime.internal.numericTools.DistributionSampling;
import org.sandwood.runtime.model.ExecutionTarget;

final class Conditional2$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
...
double[] cv$var4$stateProbabilityGlobal;
	...
	public Conditional2$SingleThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
	...
	private final void logProbabilityValue$sample21() {
		...
		if(!state.guard) {
			double cv$distributionAccumulator = (((0.0 <= state.var19) && (state.var19 < 1.0))?0.0:Double.NEGATIVE_INFINITY);
		...
		state.logProbability$value2 = (state.logProbability$value2 + cv$accumulator);
	...
	public final void forwardGeneration() {
		...
		if(state.guard)
			...
			state.var19 = DistributionSampling.sampleUniform(state.RNG$);
			state.value[0] = state.var19;
		}
