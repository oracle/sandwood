	...
	private final void logProbabilityValue$sample74() {
		if(!state.fixedProbFlag$sample74) {
			...
			for(int i$var45 = 0; i$var45 < state.n; i$var45 += 1) {
				...
				double cv$distributionAccumulator = ((0.0 < state.tau)?(DistributionSampling.logProbabilityGaussian(((state.y[i$var45] - (reduceVar$var70$3 + state.bias)) / Math.sqrt(state.tau))) - (Math.log(state.tau) * 0.5)):Double.NEGATIVE_INFINITY);
