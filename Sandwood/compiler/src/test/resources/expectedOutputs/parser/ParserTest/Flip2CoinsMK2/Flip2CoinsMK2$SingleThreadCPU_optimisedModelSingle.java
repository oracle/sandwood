...
final class Flip2CoinsMK2$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements Flip2CoinsMK2$CoreInterface {
		...
		if(!fixedProbFlag$sample45) {
			...
			for(int i = 0; i < samples; i += 1) {
				for(int j = 0; j < coins; j += 1) {
					...
					// 
					// The sample value to calculate the probability of generating
					cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var43) && (var43 <= 1.0))?Math.log((flips[i][j]?var43:(1.0 - var43))):Double.NEGATIVE_INFINITY));
				}
			}
			...
			// A guard to check if the sample value is ever reached.
			boolean cv$sampleReached = false;
			if(((0 < samples) && (0 < coins)))
				// Record that the sample was reached.
				cv$sampleReached = true;
			if(cv$sampleReached)
				logProbability$bernoulli = logProbability$var45;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var45);
