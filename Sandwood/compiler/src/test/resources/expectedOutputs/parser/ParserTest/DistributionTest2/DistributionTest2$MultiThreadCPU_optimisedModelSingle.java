...
class DistributionTest2$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements DistributionTest2$CoreInterface {
	...
	private final void logProbabilityDistribution$sample48() {
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var44 = logProbability$var45;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$v = (logProbability$v + logProbability$var45);
	...
	private final void logProbabilityValue$sample48() {
		...
		if(!fixedProbFlag$sample48) {
			...
			for(int j = 0; j < size; j += 1)
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(v[j], ((double)v1 / v2[j])));
			logProbability$var44 = cv$sampleAccumulator;
			...
			parallelFor(RNG$, 0, size, 1,
				(int forStart$j, int forEnd$j, int threadID$j, org.sandwood.random.internal.Rng RNG$1) -> { 
						...
						for(int j = forStart$j; j < forEnd$j; j += 1)
							v[j] = DistributionSampling.sampleBernoulli(RNG$1, ((double)v1 / v2[j]));
				}
			);
