...
final class DistributionTest2$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	private final void inferSample23(int i, int threadID$cv$i, Rng RNG$) {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			int $var397 = state.weightings.length;
			
			// An accumulator to allow the value for each distribution to be constructed before
			// it is added to the index probabilities.
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var397) && (0 < $var397)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	private final void inferSample5() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			// Constructing a random variable input for use later.
			int $var355 = state.weightings.length;
			
			// An accumulator to allow the value for each distribution to be constructed before
			// it is added to the index probabilities.
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var355) && (0 < $var355)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	private final void inferSample9() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			// Constructing a random variable input for use later.
			int $var380 = state.weightings.length;
			
			// An accumulator to allow the value for each distribution to be constructed before
			// it is added to the index probabilities.
			// 
									// Value of the variable at this index
			double cv$accumulatedProbabilities = (((((cv$valuePos < $var380) && (0 < $var380)) && (0.0 <= state.weightings[cv$valuePos])) && (state.weightings[cv$valuePos] <= 1.0))?Math.log(state.weightings[cv$valuePos]):Double.NEGATIVE_INFINITY);
	...
	private final void logProbabilityDistribution$sample41() {
		...
		if(!state.fixedProbFlag$sample41) {
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var41 = cv$sampleAccumulator;
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$v = (state.logProbability$v + state.logProbability$var41);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var41);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var41);
		}
	}
	...
	private final void logProbabilityValue$sample41() {
		...
		if(!state.fixedProbFlag$sample41) {
			...
			for(int j = 0; j < state.size; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var39) && (var39 <= 1.0))?Math.log((state.v[j]?var39:(1.0 - var39))):Double.NEGATIVE_INFINITY));
			}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample41)
			state.logProbability$var41 = Double.NaN;
	}
