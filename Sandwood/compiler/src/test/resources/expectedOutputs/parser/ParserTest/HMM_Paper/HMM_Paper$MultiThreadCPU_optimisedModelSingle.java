	...
	private final void logProbabilityValue$sample71() {
		...
		if(!state.fixedProbFlag$sample71) {
			...
			for(int i = 1; i < state.nFlips; i += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + ((((((0.0 <= cv$sampleValue) && (cv$sampleValue < state.nCoins)) && (0 < state.nCoins)) && (0.0 <= var68[cv$sampleValue])) && (var68[cv$sampleValue] <= 1.0))?Math.log(var68[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
