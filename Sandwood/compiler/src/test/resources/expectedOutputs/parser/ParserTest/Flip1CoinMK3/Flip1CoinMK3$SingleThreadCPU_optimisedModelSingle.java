	...
	private final void logProbabilityValue$sample20() {
		...
		if(!fixedProbFlag$sample20) {
			...
			for(int i$var16 = 0; i$var16 < samples; i$var16 += 1)
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityBernoulli(flips[i$var16], bias));
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$bernoulli = logProbability$var18;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var18);
