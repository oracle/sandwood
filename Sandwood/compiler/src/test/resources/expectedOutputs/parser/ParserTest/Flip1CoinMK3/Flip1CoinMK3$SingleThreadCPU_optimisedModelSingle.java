	...
	private final void logProbabilityValue$sample23() {
		...
		if(!state.fixedProbFlag$sample23) {
			...
			for(int i$var21 = 0; i$var21 < state.samples; i$var21 += 1) {
				...
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= state.bias) && (state.bias <= 1.0))?Math.log((state.flips[i$var21]?state.bias:(1.0 - state.bias))):Double.NEGATIVE_INFINITY));
			}
			...
			if((0 < state.samples))
				...
				cv$sampleReached = true;
			if(cv$sampleReached)
				state.logProbability$bernoulli = state.logProbability$var23;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var23);
