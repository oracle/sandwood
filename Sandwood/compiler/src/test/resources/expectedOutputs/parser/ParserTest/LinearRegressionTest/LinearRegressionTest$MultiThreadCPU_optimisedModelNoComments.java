...
class LinearRegressionTest$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements LinearRegressionTest$CoreInterface {
	...
	private final void logProbabilityValue$sample74() {
		if(!fixedProbFlag$sample74) {
			...
			for(int i$var45 = 0; i$var45 < n; i$var45 += 1) {
				...
				double cv$distributionAccumulator = (DistributionSampling.logProbabilityGaussian(((y[i$var45] - (reduceVar$var70$8 + bias)) / Math.sqrt(tau))) - (Math.log(tau) * 0.5));
	...
	private final void sample24(int var23) {
		...
		for(int i$var45 = 0; i$var45 < n; i$var45 += 1) {
			...
			for(int cv$reduction386Index = 0; cv$reduction386Index < var23; cv$reduction386Index += 1)
				reduceVar$var70$5 = (reduceVar$var70$5 + phi[i$var45][cv$reduction386Index]);
			for(int cv$reduction386Index = (var23 + 1); cv$reduction386Index < k; cv$reduction386Index += 1)
				reduceVar$var70$5 = (reduceVar$var70$5 + phi[i$var45][cv$reduction386Index]);
