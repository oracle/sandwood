	...
	private final void logProbabilityValue$sample44() {
		...
		if(!fixedProbFlag$sample44) {
			...
			for(int i = 0; i < length$observed; i += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = ((cv$sampleAccumulator + DistributionSampling.logProbabilityGaussian(((generated[i] - sample[i]) / Math.sqrt(var38)))) - (Math.log(var38) * 0.5));
			}
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$generated = (logProbability$generated + logProbability$var40);
