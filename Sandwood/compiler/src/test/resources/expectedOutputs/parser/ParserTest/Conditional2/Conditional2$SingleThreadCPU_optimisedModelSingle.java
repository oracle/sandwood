...
final class Conditional2$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void logProbabilityValue$sample21() {
		...
		if(!state.guard) {
			...
			cv$sampleAccumulator = (((0.0 <= state.var19) && (state.var19 < 1.0))?0.0:Double.NEGATIVE_INFINITY);
		}
		...
		// 
		// Accumulator for probabilities of instances of the random variable
		state.logProbability$value = (state.logProbability$value + cv$sampleAccumulator);
		...
		// 
		// Accumulator for probabilities of instances of the random variable
		state.logProbability$value2 = (state.logProbability$value2 + cv$sampleAccumulator);
