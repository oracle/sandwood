...
import org.sandwood.compiler.tests.parser.Flip1CoinMK1c$MultiThreadCPU.Scratch;
...
final class Flip1CoinMK1c$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	public Flip1CoinMK1c$MultiThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
		...
		parallelFor(state.RNG$, 0, state.samples, 1,
			(int forStart$var18, int forEnd$var18, int threadID$var18, org.sandwood.random.internal.Rng RNG$1) -> { 
					...
					for(int var18 = forStart$var18; var18 < forEnd$var18; var18 += 1)
						state.flips[var18] = DistributionSampling.sampleBernoulli(RNG$1, state.var6);
			}
		);
	}
