	...
	private final void logProbabilityDistribution$sample45() {
		...
		if(!state.fixedProbFlag$sample45) {
			...
			if(((!(state.cat == 1) && state.fixedFlag$sample31) && state.fixedFlag$sample45))
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$result = (state.logProbability$result + cv$sampleAccumulator);
			...
			if(((!(state.cat == 1) && state.fixedFlag$sample31) && state.fixedFlag$sample45))
				...
				// 
				// Variable declaration of cv$accumulator moved.
				state.logProbability$result = (state.logProbability$result + state.logProbability$var43);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var43);
			
			// If this value is fixed, add it to the probability of this model producing the fixed
			// values
			if(state.fixedFlag$sample45)
				// Variable declaration of cv$accumulator moved.
				state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var43);
		}
	}
	...
	private final void logProbabilityValue$sample45() {
		...
		if(!state.fixedProbFlag$sample45) {
			...
			if(!(state.cat == 1)) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = DistributionSampling.logProbabilityBinomial(state.var43, state.bias[state.cat], 10);
			}
