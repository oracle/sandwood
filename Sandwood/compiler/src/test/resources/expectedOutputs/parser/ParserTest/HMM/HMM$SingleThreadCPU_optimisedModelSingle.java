...
final class HMM$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void drawValueSample28(int var27) {
		DistributionSampling.sampleDirichlet(state.RNG$, state.v, state.states, state.m[var27]);
	}
	...
	private final void inferSample28(int var27) {
		...
		if(state.constrainedFlag$sample28[var27])
			...
			Conjugates.sampleConjugateDirichletCategorical(state.RNG$, state.v, scratch.cv$var28$countGlobal, state.m[var27], state.states);
	}
	...
	private final void logProbabilityValue$sample71() {
		...
		if(!state.fixedProbFlag$sample71) {
			...
			for(int i = 1; i < state.samples; i += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + ((((((0.0 <= cv$sampleValue) && (cv$sampleValue < state.states)) && (0 < state.states)) && (0.0 <= var68[cv$sampleValue])) && (var68[cv$sampleValue] <= 1.0))?Math.log(var68[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
	...
	public final void gibbsRound() {
		...
		// Infer the samples in reverse chronological order.
			...
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample45) {
				for(int var43 = (state.states - 1); var43 >= 0; var43 -= 1)
					inferSample45(var43);
			}
			
			// Constraints moved from conditionals in inner loops/scopes/etc.
			if(!state.fixedFlag$sample28) {
				for(int var27 = (state.states - 1); var27 >= 0; var27 -= 1)
					inferSample28(var27);
			}
		}
