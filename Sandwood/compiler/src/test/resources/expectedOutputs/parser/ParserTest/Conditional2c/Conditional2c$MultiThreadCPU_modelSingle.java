...
class Conditional2c$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements Conditional2c$CoreInterface {
	...
	public final void set$u(double cv$value) {
		...
		// Unset the fixed probability flag for sample 13 as it depends on u.
	...
	private final void logProbabilityValue$sample13() {
		...
		if(!fixedProbFlag$sample13) {
				...
				{
					{
						...
						double cv$weightedProbability = (Math.log(1.0) + (((var9 <= cv$sampleValue) && (cv$sampleValue <= var10))?(-Math.log((var10 - var9))):Double.NEGATIVE_INFINITY));
			...
			// Guard to ensure that v is only updated once for this probability.
			...
			// Looking for a path between Sample 13 and consumer double[] 31.
			{
				if(!guard) {
					if((0 == 0)) {
						// If the probability of the variable has not already been updated
				...
				{
					{
						...
						{
							if(guard) {
								// Looking for a path between Put 25 and consumer double[] 31.
								{
									if(guard) {
										if((0 == 0)) {
	...
	private final void sample7() {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$noStates; cv$valuePos += 1) {
			...
			guard = cv$currentValue;
			{
				...
				// Processing conditional point27.
				{
					{
						if(!cv$currentValue) {
							{
	...
	public final void propogateObservedValues() {
		...
		{
			if(!guard) {
				// Looking for a path between Put 27 and consumer double[] 31.
				{
					if(!guard) {
						if((0 == 0))
							v = value[0];
		...
		{
			// Looking for a path between Addition 15 and consumer double[] 31.
			{
				if(!guard) {
					if((0 == 0))
						u = (v - 1);
