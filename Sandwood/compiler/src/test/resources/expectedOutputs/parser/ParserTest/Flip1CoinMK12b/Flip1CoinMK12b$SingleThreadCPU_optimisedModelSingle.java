...
class Flip1CoinMK12b$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements Flip1CoinMK12b$CoreInterface {
	...
	private final void logProbabilityValue$sample20() {
		...
		if(!fixedProbFlag$sample20) {
			...
			if(guard1)
				...
				cv$sampleAccumulator = DistributionSampling.logProbabilityBeta(var16, 1.0, 1.0);
	...
	private final void logProbabilityValue$sample32() {
		...
		if(!fixedProbFlag$sample32) {
			...
			if((!guard1 && (guard2 <= 2)))
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = DistributionSampling.logProbabilityBeta((var28 * 2), 1.0, 1.0);
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$var28 = (logProbability$var28 + cv$sampleAccumulator);
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$var28 = (logProbability$var28 + logProbability$var26);
	...
	private final void logProbabilityValue$sample39() {
		...
		if(!fixedProbFlag$sample39) {
			...
			if((!guard1 && !(guard2 <= 2)))
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = DistributionSampling.logProbabilityBeta((var35 * 3), 1.0, 1.0);
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$var35 = (logProbability$var35 + cv$sampleAccumulator);
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$var35 = (logProbability$var35 + logProbability$var33);
			...
			if((!guard1 && !(guard2 <= 2)))
				...
				// 
				// Variable declaration of cv$accumulator moved.
				logProbability$bias = (logProbability$bias + logProbability$var33);
	...
	private final void sample32() {
		...
		double cv$proposedValue = ((Math.sqrt(cv$var) * DistributionSampling.sampleGaussian(RNG$)) + cv$originalValue);
		{
			...
			// 
			// Substituted "cv$temp$1$var24" with its value "1.0".
			// 
		...
		// Substituted "bias2" with its value "var28".
		bias = var28;
	...
	private final void sample39() {
		...
		double cv$proposedValue = ((Math.sqrt(cv$var) * DistributionSampling.sampleGaussian(RNG$)) + cv$originalValue);
		{
			...
			// it is added to the index probabilities.
			// 
			// Substituted "cv$temp$1$var31" with its value "1.0".
			// 
			// Set the current value to the current state of the tree.
			double cv$accumulatedProbabilities = DistributionSampling.logProbabilityBeta(cv$originalValue, 1.0, 1.0);
		...
		// Substituted "bias2" with its value "var35".
		bias = var35;
