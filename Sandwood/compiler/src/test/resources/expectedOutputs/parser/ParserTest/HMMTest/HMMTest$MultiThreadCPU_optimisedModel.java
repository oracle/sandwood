...
class HMMTest$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTest$CoreInterface {
	...
	private final void logProbabilityValue$sample51() {
		...
		if(!fixedProbFlag$sample51) {
			...
			for(int j = 0; j < samples; j += 1) {
				...
				// Store the sample task probability
				logProbability$sample51[j] = cv$distributionAccumulator;
			}
		...
		// Using cached values.
			...
			for(int j = 0; j < samples; j += 1) {
				// Variable declaration of cv$rvAccumulator moved.
				double cv$rvAccumulator = logProbability$sample51[j];
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
	...
	public final void allocator() {
		...
		logProbability$sample51 = new double[length$flipsMeasured];
