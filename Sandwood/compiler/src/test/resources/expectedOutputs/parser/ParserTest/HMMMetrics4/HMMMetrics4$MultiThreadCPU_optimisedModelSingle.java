...
class HMMMetrics4$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMMetrics4$CoreInterface {
	...
	private final void logProbabilityDistribution$sample275() {
		...
		if(!fixedProbFlag$sample275) {
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$var258 = cv$sampleAccumulator;
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var257 = logProbability$var258;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$metric_g = (logProbability$metric_g + logProbability$var258);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$$model = (logProbability$$model + logProbability$var258);
			
			// Variable declaration of cv$accumulator moved.
			logProbability$$evidence = (logProbability$$evidence + logProbability$var258);
		}
	}
	...
	private final void logProbabilityDistribution$sample71() {
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var64 = logProbability$var65;
			...
			if(fixedFlag$sample71)
				...
				// 
				// Variable declaration of cv$accumulator moved.
				logProbability$st = (logProbability$st + logProbability$var65);
	...
	private final void logProbabilityDistribution$sample90() {
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var83 = logProbability$var84;
			...
			if(fixedFlag$sample90)
				...
				// 
				// Variable declaration of cv$accumulator moved.
				logProbability$st = (logProbability$st + logProbability$var84);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$$model = (logProbability$$model + logProbability$var84);
			
			// If this value is fixed, add it to the probability of this model producing the fixed
			// values
			if(fixedFlag$sample90)
				// Variable declaration of cv$accumulator moved.
				logProbability$$evidence = (logProbability$$evidence + logProbability$var84);
		}
	}
				...
				for(int server = 0; server < noServers; server += 1) {
					for(int timeStep$var239 = 0; timeStep$var239 < length$metric[sample$var207][0]; timeStep$var239 += 1) {
						if(metric_valid_g[sample$var207][server][timeStep$var239]) {
							...
							// 
							// The sample value to calculate the probability of generating
							cv$sampleAccumulator = ((cv$sampleAccumulator + DistributionSampling.logProbabilityGaussian(((metric_g[sample$var207][server][timeStep$var239] - current_metric_mean[server][st[sample$var207][timeStep$var239]]) / Math.sqrt(var256)))) - (Math.log(var256) * 0.5));
						}
					}
				}
			}
		...
		if(!fixedProbFlag$sample90) {
			...
			for(int sample$var53 = 0; sample$var53 < noSamples; sample$var53 += 1) {
				for(int timeStep$var76 = 1; timeStep$var76 < length$metric[sample$var53][0]; timeStep$var76 += 1) {
					...
					// 
					// Store the value of the function call, so the function call is only made once.
					cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= cv$sampleValue) && (cv$sampleValue < var82.length))?Math.log(var82[cv$sampleValue]):Double.NEGATIVE_INFINITY));
				}
			}
			logProbability$var83 = cv$sampleAccumulator;
