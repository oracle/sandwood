...
public class HMMTestPart2b extends GeneratedAPIBuilder {
    @Override
    public CompilationDesc buildClass(CompilationOptions opts) {
        ...
        parFor(intVariable(0, location(27, 20, 27, 20)), samples.subtract(intVariable(1, location(27, 31, 27, 31)), location(27, 30, 27, 30)), intVariable(1, location(27, 19, 27, 22)), true, location(27, 9, 27, 33), (i) -> { 
            i.setAlias("i");
            i.setLocation(location(27, 17, 27, 17));
            st.put(i.add(intVariable(1, location(28, 18, 28, 18)), location(28, 17, 28, 17)), categorical(m.get(st.get(i, location(28, 39, 28, 41)), location(28, 36, 28, 42)), location(28, 23, 28, 43)).sample(location(28, 45, 28, 52)), location(28, 15, 28, 52));
        });

        parFor(intVariable(1, location(30, 20, 30, 20)), samples, intVariable(1, location(30, 19, 30, 22)), true, location(30, 9, 30, 31), (j) -> { 
            j.setAlias("j");
            j.setLocation(location(30, 17, 30, 17));
            flips.put(j, bernoulli(bias.get(st.get(j, location(31, 41, 31, 43)), location(31, 38, 31, 44)), location(31, 24, 31, 45)).sample(location(31, 47, 31, 54)), location(31, 18, 31, 54));
        });

        flips.observe(flipsMeasured, location(33, 15, 33, 36));

        Variable<?>[] $variableNames = {flipsMeasured, states, v, m, bias, samples, st, flips};
        String[] $constructorArgs = {"flipsMeasured"};
        Set<String> $helperClasses = new HashSet<>();
        return compileAPI(opts, $variableNames, "HMMTestPart2b", $helperClasses, "org.sandwood.compiler.tests.parser", $constructorArgs, getOriginalModel(), null);
    }
