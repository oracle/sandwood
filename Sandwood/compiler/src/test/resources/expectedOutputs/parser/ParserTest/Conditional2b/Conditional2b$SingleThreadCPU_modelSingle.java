...
import org.sandwood.compiler.tests.parser.Conditional2b$SingleThreadCPU.Scratch;
...
final class Conditional2b$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	public Conditional2b$SingleThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
				...
				state.guard = cv$currentValue;
				{
					...
					// Processing conditional point20.
					{
						{
							{
								if(cv$currentValue) {
