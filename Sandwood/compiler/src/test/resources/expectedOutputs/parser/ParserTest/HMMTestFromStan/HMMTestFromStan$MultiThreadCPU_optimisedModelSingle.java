...
class HMMTestFromStan$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestFromStan$CoreInterface {
	...
	private final void logProbabilityValue$sample76() {
		...
		if(!fixedProbFlag$sample76) {
			...
			for(int i$var67 = 1; i$var67 < samples; i$var67 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= cv$sampleValue) && (cv$sampleValue < var71.length))?Math.log(var71[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$st = (logProbability$st + logProbability$var73);
	...
	private final void sample57() {
		// Unrolled loop
		{
			...
			// Substituted "i$var67" with its value "1".
			if((1 < samples)) {
				// Processing sample task 76 of consumer random variable null.
				// Variable declaration of cv$temp$1$var71 moved.
				// 
				// Constructing a random variable input for use later.
				// 
				// Processing random variable 72.
				...
				// 
				// Substituted "i$var67" with its value "1".
				cv$accumulatedProbabilities = ((((0.0 <= st[1]) && (st[1] < cv$temp$1$var71.length))?Math.log(cv$temp$1$var71[st[1]]):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
	...
	private final void sample76(int i$var67) {
		// Unrolled loop
		{
			...
			// Variable declaration of cv$temp$0$var71 moved.
			// 
			// Constructing a random variable input for use later.
			...
			if((index$i$1_2 < samples)) {
				...
				// Set an accumulator to sum the probabilities for each possible configuration of
				// inputs.
				cv$accumulatedProbabilities = ((((0.0 <= st[index$i$1_2]) && (st[index$i$1_2] < cv$temp$1$var71.length))?Math.log(cv$temp$1$var71[st[index$i$1_2]]):Double.NEGATIVE_INFINITY) + cv$accumulatedProbabilities);
			}
			...
			// inputs.
			// 
			// Substituted "j" with its value "i$var67".
			// 
			// cv$temp$2$var86's comment
			// Constructing a random variable input for use later.
			// 
		...
		// 
		// Substituted "cv$valuePos" with its value "1".
		cv$accumulatedProbabilities = (DistributionSampling.logProbabilityBernoulli(flips[i$var67], bias[1]) + cv$accumulatedProbabilities);
		...
		// 
		// Get a local reference to the scratch space.
		st[i$var67] = DistributionSampling.sampleCategorical(RNG$, cv$var73$stateProbabilityGlobal);
	}
