...
final class HMMTestPart5$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart5$CoreInterface {
	...
	private final void logProbabilityValue$sample72() {
		...
		if(!fixedProbFlag$sample72) {
			...
			for(int i$var65 = 1; i$var65 < samples; i$var65 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				double cv$distributionAccumulator = (((0.0 <= cv$sampleValue) && (cv$sampleValue < 2))?Math.log(m[st[(i$var65 - 1)]][cv$sampleValue]):Double.NEGATIVE_INFINITY);
				...
				// 
				// Accumulator for sample probabilities for a specific instance of the random variable.
				logProbability$var70[(i$var65 - 1)] = cv$distributionAccumulator;
		...
		// Using cached values.
			...
			for(int i$var65 = 1; i$var65 < samples; i$var65 += 1) {
				...
				double cv$rvAccumulator = logProbability$sample72[(i$var65 - 1)];
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
				logProbability$var70[(i$var65 - 1)] = cv$rvAccumulator;
			}
	...
	private final void logProbabilityValue$sample89() {
		...
		if(!fixedProbFlag$sample89) {
			...
			for(int j = 0; j < samples; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				double cv$distributionAccumulator = Math.log((flips[j]?var86:(1.0 - var86)));
		...
		// Using cached values.
			...
			for(int j = 0; j < samples; j += 1) {
				// Variable declaration of cv$rvAccumulator moved.
				double cv$rvAccumulator = logProbability$sample89[j];
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
	...
	private final void sample53() {
		// Unrolled loop
		{
			...
			if((0 < samples)) {
				// Processing sample task 89 of consumer random variable null.
				// Variable declaration of cv$temp$4$var86 moved.
				// 
				// Constructing a random variable input for use later.
				// 
				// Processing random variable 87.
				...
				// 
				// Substituted "j" with its value "0".
				cv$accumulatedProbabilities = (Math.log((flips[0]?cv$temp$4$var86:(1.0 - cv$temp$4$var86))) + cv$accumulatedProbabilities);
			}
	...
	private final void sample72(int i$var65) {
		// Unrolled loop
		{
			...
			cv$accumulatedProbabilities = (Math.log((flips[i$var65]?cv$temp$4$var86:(1.0 - cv$temp$4$var86))) + cv$accumulatedProbabilities);
	...
	private final void initializeLogProbabilityFields() {
		...
		for(int i$var65 = 1; i$var65 < samples; i$var65 += 1)
			logProbability$var70[(i$var65 - 1)] = Double.NaN;
		if(!fixedProbFlag$sample72) {
			for(int i$var65 = 1; i$var65 < samples; i$var65 += 1)
				logProbability$sample72[(i$var65 - 1)] = Double.NaN;
		}
		for(int j = 0; j < samples; j += 1)
			logProbability$var87[j] = Double.NaN;
