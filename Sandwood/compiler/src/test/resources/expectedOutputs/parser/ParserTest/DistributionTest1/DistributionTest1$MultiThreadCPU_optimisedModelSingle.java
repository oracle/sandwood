		...
		for(int cv$valuePos = 0; cv$valuePos < cv$noStates; cv$valuePos += 1) {
			...
			if(fixedFlag$sample9) {
				...
				for(int index$sample9$4 = 0; index$sample9$4 < weightings.length; index$sample9$4 += 1) {
					...
					// 
					// Value of the variable at this index
					double var11 = ((double)cv$valuePos / index$sample9$4);
					
					// Record the probability of sample task 14 generating output with current configuration.
					// 
					// Substituted "cv$temp$2$var11" with its value "var11".
					if(((Math.log(cv$probabilitySample9Value5) + DistributionSampling.logProbabilityBernoulli(v, var11)) < cv$accumulatedConsumerProbabilities))
						// Substituted "cv$temp$2$var11" with its value "var11".
						cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(cv$probabilitySample9Value5) + DistributionSampling.logProbabilityBernoulli(v, var11)) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
					else {
						// If the second value is -infinity.
						if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
							// Substituted "cv$temp$2$var11" with its value "var11".
							cv$accumulatedConsumerProbabilities = (Math.log(cv$probabilitySample9Value5) + DistributionSampling.logProbabilityBernoulli(v, var11));
						else
							// Substituted "cv$temp$2$var11" with its value "var11".
							cv$accumulatedConsumerProbabilities = ((Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(cv$probabilitySample9Value5) + DistributionSampling.logProbabilityBernoulli(v, var11)))) + 1)) + Math.log(cv$probabilitySample9Value5)) + DistributionSampling.logProbabilityBernoulli(v, var11));
					}
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$noStates; cv$valuePos += 1) {
			...
			if(fixedFlag$sample7) {
				...
				for(int index$sample7$4 = 0; index$sample7$4 < weightings.length; index$sample7$4 += 1) {
					...
					// 
					// Value of the variable at this index
					double var11 = ((double)index$sample7$4 / cv$valuePos);
					
					// Record the probability of sample task 14 generating output with current configuration.
					// 
					// Substituted "cv$temp$2$var11" with its value "var11".
					if(((Math.log(cv$probabilitySample7Value5) + DistributionSampling.logProbabilityBernoulli(v, var11)) < cv$accumulatedConsumerProbabilities))
						// Substituted "cv$temp$2$var11" with its value "var11".
						cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(cv$probabilitySample7Value5) + DistributionSampling.logProbabilityBernoulli(v, var11)) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
					else {
						// If the second value is -infinity.
						if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
							// Substituted "cv$temp$2$var11" with its value "var11".
							cv$accumulatedConsumerProbabilities = (Math.log(cv$probabilitySample7Value5) + DistributionSampling.logProbabilityBernoulli(v, var11));
						else
							// Substituted "cv$temp$2$var11" with its value "var11".
							cv$accumulatedConsumerProbabilities = ((Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(cv$probabilitySample7Value5) + DistributionSampling.logProbabilityBernoulli(v, var11)))) + 1)) + Math.log(cv$probabilitySample7Value5)) + DistributionSampling.logProbabilityBernoulli(v, var11));
					}
