...
import org.sandwood.compiler.tests.parser.Flip2CoinsMK3$MultiThreadCPU.Scratch;
import org.sandwood.compiler.tests.parser.Flip2CoinsMK3.State;
...
final class Flip2CoinsMK3$MultiThreadCPU extends CoreModelMultiThreadCPU<State, Scratch> {
	...
	public Flip2CoinsMK3$MultiThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}

	// Pick a value from the distribution for the unconditioned variable from sample17
	private final void drawValueSample17(int var16, int threadID$cv$var16, Rng RNG$) {
		state.bias[var16] = DistributionSampling.sampleBeta(RNG$, state.a, state.b);
	}

	// Method to perform the inference steps to calculate new values for the samples generated
	// by sample task 17 drawn from Beta 5. Inference was performed using a Beta to Bernoulli/Binomial
	// conjugate prior.
	private final void inferSample17(int var16, int threadID$cv$var16, Rng RNG$) {
		if(true) {
			state.constrainedFlag$sample17[((var16 - 0) / 1)] = false;
									...
									{
										{
											for(int var43 = 0; var43 < state.length$flipsMeasured[j]; var43 += 1) {
												...
												if(cv$sampleConstrained) {
													...
													state.constrainedFlag$sample17[((var16 - 0) / 1)] = true;
													{
														{
															{
																{
			...
			if(state.constrainedFlag$sample17[((var16 - 0) / 1)]) {
				// Write out the value of the sample to a temporary variable prior to updating the
				// intermediate variables.
				double var17 = Conjugates.sampleConjugateBetaBinomial(RNG$, state.a, state.b, cv$sum, cv$count);
				
				// Guards to ensure that bias is only updated when there is a valid path.
				{
					{
						{
							state.bias[var16] = var17;
						}
					}
				}
			}
				...
				{
					{
						...
						double cv$sampleValue = state.bias[var16];
						{
							{
	...
	private final void logProbabilityValue$sample44() {
		...
		if(!state.fixedProbFlag$sample44) {
			...
			if(cv$sampleReached)
				// Store the random variable instance probability
				state.logProbability$var44 = cv$accumulator;
		...
		parallelFor(state.RNG$, 0, state.coins, 1,
			(int forStart$var16, int forEnd$var16, int threadID$var16, org.sandwood.random.internal.Rng RNG$1) -> { 
					...
					for(int var16 = forStart$var16; var16 < forEnd$var16; var16 += 1) {
						if(!state.fixedFlag$sample17)
							state.bias[var16] = DistributionSampling.sampleBeta(RNG$1, state.a, state.b);
					}
			}
		);
	...
	public final void gibbsRound() {
		...
		if(state.system$gibbsForward)
			...
			parallelFor(state.RNG$, 0, state.coins, 1,
				(int forStart$var16, int forEnd$var16, int threadID$var16, org.sandwood.random.internal.Rng RNG$1) -> { 
						...
						for(int var16 = forStart$var16; var16 < forEnd$var16; var16 += 1) {
							if(!state.fixedFlag$sample17)
								inferSample17(var16, threadID$var16, RNG$1);
						}
				}
			);
		...
		parallelFor(state.RNG$, 0, state.coins, 1,
			(int forStart$var16, int forEnd$var16, int threadID$var16, org.sandwood.random.internal.Rng RNG$1) -> { 
					...
					for(int var16 = forStart$var16; var16 < forEnd$var16; var16 += 1) {
						if(!state.constrainedFlag$sample17[((var16 - 0) / 1)])
							drawValueSample17(var16, threadID$var16, RNG$1);
					}
			}
		);
	}
