...
class HMMTestFromStan$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements HMMTestFromStan$CoreInterface {
	...
	private final void logProbabilityValue$sample72() {
		...
		if(!fixedProbFlag$sample72) {
			...
			if(cv$sampleReached)
				// Store the random variable instance probability
				logProbability$var71 = cv$accumulator;
		...
		// Using cached values.
			...
			for(int i$var65 = 1; i$var65 < samples; i$var65 += 1)
				// Record that the sample was reached.
				cv$sampleReached = true;
			double cv$sampleValue = logProbability$var71;
			cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
			cv$accumulator = (cv$accumulator + cv$rvAccumulator);
			if(cv$sampleReached)
	...
	private final void logProbabilityValue$sample87() {
		...
		if(!fixedProbFlag$sample87) {
			...
			if(cv$sampleReached)
				// Store the random variable instance probability
				logProbability$var86 = cv$accumulator;
		...
		// Using cached values.
			...
			for(int j = 0; j < samples; j += 1)
				// Record that the sample was reached.
				cv$sampleReached = true;
			double cv$sampleValue = logProbability$var86;
			cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
			cv$accumulator = (cv$accumulator + cv$rvAccumulator);
			if(cv$sampleReached)
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numNumStates; cv$valuePos += 1) {
				...
				}
				{
					...
					int cv$temp$1$$var175;
					{
						// Constructing a random variable input for use later.
						int $var175 = states;
						cv$temp$1$$var175 = $var175;
					}
					
					// An accumulator to allow the value for each distribution to be constructed before
					// it is added to the index probabilities.
					double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < cv$temp$1$$var175))?Math.log(cv$temp$0$var50[cv$currentValue]):Double.NEGATIVE_INFINITY));
												...
												{
													{
														...
														int cv$temp$3$$var184;
														{
															// Constructing a random variable input for use later.
															int $var184 = states;
															cv$temp$3$$var184 = $var184;
														}
														
														// Record the probability of sample task 72 generating output with current configuration.
														if(((Math.log(1.0) + (((0.0 <= st[i$var65]) && (st[i$var65] < cv$temp$3$$var184))?Math.log(cv$temp$2$var69[st[i$var65]]):Double.NEGATIVE_INFINITY)) < cv$accumulatedConsumerProbabilities))
															cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(1.0) + (((0.0 <= st[i$var65]) && (st[i$var65] < cv$temp$3$$var184))?Math.log(cv$temp$2$var69[st[i$var65]]):Double.NEGATIVE_INFINITY)) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
														else {
															// If the second value is -infinity.
															if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
																cv$accumulatedConsumerProbabilities = (Math.log(1.0) + (((0.0 <= st[i$var65]) && (st[i$var65] < cv$temp$3$$var184))?Math.log(cv$temp$2$var69[st[i$var65]]):Double.NEGATIVE_INFINITY));
															else
																cv$accumulatedConsumerProbabilities = (Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(1.0) + (((0.0 <= st[i$var65]) && (st[i$var65] < cv$temp$3$$var184))?Math.log(cv$temp$2$var69[st[i$var65]]):Double.NEGATIVE_INFINITY)))) + 1)) + (Math.log(1.0) + (((0.0 <= st[i$var65]) && (st[i$var65] < cv$temp$3$$var184))?Math.log(cv$temp$2$var69[st[i$var65]]):Double.NEGATIVE_INFINITY)));
														}
		...
		if(true) {
			...
			for(int cv$valuePos = 0; cv$valuePos < cv$numNumStates; cv$valuePos += 1) {
				...
				}
				{
					...
					int cv$temp$1$$var206;
					{
						// Constructing a random variable input for use later.
						int $var206 = states;
						cv$temp$1$$var206 = $var206;
					}
					
					// An accumulator to allow the value for each distribution to be constructed before
					// it is added to the index probabilities.
					double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < cv$temp$1$$var206))?Math.log(cv$temp$0$var69[cv$currentValue]):Double.NEGATIVE_INFINITY));
												...
												{
													{
														...
														int cv$temp$3$$var215;
														{
															// Constructing a random variable input for use later.
															int $var215 = states;
															cv$temp$3$$var215 = $var215;
														}
														
														// Record the probability of sample task 72 generating output with current configuration.
														if(((Math.log(1.0) + (((0.0 <= st[index$i$2_2]) && (st[index$i$2_2] < cv$temp$3$$var215))?Math.log(cv$temp$2$var69[st[index$i$2_2]]):Double.NEGATIVE_INFINITY)) < cv$accumulatedConsumerProbabilities))
															cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(1.0) + (((0.0 <= st[index$i$2_2]) && (st[index$i$2_2] < cv$temp$3$$var215))?Math.log(cv$temp$2$var69[st[index$i$2_2]]):Double.NEGATIVE_INFINITY)) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
														else {
															// If the second value is -infinity.
															if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
																cv$accumulatedConsumerProbabilities = (Math.log(1.0) + (((0.0 <= st[index$i$2_2]) && (st[index$i$2_2] < cv$temp$3$$var215))?Math.log(cv$temp$2$var69[st[index$i$2_2]]):Double.NEGATIVE_INFINITY));
															else
																cv$accumulatedConsumerProbabilities = (Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(1.0) + (((0.0 <= st[index$i$2_2]) && (st[index$i$2_2] < cv$temp$3$$var215))?Math.log(cv$temp$2$var69[st[index$i$2_2]]):Double.NEGATIVE_INFINITY)))) + 1)) + (Math.log(1.0) + (((0.0 <= st[index$i$2_2]) && (st[index$i$2_2] < cv$temp$3$$var215))?Math.log(cv$temp$2$var69[st[index$i$2_2]]):Double.NEGATIVE_INFINITY)));
														}
