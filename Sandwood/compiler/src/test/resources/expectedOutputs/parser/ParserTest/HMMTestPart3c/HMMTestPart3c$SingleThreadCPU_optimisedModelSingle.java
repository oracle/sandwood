...
class HMMTestPart3c$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements HMMTestPart3c$CoreInterface {
	...
	private final void logProbabilityValue$sample48() {
		...
		if(!fixedProbFlag$sample48) {
			...
			for(int i$var37 = 1; i$var37 < samples; i$var37 += 1)
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityCategorical(st[((i$var37 + i$var37) / 2)], m[st[(i$var37 - 1)]]));
	...
	private final void sample26(int var24) {
		...
		// 
		// Looking for a path between Sample 26 and consumer Bernoulli 54.
		for(int j = 0; j < samples; j += 1) {
			if((var24 == st[j])) {
				// Processing sample task 57 of consumer random variable null.
				// 
				// Include the value sampled by task 57 from random variable var54.
	...
	private final void sample35() {
		{
			...
			if((1 < samples))
				...
				// Substituted "i$var37" with its value "1".
				// 
				// Substituted "cv$temp$1$var44" with its value "var44".
				// 
				// Constructing a random variable input for use later.
				// 
			...
			if((0 < samples))
				...
				// Substituted "j" with its value "0".
				// 
				// Substituted "cv$temp$2$var53" with its value "var53".
				// 
				// Constructing a random variable input for use later.
				// 
				// Variable declaration of cv$currentValue moved.
	...
	// Method to perform the inference steps to calculate new values for the samples generated
	// by sample task 48 drawn from Categorical 45. Inference was performed using variable
	// marginalization.
	private final void sample48(int i$var37) {
		{
			...
			if((index$i$1_2 < samples))
				...
				// 
				// Substituted "cv$valuePos" with its value "0".
				cv$accumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[((index$i$1_2 + index$i$1_2) / 2)], m[0]) + cv$accumulatedProbabilities);
		...
		if((index$i$1_2 < samples))
			...
			// 
			// Substituted "cv$valuePos" with its value "1".
			cv$accumulatedProbabilities = (DistributionSampling.logProbabilityCategorical(st[((index$i$1_2 + index$i$1_2) / 2)], m[1]) + cv$accumulatedProbabilities);
		int j = ((i$var37 + i$var37) / 2);
		if((j < samples))
			// Processing sample task 57 of consumer random variable null.
			// 
	...
	public final void allocateScratch() {
		...
		// 
		// Variable to record the maximum value of Task Get 46. Initially set to the value
		// of putTask 18.
