...
import org.sandwood.compiler.tests.parser.HMMTestPart3c$SingleThreadCPU.Scratch;
...
final class HMMTestPart3c$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	public HMMTestPart3c$SingleThreadCPU(State state, ExecutionTarget target) {
		super(state, target);
		scratch = new Scratch();
	}
	...
	private final void inferSample28(int var27) {
		...
		for(int i$var64 = 1; i$var64 < state.samples; i$var64 += 1) {
			if(((var27 == state.st[(i$var64 - 1)]) && (state.fixedFlag$sample74 || state.constrainedFlag$sample74[(i$var64 - 1)]))) {
				...
				// 
												// A local reference to the scratch space.
				scratch.cv$var28$countGlobal[state.st[((i$var64 + i$var64) / 2)]] = (scratch.cv$var28$countGlobal[state.st[((i$var64 + i$var64) / 2)]] + 1.0);
			}
		}
	...
	private final void logProbabilityValue$sample74() {
		...
		if(!state.fixedProbFlag$sample74) {
			...
			for(int i$var64 = 1; i$var64 < state.samples; i$var64 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((((0.0 <= cv$sampleValue) && (cv$sampleValue < 2)) && (0.0 <= var71[cv$sampleValue])) && (var71[cv$sampleValue] <= 1.0))?Math.log(var71[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$st = (state.logProbability$st + state.logProbability$var73);
	...
	private final void logProbabilityValue$sample90() {
		...
		if(!state.fixedProbFlag$sample90) {
			...
			for(int j = 0; j < state.samples; j += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var87) && (var87 <= 1.0))?Math.log((state.flips[j]?var87:(1.0 - var87))):Double.NEGATIVE_INFINITY));
			}
			...
			if(cv$sampleReached)
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var89 = cv$sampleAccumulator;
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var89);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var89);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var89);
		}
	}
	...
	private final void initializeLogProbabilityFields() {
		...
		if(!state.fixedProbFlag$sample90)
			state.logProbability$var89 = Double.NaN;
	}
