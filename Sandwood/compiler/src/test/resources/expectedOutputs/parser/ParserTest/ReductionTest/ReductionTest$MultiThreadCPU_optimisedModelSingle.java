...
final class ReductionTest$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements ReductionTest$CoreInterface {
	...
	private final void sample62(int i$var58) {
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numStates; cv$valuePos += 1) {
			...
			for(int cv$reduction520Index = 0; cv$reduction520Index < i$var58; cv$reduction520Index += 1)
				// Execute the reduction function, saving the result into the return value.
				// 
				// Copy the result of the reduction into the variable returned by the reduction.
				// 
				// j$var80's comment
				// Set the right hand term to a value from the array st
				reduceVar$var82$6 = (reduceVar$var82$6 + st[cv$reduction520Index]);
			for(int cv$reduction520Index = (i$var58 + 1); cv$reduction520Index < noCats; cv$reduction520Index += 1)
				// Execute the reduction function, saving the result into the return value.
				// 
				// Copy the result of the reduction into the variable returned by the reduction.
				// 
