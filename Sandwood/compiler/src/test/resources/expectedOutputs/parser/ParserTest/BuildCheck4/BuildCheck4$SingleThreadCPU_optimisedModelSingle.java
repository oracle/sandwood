...
class BuildCheck4$SingleThreadCPU extends org.sandwood.runtime.internal.model.CoreModelSingleThreadCPU implements BuildCheck4$CoreInterface {
	...
	public final void set$fixedFlag$sample104(boolean calculationVariable$value) {
		...
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample104" with its value "calculationVariable$value".
	...
	public final void set$fixedFlag$sample44(boolean calculationVariable$value) {
		...
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample44" with its value "calculationVariable$value".
	...
	public final void set$fixedFlag$sample88(boolean calculationVariable$value) {
		...
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample88" with its value "calculationVariable$value".
	...
	public final void set$fixedFlag$sample91(boolean calculationVariable$value) {
		...
		// the flag to false.
		// 
		// Substituted "fixedFlag$sample91" with its value "calculationVariable$value".
	...
	private final void logProbabilityValue$sample104() {
		...
		if(!fixedProbFlag$sample104) {
			...
			if((ci_parsed && deploy_kws))
				...
				// 
				// Substituted "p_deploy_kws" with its value "0.27999999999999997".
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var80, 0.27999999999999997);
			...
			logProbability$var80 = calculationVariable$sampleAccumulator;
			...
			if((ci_parsed && deploy_kws))
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				logProbability$deploy_kws_certainty = (logProbability$deploy_kws_certainty + calculationVariable$sampleAccumulator);
		...
		// Using cached values.
			...
			if((ci_parsed && deploy_kws))
				...
				logProbability$deploy_kws_certainty = (logProbability$deploy_kws_certainty + logProbability$var80);
	...
	private final void logProbabilityValue$sample44() {
		...
		if(!fixedProbFlag$sample44) {
			...
			if((ci_parsed && deploy_action))
				...
				// Store the value of the function call, so the function call is only made once.
				// 
				// Substituted "p_deploy_action" with its value "0.7200000000000001".
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var32, 0.7200000000000001);
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$var32 = calculationVariable$sampleAccumulator;
			...
			if((ci_parsed && deploy_action))
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				logProbability$deploy_action_certainty = (logProbability$deploy_action_certainty + calculationVariable$sampleAccumulator);
		...
		// Using cached values.
			...
			if((ci_parsed && deploy_action))
				...
				// 
				// Variable declaration of calculationVariable$accumulator moved.
				logProbability$deploy_action_certainty = (logProbability$deploy_action_certainty + logProbability$var32);
	...
	private final void logProbabilityValue$sample46() {
		...
		if(!fixedProbFlag$sample46) {
			...
			if((ci_parsed && deploy_action)) {
				...
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var34, p_tested_deploy_action);
			}
		...
		// Using cached values.
			...
			if((ci_parsed && deploy_action))
				...
				// 
				// Variable declaration of calculationVariable$accumulator moved.
				logProbability$deploy_action_certainty = (logProbability$deploy_action_certainty + logProbability$var34);
	...
	private final void logProbabilityValue$sample49() {
		...
		if(!fixedProbFlag$sample49) {
			...
			if((ci_parsed && deploy_action)) {
				...
				// 
				// The sample value to calculate the probability of generating
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var37, p_release_workflow_trigger_deploy$var25);
			}
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var36 = logProbability$var37;
			...
			if((ci_parsed && deploy_action))
				...
				// 
				// Variable declaration of calculationVariable$accumulator moved.
				logProbability$deploy_action_certainty = (logProbability$deploy_action_certainty + logProbability$var37);
	...
	private final void logProbabilityValue$sample52() {
		...
		if(!fixedProbFlag$sample52) {
			...
			if((ci_parsed && deploy_action)) {
				...
				// 
				// The sample value to calculate the probability of generating
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var40, p_step_uses_secrets$var30);
			}
		...
		// Using cached values.
			...
			if((ci_parsed && deploy_action))
				...
				// 
				// Variable declaration of calculationVariable$accumulator moved.
				logProbability$deploy_action_certainty = (logProbability$deploy_action_certainty + logProbability$var40);
	...
	private final void logProbabilityValue$sample86() {
		...
		if(!fixedProbFlag$sample86) {
			...
			if((ci_parsed && deploy_command))
				...
				// Store the value of the function call, so the function call is only made once.
				// 
				// Substituted "p_deploy_command" with its value "0.6000000000000001".
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var64, 0.6000000000000001);
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$var64 = calculationVariable$sampleAccumulator;
			...
			if((ci_parsed && deploy_command))
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				logProbability$deploy_command_certainty = (logProbability$deploy_command_certainty + calculationVariable$sampleAccumulator);
		...
		// Using cached values.
			...
			// Updating random variable and model probabilities using cached probabilities for
			// this sample
			logProbability$var63 = logProbability$var64;
			...
			if((ci_parsed && deploy_command))
				...
				// 
				// Variable declaration of calculationVariable$accumulator moved.
				logProbability$deploy_command_certainty = (logProbability$deploy_command_certainty + logProbability$var64);
	...
	private final void logProbabilityValue$sample88() {
		...
		if(!fixedProbFlag$sample88) {
			...
			if((ci_parsed && deploy_command)) {
				...
				// 
				// The sample value to calculate the probability of generating
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var66, p_release_workflow_trigger_deploy$var57);
			}
	...
	private final void logProbabilityValue$sample91() {
		...
		if(!fixedProbFlag$sample91) {
			...
			if((ci_parsed && deploy_command)) {
				...
				// 
				// The sample value to calculate the probability of generating
				calculationVariable$sampleAccumulator = org.sandwood.runtime.internal.numericTools.DistributionSampling.logProbabilityBernoulli(var69, p_step_uses_secrets$var62);
			}
			...
			// 
			// Accumulator for probabilities of instances of the random variable
			logProbability$var69 = calculationVariable$sampleAccumulator;
		...
		// Using cached values.
			...
			if((ci_parsed && deploy_command))
				...
				// 
				// Variable declaration of calculationVariable$accumulator moved.
				logProbability$deploy_command_certainty = (logProbability$deploy_command_certainty + logProbability$var69);
