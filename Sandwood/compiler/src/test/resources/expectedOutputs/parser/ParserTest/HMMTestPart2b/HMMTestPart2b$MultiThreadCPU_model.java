...
class HMMTestPart2b$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart2b$CoreInterface {
	...
	public final void set$flips(boolean[] cv$value) {
		...
		// Unset the fixed probability flag for sample 86 as it depends on flips.
		fixedProbFlag$sample86 = false;
	}
	...
	private final void logProbabilityValue$sample71() {
		...
		if(!fixedProbFlag$sample71) {
			...
			for(int i$var64 = 0; i$var64 < (samples - 1); i$var64 += 1) {
				...
				// of all instances of the random variable.
				cv$accumulator = (cv$accumulator + cv$sampleAccumulator);
				logProbability$var69[((i$var64 - 0) / 1)] = cv$sampleAccumulator;
				
				// Store the sample task probability
				logProbability$sample71[((i$var64 - 0) / 1)] = cv$sampleProbability;
			}
		...
		// Using cached values.
			...
			for(int i$var64 = 0; i$var64 < (samples - 1); i$var64 += 1) {
				double cv$rvAccumulator = 0.0;
				double cv$sampleValue = logProbability$sample71[((i$var64 - 0) / 1)];
				cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
				logProbability$var69[((i$var64 - 0) / 1)] = cv$rvAccumulator;
			}
	...
	private final void logProbabilityValue$sample86() {
		...
		if(!fixedProbFlag$sample86) {
			...
			for(int j = 1; j < samples; j += 1) {
				...
				// of all instances of the random variable.
				cv$accumulator = (cv$accumulator + cv$sampleAccumulator);
				logProbability$var84[((j - 1) / 1)] = cv$sampleAccumulator;
				
				// Store the sample task probability
				logProbability$sample86[((j - 1) / 1)] = cv$sampleProbability;
			}
		...
		// Using cached values.
			...
			for(int j = 1; j < samples; j += 1) {
				double cv$rvAccumulator = 0.0;
				double cv$sampleValue = logProbability$sample86[((j - 1) / 1)];
				cv$rvAccumulator = (cv$rvAccumulator + cv$sampleValue);
				cv$accumulator = (cv$accumulator + cv$rvAccumulator);
				logProbability$var84[((j - 1) / 1)] = cv$rvAccumulator;
			}
		...
		for(int cv$valuePos = 0; cv$valuePos < cv$numNumStates; cv$valuePos += 1) {
			...
			st[(i$var64 + 1)] = cv$currentValue;
			{
				...
				int cv$temp$1$$var300;
				{
					// Constructing a random variable input for use later.
					int $var300 = states;
					cv$temp$1$$var300 = $var300;
				}
				
				// An accumulator to allow the value for each distribution to be constructed before
				// it is added to the index probabilities.
				double cv$accumulatedProbabilities = (Math.log(1.0) + (((0.0 <= cv$currentValue) && (cv$currentValue < cv$temp$1$$var300))?Math.log(cv$temp$0$var68[cv$currentValue]):Double.NEGATIVE_INFINITY));
											...
											{
												{
													...
													int cv$temp$3$$var309;
													{
														// Constructing a random variable input for use later.
														int $var309 = states;
														cv$temp$3$$var309 = $var309;
													}
													
													// Record the probability of sample task 71 generating output with current configuration.
													if(((Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var309))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)) < cv$accumulatedConsumerProbabilities))
														cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var309))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
													else {
														// If the second value is -infinity.
														if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
															cv$accumulatedConsumerProbabilities = (Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var309))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY));
														else
															cv$accumulatedConsumerProbabilities = (Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var309))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)))) + 1)) + (Math.log(1.0) + (((0.0 <= st[(index$i$1_2 + 1)]) && (st[(index$i$1_2 + 1)] < cv$temp$3$$var309))?Math.log(cv$temp$2$var68[st[(index$i$1_2 + 1)]]):Double.NEGATIVE_INFINITY)));
													}
									...
									{
										{
											{
												{
													double cv$temp$4$var83;
													{
														// Constructing a random variable input for use later.
														double var83 = bias[traceTempVariable$var82$4_1];
														cv$temp$4$var83 = var83;
													}
													
													// Record the probability of sample task 86 generating output with current configuration.
													if(((Math.log(1.0) + DistributionSampling.logProbabilityBernoulli(flips[j], cv$temp$4$var83)) < cv$accumulatedConsumerProbabilities))
														cv$accumulatedConsumerProbabilities = (Math.log((Math.exp(((Math.log(1.0) + DistributionSampling.logProbabilityBernoulli(flips[j], cv$temp$4$var83)) - cv$accumulatedConsumerProbabilities)) + 1)) + cv$accumulatedConsumerProbabilities);
													else {
														// If the second value is -infinity.
														if((cv$accumulatedConsumerProbabilities == Double.NEGATIVE_INFINITY))
															cv$accumulatedConsumerProbabilities = (Math.log(1.0) + DistributionSampling.logProbabilityBernoulli(flips[j], cv$temp$4$var83));
														else
															cv$accumulatedConsumerProbabilities = (Math.log((Math.exp((cv$accumulatedConsumerProbabilities - (Math.log(1.0) + DistributionSampling.logProbabilityBernoulli(flips[j], cv$temp$4$var83)))) + 1)) + (Math.log(1.0) + DistributionSampling.logProbabilityBernoulli(flips[j], cv$temp$4$var83)));
													}
	...
	public final void allocator() {
		...
		{
			logProbability$var69 = new double[(((((length$flipsMeasured - 1) - 1) - 0) / 1) + 1)];
		}
		
		// Constructor for logProbability$sample71
		{
			logProbability$sample71 = new double[(((((length$flipsMeasured - 1) - 1) - 0) / 1) + 1)];
		}
		
		// Constructor for logProbability$var84
		{
			logProbability$var84 = new double[((((length$flipsMeasured - 1) - 1) / 1) + 1)];
		}
		
		// Constructor for logProbability$sample86
		{
			logProbability$sample86 = new double[((((length$flipsMeasured - 1) - 1) / 1) + 1)];
		}
	...
	public final void forwardGeneration() {
		...
		for(int i$var64 = 0; i$var64 < (samples - 1); i$var64 += 1) {
			if(!fixedFlag$sample71)
				st[(i$var64 + 1)] = DistributionSampling.sampleCategorical(RNG$, m[st[i$var64]], states);
		}
	...
	public final void gibbsRound() {
		...
		// Infer the samples in reverse chronological order.
			...
			for(int i$var64 = ((samples - 1) - (((((samples - 1) - 1) - 0) % 1) + 1)); i$var64 >= ((0 - 1) + 1); i$var64 -= 1) {
				if(!fixedFlag$sample71)
					sample71(i$var64);
			}
	...
	private final void initializeLogProbabilityFields() {
		...
		for(int i$var64 = 0; i$var64 < (samples - 1); i$var64 += 1)
			logProbability$var69[((i$var64 - 0) / 1)] = 0.0;
		logProbability$st = 0.0;
		if(!fixedProbFlag$sample71) {
			for(int i$var64 = 0; i$var64 < (samples - 1); i$var64 += 1)
				logProbability$sample71[((i$var64 - 0) / 1)] = 0.0;
		}
		for(int j = 1; j < samples; j += 1)
			logProbability$var84[((j - 1) / 1)] = 0.0;
		logProbability$flips = 0.0;
		if(!fixedProbFlag$sample86) {
			for(int j = 1; j < samples; j += 1)
				logProbability$sample86[((j - 1) / 1)] = 0.0;
		}
	}
