	...
	private final void logProbabilityValue$sample26() {
		...
		if(!state.fixedProbFlag$sample26) {
			...
			for(int i = 0; i < state.samples; i += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var24) && (var24 <= 1.0))?Math.log((state.flips[i]?var24:(1.0 - var24))):Double.NEGATIVE_INFINITY));
			}
			...
			if(cv$sampleReached) {
				...
				// 
				// Accumulator for probabilities of instances of the random variable
				state.logProbability$var26 = cv$sampleAccumulator;
			}
			...
			if((0 < state.samples))
				...
				cv$sampleReached = true;
			if(cv$sampleReached)
				state.logProbability$bernoulli = state.logProbability$var26;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var26);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$model = (state.logProbability$$model + state.logProbability$var26);
			
			// Variable declaration of cv$accumulator moved.
			state.logProbability$$evidence = (state.logProbability$$evidence + state.logProbability$var26);
		}
	}
