...
final class RaggedArray2$SingleThreadCPU extends CoreModelSingleThreadCPU<State, Scratch> {
	...
	private final void logProbabilityValue$sample84() {
		...
		if(!state.fixedProbFlag$sample84) {
			...
			// 
									// The sample value to calculate the probability of generating
			double cv$distributionAccumulator = ((((((0.0 <= state.i) && (state.i < lengthCV$a$82_4)) && (0 < lengthCV$a$82_4)) && (0.0 <= var78[state.i])) && (var78[state.i] <= 1.0))?Math.log(var78[state.i]):Double.NEGATIVE_INFINITY);
