	...
	private final void logProbabilityValue$sample103() {
		...
		if(!state.fixedProbFlag$sample103) {
			...
			for(int j = 5; j < (state.samples + 5); j += 1) {
				...
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= var100) && (var100 <= 1.0))?Math.log((state.flips[(j - 5)]?var100:(1.0 - var100))):Double.NEGATIVE_INFINITY));
			}
			...
			state.logProbability$flips = (state.logProbability$flips + state.logProbability$var102);
	...
	private final void logProbabilityValue$sample78() {
		...
		if(!state.fixedProbFlag$sample78) {
			...
			for(int i$var66 = 4; i$var66 < (state.samples + 3); i$var66 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((((0.0 <= cv$sampleValue) && (cv$sampleValue < 2)) && (0.0 <= var75[cv$sampleValue])) && (var75[cv$sampleValue] <= 1.0))?Math.log(var75[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
			...
			// 
			// Variable declaration of cv$accumulator moved.
			state.logProbability$st = (state.logProbability$st + state.logProbability$var77);
		...
		parallelFor(state.RNG$, 5, (state.samples + 5), 1,
			(int forStart$j, int forEnd$j, int threadID$j, org.sandwood.random.internal.Rng RNG$1) -> { 
					...
					for(int j = forStart$j; j < forEnd$j; j += 1)
						state.flips[(j - 5)] = DistributionSampling.sampleBernoulli(RNG$1, state.bias[(1 - state.st[(j - 5)])]);
			}
		);
	}
