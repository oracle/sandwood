...
class HMMTestPart6$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart6$CoreInterface {
	...
	private final void logProbabilityValue$sample52() {
		...
		if(!fixedProbFlag$sample52) {
			...
			for(int i$var39 = 4; i$var39 < (samples + 3); i$var39 += 1)
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + DistributionSampling.logProbabilityCategorical(st[(i$var39 - 3)], m[(1 - st[(i$var39 - 4)])]));
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$st = (logProbability$st + logProbability$var50);
	...
	private final void logProbabilityValue$sample70() {
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var68);
	...
	private final void sample52(int i$var39) {
		{
			...
			// Constructing a random variable input for use later.
			double cv$accumulatedProbabilities = DistributionSampling.logProbabilityCategorical(0, m[(1 - st[(i$var39 - 4)])]);
		...
		// cv$temp$0$var48's comment
		// Constructing a random variable input for use later.
		double cv$accumulatedProbabilities = DistributionSampling.logProbabilityCategorical(1, m[(1 - st[(i$var39 - 4)])]);
		int index$i$1_2 = (i$var39 + 1);
		if((index$i$1_2 < (samples + 3)))
			// Processing sample task 52 of consumer random variable null.
			// 
