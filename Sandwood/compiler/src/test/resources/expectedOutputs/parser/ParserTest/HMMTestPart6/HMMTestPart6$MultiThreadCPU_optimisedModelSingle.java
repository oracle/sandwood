...
class HMMTestPart6$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements HMMTestPart6$CoreInterface {
	...
	private final void logProbabilityValue$sample107() {
		...
		if(!fixedProbFlag$sample107) {
			...
			logProbability$var104 = cv$sampleAccumulator;
		...
		// Using cached values.
			...
			// this sample
			logProbability$var103 = logProbability$var104;
			
			// Update the variable probability
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$flips = (logProbability$flips + logProbability$var104);
			
			// Add probability to model
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$$model = (logProbability$$model + logProbability$var104);
			
			// Variable declaration of cv$accumulator moved.
			logProbability$$evidence = (logProbability$$evidence + logProbability$var104);
		}
	}
	...
	private final void logProbabilityValue$sample82() {
		...
		if(!fixedProbFlag$sample82) {
			...
			for(int i$var68 = 4; i$var68 < (samples + 3); i$var68 += 1) {
				...
				// 
				// Store the value of the function call, so the function call is only made once.
				cv$sampleAccumulator = (cv$sampleAccumulator + (((0.0 <= cv$sampleValue) && (cv$sampleValue < var77.length))?Math.log(var77[cv$sampleValue]):Double.NEGATIVE_INFINITY));
			}
		...
		// Using cached values.
			...
			// 
			// Variable declaration of cv$accumulator moved.
			logProbability$st = (logProbability$st + logProbability$var79);
