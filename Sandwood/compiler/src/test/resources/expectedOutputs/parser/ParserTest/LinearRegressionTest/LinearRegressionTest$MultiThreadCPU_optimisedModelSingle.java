	...
	private final void inferSample24(int var23) {
		...
		for(int i$var45 = 0; i$var45 < n; i$var45 += 1) {
			...
			for(int cv$reduction429Index = 0; cv$reduction429Index < var23; cv$reduction429Index += 1)
				...
				reduceVar$var70$6 = (reduceVar$var70$6 + phi[i$var45][cv$reduction429Index]);
			
			// Substituted "j$var55" with its value "var23".
			for(int cv$reduction429Index = (var23 + 1); cv$reduction429Index < k; cv$reduction429Index += 1)
				// Execute the reduction function, saving the result into the return value.
				// 
				// Copy the result of the reduction into the variable returned by the reduction.
				// 
	...
	private final void logProbabilityValue$sample74() {
		...
		if(!fixedProbFlag$sample74) {
			...
			for(int i$var45 = 0; i$var45 < n; i$var45 += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = (cv$sampleAccumulator + ((0.0 < tau)?(DistributionSampling.logProbabilityGaussian(((y[i$var45] - (reduceVar$var70$9 + bias)) / Math.sqrt(tau))) - (Math.log(tau) * 0.5)):Double.NEGATIVE_INFINITY));
			}
