...
final class LinearRegressionTest$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements LinearRegressionTest$CoreInterface {
	...
	private final void sample24(int var23) {
		...
		for(int i$var45 = 0; i$var45 < n; i$var45 += 1) {
			...
			// 
			// Substituted "j$var55" with its value "var23".
			for(int cv$reduction406Index = 0; cv$reduction406Index < var23; cv$reduction406Index += 1)
				// Execute the reduction function, saving the result into the return value.
				// 
				// Copy the result of the reduction into the variable returned by the reduction.
				// 
				// j$var68's comment
				// Set the right hand term to a value from the array phi
				reduceVar$var70$6 = (reduceVar$var70$6 + phi[i$var45][cv$reduction406Index]);
			
			// Substituted "j$var55" with its value "var23".
			for(int cv$reduction406Index = (var23 + 1); cv$reduction406Index < k; cv$reduction406Index += 1)
				// Execute the reduction function, saving the result into the return value.
				// 
				// Copy the result of the reduction into the variable returned by the reduction.
				// 
