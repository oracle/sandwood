...
class LinearRegressionTest$MultiThreadCPU extends org.sandwood.runtime.internal.model.CoreModelMultiThreadCPU implements LinearRegressionTest$CoreInterface {
	...
	private final void logProbabilityValue$sample85() {
		...
		if(!fixedProbFlag$sample85) {
			...
			for(int i$var52 = 0; i$var52 < n; i$var52 += 1) {
				...
				// 
				// The sample value to calculate the probability of generating
				cv$sampleAccumulator = ((cv$sampleAccumulator + DistributionSampling.logProbabilityGaussian(((y[i$var52] - (reduceVar$var77$8 + bias)) / Math.sqrt(tau)))) - (Math.log(tau) * 0.5));
			}
	...
	private final void sample35(int var30) {
		...
		for(int i$var52 = 0; i$var52 < n; i$var52 += 1) {
			...
			// 
			// Substituted "j$var62" with its value "var30".
			for(int cv$reduction381Index = 0; cv$reduction381Index < var30; cv$reduction381Index += 1)
				// Execute the reduction function, saving the result into the return value.
				// 
				// Copy the result of the reduction into the variable returned by the reduction.
				// 
				// j$var75's comment
				// Set the right hand term to a value from the array phi
				reduceVar$var77$5 = (reduceVar$var77$5 + phi[i$var52][cv$reduction381Index]);
			
			// Substituted "j$var62" with its value "var30".
			for(int cv$reduction381Index = (var30 + 1); cv$reduction381Index < k; cv$reduction381Index += 1)
				// Execute the reduction function, saving the result into the return value.
				// 
				// Execute the reduction function, saving the result into the return value.
				// 
