...
public class TransMin<A extends NumberVariable<A>> extends TransBinOp<A, A, A> {
    ...
    public static <A extends NumberVariable<A>> TransTreeReturn<A> min(TransTreeReturn<A> left,
        ...
        if(left.equivalent(right))
            ...
            if(seenSize == initialSize + 2)
                ...
                TransTreeReturn<A> t = new TransMin<>(seen.get(0).tree, seen.get(1).tree);
                for(int i = 2; i < seenSize; i++)
                    t = new TransMin<>(t, seen.get(i).tree);
                return t;
            }
        }
    }
    ...
    public TransTreeReturn<A> applyTransformationInternal(Transformer t) {
        return min(t.transform(left), t.transform(right));
    }
    ...
    public TransTreeReturn<A> maxValueInternal(Bounds bounds) {
        return min(left.maxValue(bounds), right.maxValue(bounds));
    }

    @Override
    public TransTreeReturn<A> minValueInternal(Bounds bounds) {
        return min(left.minValue(bounds), right.minValue(bounds));
    }
