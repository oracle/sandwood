...
public class Cos<A extends NumberVariable<A>> extends NumberProducingDataflowTaskImplementation<DoubleVariable> {

    private final A input;

    private Cos(A input, Location location) {
        super(DFType.COS, VariableType.DoubleVariable, location, input);
        this.input = input;
    }
    ...
    public static <A extends NumberVariable<A>> DoubleVariable cos(A v) {
        return cos(v, null);
    }

    public static <A extends NumberVariable<A>> DoubleVariable cos(A v, Location location) {
        return DoubleVariable.doubleVariable(new Cos<>(v, location));
    }

    @Override
    public IRTreeReturn<DoubleVariable> getForwardIRinternal(CompilationContext compilationCtx) {
        return IRTree.cos(input.getForwardIR(compilationCtx));
    }
    ...
    public String checkInversionError(int argPos) {
        return "Cos is not invertible in the general case.";
    }
    ...
    public boolean equivalent(DataflowTask<?> other) {
        if(getType() != other.getType())
            return false;
        return input.equivalent(((Cos<?>) other).input);
    }
