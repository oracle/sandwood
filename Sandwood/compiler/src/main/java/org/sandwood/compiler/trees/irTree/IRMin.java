...
public class IRMin<A extends NumberVariable<A>> extends IRTreeReturn<A> {

    protected final IRTreeReturn<A> left;
    protected final IRTreeReturn<A> right;

    private IRMin(IRTreeReturn<A> left, IRTreeReturn<A> right) {
        super(IRTreeType.MIN);
        this.left = left;
        this.right = right;
    }
    ...
    public TransTreeReturn<A> toTransformationTree() {
        return TransTree.min(left.toTransformationTree(), right.toTransformationTree());
    }

    public static <A extends NumberVariable<A>> IRMin<A> min(IRTreeReturn<A> left, IRTreeReturn<A> right) {
        return new IRMin<>(left, right);
    }

    @Override
    public IRMin<A> applyTransformation(TreeTransformation t) {
        return new IRMin<>(t.transformReturn(left), t.transformReturn(right));
    }
